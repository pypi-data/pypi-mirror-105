...
class Indicator(PipelineStage):
    def __init__(self, **kwargs):
        ...
        self._vlines = kwargs.pop("vlines", False)
    ...
    def _indicator(self, df, panel=0):
        return [
            mpf.make_addplot(
                self._get_or_apply(df)[self._tgt_col],
                panel=panel,
                color=self._color,
                type="line",
                width=self._width,
...
class EMA(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Determining Exponential Moving Average (period={}) for: "{}"'.format(
...
class SMMA(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Determining Smoothed Moving Average (period={}) for: "{}"'.format(
                    self._period, self._tgt_col
                )
            )
        df[self._tgt_col] = (
            df[self._src_col].ewm(alpha=1.0 / self._period).mean().values.flatten()
        )
        return df
...
class RollingMax(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Determining Rolling Maximum (period={}) for: "{}"'.format(
...
class RateOfChange(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Determining Rate of Change (period={}) for: "{}"'.format(
                    self._period, self._src_col
                )
            )
        ...
        df[self._tgt_col] = (
            (df[self._src_col] - df["_historical_val"]) / df["_historical_val"]
        ) * 100
...
class TrueRange(Indicator):
    ...
    def _transform(self, df, verbose):
        ...
        df[self._tgt_col] = df[["_tr_comp_a_", "_tr_comp_b_", "_tr_comp_c_"]].max(
            axis=1
        )

        df.drop(
            ["_last_close_", "_tr_comp_a_", "_tr_comp_b_", "_tr_comp_c_"],
            axis=1,
            inplace=True,
        )
...
class ATR(Indicator):
    def __init__(
        self, tgt_col, period=5, high="High", low="Low", close="Close", **kwargs
    ):
        self._tgt_col = tgt_col
        self._high = high
        self._low = low
...
class SuperTrend(Indicator):
    ...
    def _transform(self, df, verbose):
        ...
        for i in range(self._period, len(df)):
            df["_lower_band"].iat[i] = (
                max(df["_basic_lower_band"].iat[i], df["_lower_band"].iat[i - 1])
                if df[self._close].iat[i - 1] > df["_lower_band"].iat[i - 1]
                else df["_basic_lower_band"].iat[i]
            )
            df["_upper_band"].iat[i] = (
                min(df["_basic_upper_band"].iat[i], df["_upper_band"].iat[i - 1])
                if df[self._close].iat[i - 1] < df["_upper_band"].iat[i - 1]
                else df["_basic_upper_band"].iat[i]
            )
        ...
        for i in range(self._period, len(df)):
            if (
                df["_trend"].iat[i - 1] < 0
                and df[self._close].iat[i] > df["_upper_band"].iat[i - 1]
            ):
                df["_trend"].iat[i] = 1
            elif (
                df["_trend"].iat[i - 1] > 0
                and df[self._close].iat[i] < df["_lower_band"].iat[i - 1]
            ):
                df["_trend"].iat[i] = -1
            else:
        ...
        if self._as_offset:
            df[self._tgt_col] = np.where(
                df["_trend"] > 0,
                df["_hl2"] - df["_lower_band"],
                df["_hl2"] - df["_upper_band"],
            )
        else:
            df[self._tgt_col] = np.where(
    ...
    def _indicator(self, df, panel=0):
        ...
        return [
            mpf.make_addplot(
                ss_upper, panel=panel, color="g", type="line", width=3, alpha=0.5
            ),
            mpf.make_addplot(
                ss_lower, panel=panel, color="r", type="line", width=3, alpha=0.5
            ),
        ]
...
class DonchianRibbon(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                "Calculating Donchian channels for a period of: {}".format(self._period)
            )
        ...
        df[self._tgt_col] = (
            df[["_main", "_t1", "_t2", "_t3", "_t4", "_t5", "_t6", "_t7", "_t8", "_t9"]]
            .sum(axis=1)
            .astype("int")
        )
    ...
    def _indicator(self, df, panel=0):
        ...
        return [
            mpf.make_addplot(
                pos, panel=panel, color="g", type="bar", width=0.75, alpha=self._alpha
            ),
            mpf.make_addplot(
                neg, panel=panel, color="r", type="bar", width=0.75, alpha=self._alpha
            ),
        ]
...
class Highest(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Calculating highest value for column "{}" over period {}'.format(
                    self._src_col, self._period
                )
            )

        df[self._tgt_col] = (
            df[self._src_col].rolling(self._period).max().shift(self._shift)
        )
        return df
...
class Lowest(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Calculating lowest value for column "{}" over period {}'.format(
                    self._src_col, self._period
                )
            )

        df[self._tgt_col] = (
            df[self._src_col].rolling(self._period).min().shift(self._shift)
        )
        return df
...
class AverageDirectionalIndex(Indicator):
    def __init__(
        self, tgt_col, period=5, close="Close", high="High", low="Low", **kwargs
    ):
        self._tgt_col = tgt_col
        self._close = close
        self._high = high
    ...
    def _transform(self, df, verbose):
        ...
        if verbose:
            print(
                "Calculating average directional index over period {}".format(
                    self._period
                )
            )
        ...
        pipeline = Pipeline(
            [
                ...
                pdp.ColByFrameFunc(
                    "pdm", lambda df: np.where(df["up"] > df["down"], df["up"], 0)
                ),
                pdp.ColByFrameFunc(
                    "ndm", lambda df: np.where(df["down"] > df["up"], df["down"], 0)
                ),
                SMMA("smma_pdm", "pdm", self._period),
                SMMA("smma_ndm", "ndm", self._period),
                ATR("atr", self._period, close=self._close),
                pdp.ColByFrameFunc(
                    "pdi", lambda df: (100 * df["smma_pdm"] / df["atr"])
                ),
                pdp.ColByFrameFunc(
                    "ndi", lambda df: (100 * df["smma_ndm"] / df["atr"])
                ),
                pdp.ColByFrameFunc(
                    "avg_di",
                    lambda df: (
...
class DeviationSpread(Indicator):
    ...
    def _indicator(self, df, panel=0):
        ...
        return [
            mpf.make_addplot(
                tmp["{}_lowest".format(self._tgt_col)],
                panel=panel,
                color="black",
                type="line",
                width=self._width,
            ...
            mpf.make_addplot(
                tmp["{}_high".format(self._tgt_col)],
                panel=panel,
                color="blue",
                type="line",
                width=self._width,
            ...
            mpf.make_addplot(
                tmp["{}_highest".format(self._tgt_col)],
                panel=panel,
                color="black",
                type="line",
                width=self._width,
...
class Backtest(Indicator):
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                "Backtesting via Buy/Sell signals from {} with starting amount {}".format(
...
class BuySell(Indicator):
    def __init__(
        ...
        trail_frac=None,
        sell_eod=False,
        buy_window=(None, None),
        sell_window=(None, None),
        **kwargs
    ):
        self._tgt_col = tgt_col
        self._src_col = src_col
    ...
    def _transform(self, df, verbose):
        if verbose:
            print(
                'Converting raw signals ({}) to BuySell timeline events ({}) with trailing stop ({})"'.format(
        ...
        eod_times = np.unique(
            ts_vals.groupby([ts_vals.dt.year, ts_vals.dt.month, ts_vals.dt.day])
            .transform("max")
            .values
        )

        eod_ind = (
            np.where(df.index.isin(eod_times), True, False) if self._sell_eod else []
        )
        ...
        for i in range(len(df)):
            ...
            buy_start = (
                cur_time.replace(
                    hour=int(self._buy_window[0].split(":")[0]),
                    minute=int(self._buy_window[0].split(":")[1]),
                )
                if self._buy_window[0]
                else cur_time.replace(hour=0, minute=0)
            )
            buy_end = (
                cur_time.replace(
                    hour=int(self._buy_window[1].split(":")[0]),
                    minute=int(self._buy_window[1].split(":")[1]),
                )
                if self._buy_window[0]
                else cur_time.replace(hour=23, minute=59)
            )
            ...
            if src_val.iat[i] > 0 and not in_pos and in_buy_window:
                ...
                if stopped_out or short_sig or is_eod:
                    ...
                    cur_stop_price = (
                        max(
                            cur_stop_price,
                            df[self._high].iat[i - 1] * (1.0 - self._trail_frac),
                        )
                        if self._trail_frac
                        else -1.0
                    )
