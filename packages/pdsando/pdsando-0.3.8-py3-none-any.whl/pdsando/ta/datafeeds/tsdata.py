...
def to_time_series_data(
    df, timespan, multiplier, index_col=None, source=None, category=None
):
    temp = df.copy()
    ...
    return TimeSeriesData(
        temp, timespan=timespan, multiplier=multiplier, source=source, category=category
    )
...
def match_to_resolution(ts_series, resolution):
    ...
    seconds_of_day = (
        3600 * ts_series.dt.hour + 60 * ts_series.dt.minute + ts_series.dt.second
    )
    ...
    if resolution.timespan == "second":
        ...
        seconds_to_delete = (
            ((ts_series.dt.day - 1) % resolution.multiplier) * 86400
        ) + seconds_of_day
    # elif timespan == 'week':
    ...
    return pd.to_datetime(
        (seconds_from_epoch - seconds_to_delete), unit="s", origin="unix"
    )
...
class TimeSeriesData(pd.DataFrame):
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(
            ...
            **{
                k: kwargs[k]
                for k in kwargs
                if k not in ["timespan", "multiplier", "source", "category"]
            },
        )
    ...
    @property
    def timespan(self):
        return self._resolution.timespan

    @property
    def multiplier(self):
        return self._resolution.multiplier
    ...
    def supplement(self, sup_data):
        ...
        renames = {}
        for c in sup_data.columns:
            if c.lower() in ('open', 'high', 'low', 'close', 'volume'):
                renames[c] = c + '_model'
        sup_data.rename(columns=renames, inplace=True)
        ...
        if sup_data.resolution > self.resolution:
            ...
            sup_data["_rank"] = (
                sup_data.index.to_series()
                .groupby(sup_data["_join_ts"])
                .rank(method="first", ascending=True)
            )
            temp = self.join(
                sup_data[sup_data["_rank"] == 1.0].set_index("_join_ts"),
                rsuffix="_model",
                how="left",
            )
        ...
        return TimeSeriesData(
            temp.drop(["_join_ts", "_rank"], errors="ignore", axis=1),
            timespan=self.timespan,
            multiplier=self.multiplier,
            source=self._source,
            category=self._category,
    ...
    def match_resolution(self, model_data):
        ...
        if model_data.timespan != self.timespan:
            raise NotImplementedError(
                "Currently cannot support matching different timespan (source: {} | target: {})".format(
        ...
        if model_data.multiplier > self.multiplier:
            ...
            temp = self.join(model_data, rsuffix="_model", how="right").fillna(
                method="ffill"
            )

        return TimeSeriesData(
            temp[list(self.columns)].copy(),
            timespan=self.timespan,
            multiplier=self.multiplier,
            source=self._source,
            category=self._category,
...
class Resolution:
    ...
    valid_timespans = {"second": 5, "minute": 4, "hour": 3, "day": 2, "week": 1}
    ...
    def __lt__(self, other):
        return Resolution.valid_timespans[self.timespan] < Resolution.valid_timespans[
            other.timespan
        ] or (
            Resolution.valid_timespans[self.timespan]
            == Resolution.valid_timespans[other.timespan]
            and
    ...
    def __ne__(self, other):
        return (
            Resolution.valid_timespans[self.timespan]
            != Resolution.valid_timespans[other.timespan]
            or self.multiplier != other.multiplier
        )
