...
def input_from_feature_columns(features, feature_columns, l2_reg, seed, prefix='', seq_mask_zero=True,
    ...
    ## 키는 임베딩의 이름이며, 값의 shape은 (batch, 1, output_dim)
    embedding_matrix_dict = create_embedding_matrix(feature_columns, l2_reg, seed, prefix=prefix,  # embedding layer 선언
                                                    seq_mask_zero=seq_mask_zero)
    # argument는 embedding(dict 타입), input(dict 타입), feature info(namedtuple 타입), sparse feature에 대한 embedding 정보를 dict형태로 반환
    ...
    if not support_group:
        group_embedding_dict = list(chain.from_iterable(group_embedding_dict.values())) # 2차원 리스트를 1차원 리스트로 만드는 방법: [[1, 2], [3, 4]] -> [1, 2, 3, 4]
