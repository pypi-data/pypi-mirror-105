...
problem2 = {'objective':{'y1':lambda x: -2*x**2,'y2':lambda x: 2*x**2, 'x0':lambda x: -2*x + x**2,'x1':lambda x: 2*x**2 ,'value':None},
           'constraints_1': {'y1':lambda x:0,'y2':lambda x:0,'x0':lambda x:x**3, 'x1':lambda x:-x,'value':0},
...
problem3 = {'objective':{'x1':lambda x: x**2 -6*x,'x2':lambda x: x**2 - 8*x ,'x3':lambda x: -0.5*x,'value':None},
...
problem4 = {'objective':{'x1':(lambda x: ((-(x**2) + 4*x + 2) if x > 0 else (0*x))),'x2':lambda x: ((-(x**2) + 2*x + 4) if x > 0 else (2+0*x)),'value':None},
