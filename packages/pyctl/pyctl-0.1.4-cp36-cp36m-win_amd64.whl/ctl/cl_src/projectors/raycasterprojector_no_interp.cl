...
// non-interpolating sampler with `0` as boundary color
constant sampler_t samp = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
// slightly earlier/later entry/exit point (1% of voxel size) allowed for numerical reasons
constant float3 EPS = (float3)(0.01f, 0.01f, 0.01f);

// the kernel
kernel void ray_caster( float increment_mm,
                        constant uint2* raysPerPixel,
                        constant float3* src_mm,
                        constant float3* corner_mm,
                        constant float3* voxelSize_mm,
                        constant double16* QR,
