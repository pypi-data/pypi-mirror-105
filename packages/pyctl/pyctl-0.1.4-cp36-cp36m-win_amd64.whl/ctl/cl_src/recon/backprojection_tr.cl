                               ...
                               global const float* restrict angleCorrPol)
{
    ...
    for(uint module = 0; module < nbModules; ++module)
    {
        ...
        // determine range of footprint (t direction) - rectangular shape
        rEdge = rCenter + (float4)(0.0f, 0.0f, +0.5f * (*voxelSize_mm).z, 0.0f);
        t_plusMinus.s0 = dot(pMatRow1, rEdge) / dot(pMatRow2, rEdge);
        rEdge = rCenter + (float4)(0.0f, 0.0f, -0.5f * (*voxelSize_mm).z, 0.0f);
        t_plusMinus.s1 = dot(pMatRow1, rEdge) / dot(pMatRow2, rEdge);

        // check if voxel is projected outside detector
        if(t_plusMinus.s0 + 0.5f >= projDim.y || t_plusMinus.s1 + 0.5f < 0.0f)
            continue;

        // clamp range to detector size [0, projDim.y-1]
        tRange.s0 = max((int)(t_plusMinus.s0 + 0.5f), 0);
        tRange.s1 = min((int)(t_plusMinus.s1 + 0.5f), (int)(projDim.y) - 1);

        // compute F2 footprint values and accumulate backprojected values
        for (t = 0; t <= tRange.s1 - tRange.s0; t++)
        {
            F2 = ( min((float)(tRange.s0 + t) + 0.5f, t_plusMinus.s1)
                 - max((float)(tRange.s0 + t) - 0.5f, t_plusMinus.s0) )
