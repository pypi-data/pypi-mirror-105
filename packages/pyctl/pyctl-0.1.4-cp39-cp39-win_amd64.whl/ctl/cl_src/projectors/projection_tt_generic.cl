                  ...
                  local int* restrict localMemArray )
{
    ...
    for(voxel.s0 = xStart; voxel.s0 < xEnd; ++voxel.s0)
    {
        ...
        for(voxel.s1 = 0; voxel.s1<par_locI.s1; ++voxel.s1)
        {
            ...
            for(z = 0; z < par_locI.s2; ++z)
            {
                ...
                voxel.s2   = z;
                voxelValue = read_imagef(cube, voxel).s0 * intConverter;
                ...
                for(t = 0; t <= tRange.s1 - tRange.s0; ++t)
                {
                    ...
                    for (s = 0; s <= sLength; ++s)
                    {
                        ...
                        atomic_add(&localMemArray[ tempLU + s ], (int)round(gammaCT_f((float)sRange.s0 + sPos,tau) * F2));
                    }
                }
            }
        }
 ...
 if(xWorkerID == 0)
     for(int pix = 0; pix < boundary; ++pix)
         projData[pix + projOffset] = localMemArray[pix] * invIntConverter * angleCorrAzi[aziCorrOffset + pix%par_locI.s3]
