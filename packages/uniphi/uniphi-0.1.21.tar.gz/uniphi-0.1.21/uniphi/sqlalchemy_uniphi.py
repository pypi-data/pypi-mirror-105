...
class UniphiDialect(default.DefaultDialect):
    ...
    def get_view_names(self, connection, schema=None, **kw):
        ...
        _logger.info("got conn object {conn}".format(conn=connection))
        engine = connection
    ...
    def _get_table_columns(self, connection, uniphi_view_name):
        ...
        try:
            ...
            full_table = full_table.split(":")[0]
            connection.connection.cursor().setContextForQuery(uuid)
            rows = connection.connection.cursor().execute('DESCRIBE {}'.format(full_table))
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        _logger.info("**************************************** Type of conn object is {conn}".format(conn=type(connection)))
