...
class BaseError(Exception):
    ...
    def __init__(
        self, message: str, *, response: Response, code: str, reference: str, trace_id: str, status: int
    ):
    ...
    def from_response(cls, response: Any) -> "BaseError":
        ...
        for ErrorClass in BaseError.__subclasses__():
            if error["code"] == ErrorClass.code:
                return ErrorClass(
                    error["message"], response=response, code=error["code"], reference=error["reference"], trace_id=error["traceId"], status=error["status"]
                )
        ...
        return cls(error["message"], response=response, code=error["code"], reference=error["reference"], trace_id=error["traceId"], status=error["status"])
