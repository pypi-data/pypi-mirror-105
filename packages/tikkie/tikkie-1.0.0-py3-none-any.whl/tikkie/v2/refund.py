...
from tikkie.v2.models import Refund
...
def create(
    *,
    ...
    response = s().post(
        f"paymentrequests/{payment_request_token}/payments/{payment_token}/refunds", json=payload
    )
    return Refund.parse_raw(response.content)


def get(*, payment_request_token: str, payment_token: str, refund_token: str) -> Refund:
    """
