...
def get_heatmap(kin, threshold, stepsize=50):
    ...
    try:
        while True:
            ...
            im2, contours, hierarchy = cv2.findContours(b, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
...
def find_user_area(img):
    ...
    contours, hierarchy = cv2.findContours(tmp, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
    res = []
    for cnt in contours:
        M = cv2.moments(cnt)
        if M['m00'] == 0:
