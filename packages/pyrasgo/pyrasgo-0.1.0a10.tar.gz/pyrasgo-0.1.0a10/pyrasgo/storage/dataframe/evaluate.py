...
class Evaluate(Connection):
    ...
    def profile(self, df: pd.DataFrame, 
        ...
        histo_data = {}
        for col_label in p_df:
            try: #histogram for numeric
                count, edges = np.histogram(p_df[col_label], bins='auto', density=False)
                df_hist = pd.DataFrame(zip(count, edges), columns=['height','beginVal'])
                fhist = df_hist.to_dict(orient="records")
            except: #histogram for non-numeric
                dh = p_df[col_label].apply(str).value_counts().rename_axis('beginVal').reset_index(name='height')
