...
from ae.gui_app import (                                                                                  # type: ignore
    FLOW_KEY_SEP, flow_action, flow_class_name, flow_path_id, id_of_flow, module_globals, update_tap_kwargs,
    MainAppBase)
...
def id_of_state_help(state_name: str) -> str:
    ...
    """
    return f'{APP_STATE_HELP_ID_PREFIX}{state_name}'
...
class HelpAppBase(MainAppBase, ABC):
    ...
    def help_app_state_display(self, help_vars: Dict[str, Any], changed: bool = False) -> bool:
        ...
        help_id = id_of_state_help(state_name)
    ...
    def starting_tour(self, tour_class: Optional[Type['TourBase']] = None) -> bool:
        ...
        if tour_layout_class:
            ...
            self.vpo("HelpAppBase.starting_tour called but tour overlay/layout class got not set")
...
class TourBase:
    ...
    """ set to True for to automatically switch tour pages within the seconds specified by page_data['next_page_delay'].
    """
    ...
    (empty string)      tour page fstring message text template.
    app_flow_delay      time in seconds to wait until app flow change is fully completed (def=1.2, >0.9 for auto-width).
    app_reset_delay     time in seconds between app reset/close-popups and flow reset/prepare-first-tour-page (def=0.3).
    back                caption of tour previous page button (def='back').
    help_vars           additional help variables, e.g. `help_translation` providing context help text.
    next                caption of tour next page button (def='next').
    ...
    def __init__(self, main_app: HelpAppBase):
        ...
        self.page_data = deepcopy(self.page_data)   # copy defaults to instance attr, overwritten via load_page_data()
    ...
    def load_page_data(self):
        ...
        page_data = dict(help_vars=dict())

        help_translation = translation_short_help_id(id_of_flow_help(page_id))[0]
        if not help_translation:
            help_translation = translation_short_help_id(id_of_state_help(page_id))[0]
        ...
        page_data.update(tour_translation if isinstance(tour_translation, dict) else {'': tour_translation})
    ...
    def setup_explained_widget(self):
        ...
        wid = self.main_app.widget_by_flow_id(page_id)
        if not wid:
            wid = self.main_app.widget_by_state_name(page_id)
        if wid:
    ...
    def setup_layout(self):
        ...
        page_idx = self.page_idx
        page_cnt = len(self.page_ids)
        assert 0 <= page_idx < page_cnt, f"page_idx ({page_idx}) has to be positive and less than {page_cnt}"
        page_data = self.page_data
        layout = self.layout
        main_app = self.main_app

        main_app.vpo(f"TourBase.setup_layout(): page{page_idx}/{page_cnt} id={self.page_ids[page_idx]} layout={layout}")
        ...
        glo_vars, help_vars = main_app.help_variables(page_data['help_vars'])

        layout.title_text = self.tour_text(page_data.get('title_text'), glo_vars, help_vars)
        layout.page_text = self.tour_text(page_data.get('page_text'), glo_vars, help_vars)
        layout.tip_text = self.tour_text(page_data.get(''), glo_vars, help_vars)
        layout.next_text = page_data.get('next', get_text('next')) if page_idx + 1 < page_cnt else ""
        layout.prev_text = page_data.get('back', get_text('back')) if page_idx > 0 else ""
    ...
    @staticmethod
    def tour_text(text: str, glo_vars: Dict[str, Any], loc_vars: Dict[str, Any]) -> str:
        """ load, translate and f-string-evaluate text of tour layout/overlay child widget.

        :param text:            text to load, translate and f-string-evaluate.
        :param glo_vars:        global variables.
        :param loc_vars:        local variables.
        :return:                text of translation/help/tour id.
        """
        if text is None:
            return ""
        return get_f_string(text, glo_vars=glo_vars, loc_vars=loc_vars) or ""
...
class TourDropdownFromButton(TourBase):
    ...
    def setup_page_ids(self):
        ...
        if self.page_idx == 1 and len(page_ids) == 2 and page_ids[1] == TourDropdownFromButton.determine_page_ids:
            ...
            top_most_popup = self.main_app.popups_opened()[-1]
            children = self.main_app.widget_tourable_children_page_ids(top_most_popup)
            if children:
                page_ids.extend(children)
                self.load_page_data()   # reload page data of 2nd tour page (explaining the first dropdown child)
            else:
                self.main_app.po(f"TourDropDownFromButton.setup_page_ids missing tour-able child in {top_most_popup}")
...
class OnboardingTour(TourBase):
    ...
    page_ids = ['', 'help_activate', 'tooltip', 'start_tour', 'explained_widget', 'switch_page', 'stop_tour']


class UserPreferencesTour(TourDropdownFromButton):
    """ user preferences menu tour. """
    auto_switch_pages = True
    # resulting in page_data float val: page_data = deepcopy(TourDropdownFromButton.page_data)['next_page_delay'] = 3.6
    page_data = dict(help_vars=dict(), next_page_delay=3.6)
    page_ids = [id_of_flow('open', 'user_preferences'), TourDropdownFromButton.determine_page_ids]
