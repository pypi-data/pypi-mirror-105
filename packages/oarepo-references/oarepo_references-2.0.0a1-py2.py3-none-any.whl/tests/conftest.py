...
from tests.utils import TestRecord
...
@pytest.fixture(scope="module")
def references_api(app):
    """Returns an instance of RecordReferenceAPI."""
    return RecordReferenceAPI(app)
...
def app_config(app_config):
    ...
    app_config = dict(
        ...
        INDEXER_RECORD_TO_INDEX='tests.utils.test_record_to_index'
    )
...
@pytest.fixture
def cleaned_dependencies(app):
    dependencies = app.extensions['oarepo-references'].dependencies
    cloned = {**dependencies}
    dependencies.clear()
    try:
        yield
    finally:
        dependencies.update(cloned)


@pytest.fixture(scope='function')
def test_es(app):
    RecordIndexer().client.indices.delete(TEST_INDEX, ignore=[404])
    RecordIndexer().client.indices.create(TEST_INDEX, body={
        'mappings': {
            'properties': {
                'n': {
                    'type': 'nested',
