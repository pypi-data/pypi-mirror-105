...
class AppService:
    ...
    def run(self, template_path=None, data=None, state=None, initial_events=None):
        ...
        if template_path is not None and not file_exists(template_path):
    ...
    def get_template_path(self):
        ...
        if file_exists(config_path):
            ...
            if self._template_path is None:
                ...
                res_path = os.path.join(self.repo_dir, self._template_path)
        if self._template_path is None:
    ...
    def compile_template(self, root_source_dir=None):
        ...
        template_path = self.get_template_path()
        if template_path is None:
            self.logger.warning("Template file not found")
            return
        with open(template_path, 'r') as file:
            template = file.read()
