...
class GSDBS:
    ...
    async def schemaCheck(self):
        ...
        iQuery = await self.schemaQuery(self.dtablename)
        if iQuery == None:
            return -4
    ...
    async def addDObject(self, dtablename, sribuildinfo, dataframe, superdtablename="DTABLE"):
        self.superdtablename = superdtablename
        ...
        rc = await self.schemaCheck()
        if rc != 0: return rc

        rc = self.generateStatement()
        if rc != 0: return rc

        rc = await self.executeStatement()
        return rc
    ...
    async def executeStatement(self, statement=None):
        if statement == None:
    ...
    async def schemaQuery(self, dtablename):
