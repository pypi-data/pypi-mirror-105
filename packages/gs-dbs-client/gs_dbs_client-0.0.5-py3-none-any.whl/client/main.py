...
class GSDBS:
    ...
    # reading data (trivial)
    # -----------------------
    df = pd.read_csv('D:\Welge\Welge_NextCloud\AI-Cloud\Daten\gun\guns.csv', sep=';')
    df = df.astype(
    ...
    # 1.1 addDTable
    # 1.1.1 creating a dtable
    # 1.1.2 inserting dObject(s)
    # 1.1.3 updating explicit dObjects
    rc = _gsdbs.addDObject("gun", ["streamkey", "fragmentid", "frameid", "objectid"], df)
    print(rc)
    exit(rc)

    # 1.2 updateDObject
    # updating Data Links of dObjects selected by a filter

    # 1.2 deleteDTable:
    # deleting a dTable

    # 1.3. deleteAllObjects
    # deleting all SRI of a Dtable

    # 1.4 deleteDObject
    # deleting SRI of a dTable with filter

    # 1.5 addForeignLink
    # add a Foreign Link to a dTable schema

    # 1.6 deleteForeignLink
    # removing a Foreign Link from a dTable schema

    # 2. queries
    # ----------

    # 2.1 query
    # selecting linked data from the GS-DBS with filter
