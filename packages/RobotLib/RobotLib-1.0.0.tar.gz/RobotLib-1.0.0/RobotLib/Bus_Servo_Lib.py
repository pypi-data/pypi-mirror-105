...
class BusServo(object):
    ...
    def __init__(self):
        ...
        # 初始化串口
        GPIO.setup(self.rx_pin, GPIO.OUT)  # 配置RX_CON为输入
        GPIO.output(self.rx_pin, 0)
        GPIO.setup(self.tx_pin, GPIO.OUT)  # 配置TX_CON为输出
...
if __name__ == "__main__":
    ...
    def getBusServoStatus(id):
        Pulse = object1.getBusServoPulse(id)  # 获取8号舵机的位置信息
        Temp = object1.getBusServoTemp(id)  # 获取8号舵机的温度信息
        Vin = object1.getBusServoVin(id)  # 获取8号舵机的电压信息
