...
class MeshGroup(Assembler):
    ...
    def assemble_linear(self, elem=None,verbose=False,**kwds)->Callable:
        ...
        vdisp = jax.vmap( 
                lambda U,dofs:  anp.take(U, anp.asarray(dofs, dtype='int32')-1)[:,None], (None,0)
        )
        ...
        #-----------------
        DOF_el_jac = {i: {j: [] for j in range(nr+nf)} for i in range(nr+nf)}
