...
def test_train_fails_with_no_arguments(monkeypatch):
    ...
    monkeypatch.setattr(grid.Grid, 'check_is_blocked', lambda x: True)
...
def test_train_fails_if_github_repo_not_accessible(monkeypatch):
    ...
    assert launch_url == os.getenv('GRID_URL', env.DEFAULT_GRID_URL).replace("/graphql",
                                                                             "") + '/#/settings?tabId=integrations'
