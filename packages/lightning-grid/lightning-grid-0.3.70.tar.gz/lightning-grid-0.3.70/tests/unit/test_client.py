...
from grid.client import CREDIT_CARD_ERROR_MESSAGE
...
class TestGridClient:
    ...
    def test_train_redirects_settings_on_credit_card_error_confirm_y(self, monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()

        env.IGNORE_WARNINGS = True

        def mp_train_script(*args, **kwargs):
            raise Exception(CREDIT_CARD_ERROR_MESSAGE)

        confirm_arg = None

        def mp_click_confirm(*args, **kwargs):
            nonlocal confirm_arg
            confirm_arg = args[0]
            return True

        launch_arg = None

        def mp_click_launch(*args, **kwargs):
            nonlocal launch_arg
            launch_arg = args[0]

        monkeypatch.setattr(resolvers, 'train_script', mp_train_script)
        monkeypatch.setattr(click, 'confirm', mp_click_confirm)
        monkeypatch.setattr(click, 'launch', mp_click_launch)

        G.train(**self.train_kwargs)

        assert "Would you like to validate one now?" in confirm_arg
        assert launch_arg == os.getenv('GRID_URL', env.DEFAULT_GRID_URL).replace("/graphql",
                                                                                 "") + "/#/settings?tabId=profile"

    def test_train_redirects_settings_on_credit_card_error_confirm_n(self, monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()
        ...
        assert CREDIT_CARD_ERROR_MESSAGE in ex_info.value.args[0]
        assert "Would you like to validate one now?" in confirm_arg
        assert launch_arg is None

    def test_train_doesnt_redirect_to_settings_on_any_error(self, monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()
        ...
        assert "Some generic error" in ex_info.value.args[0]
    ...
    def test_create_interactive_node(monkeypatch):
        ...
        result = G.create_interactive_node(name='test-interactive-id', config=config, description="test-description")
        assert result
        ...
        env.DEBUG = True
        with pytest.raises(click.ClickException):
            G.create_interactive_node(name='test-interactive-id', config=config, description='test-description')
    ...
    @staticmethod
    def test_create_interactive_node_redirects_settings_on_credit_card_error_confirm_y(monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()
        ...
        def mp_create_interactive_node(*args, **kwargs):
            return {"success": False, "message": CREDIT_CARD_ERROR_MESSAGE}
        ...
        assert not G.create_interactive_node(name='test-interactive-id', config=config, description="test-description")
    ...
    @staticmethod
    def test_create_interactive_node_redirects_settings_on_credit_card_error_confirm_n(monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()
    ...
    @staticmethod
    def test_create_interactive_node_doesnt_redirect_to_settings_on_any_error(monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()
        ...
        def mp_create_interactive_node(*args, **kwargs):
            return {"success": False, "message": "Some generic error"}
