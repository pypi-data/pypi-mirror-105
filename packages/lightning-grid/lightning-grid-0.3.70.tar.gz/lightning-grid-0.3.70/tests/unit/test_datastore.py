...
class TestDatastore:
    ...
    def test_datastore_recover(monkeypatch):
        with TemporaryDirectory() as tempdir:
            session = DatastoreUploadSession(name="test", source_dir="test_dir", credential_id="cc-abcdef")
    ...
    @staticmethod
    def test_datastore_upload_directory(monkeypatch):
        class MockClient:
            def execute(self, query, variable_values):
                if "createDatastore" in query:
                    assert variable_values == {'credentialId': 'cc-abcdef', 'name': 'test', 'source': 'data.tar.gz'}
                ...
                if "GetMultiPartPresignedUrls" in query:
                    assert variable_values == {'datastoreId': 'abcde', 'count': 50, 'path': 'data.tar.gz'}

                    return {'getMultiPartPresignedUrls': {'presignedUrls': [], 'uploadId': "upload1"}}
                ...
                if "completeMultipartDatastoreUpload" in query:
                    ...
                    return {'completeMultipartDatastoreUpload': {'success': True, 'message': ''}}
                ...
                if "completeDatastoreUpload" in query:
                    ...
                    return {'completeDatastoreUpload': {'success': True, 'message': ''}}

                raise ValueError(f"Unexpected query {query}")

        client = MockClient()
        with TemporaryDirectory() as tmpdirname:
            session = DatastoreUploadSession(name="test", source_dir=tmpdirname, credential_id="cc-abcdef")
            session.configure(client=client)

            def tar_path(**kwargs):
                return TarResults(before_size=1024 * 1000 * 1000, after_size=1024 * 1000 * 1000)
    ...
    def test_remote_datastore_url(self, monkeypatch):
        ...
        client = MockClient()
        remote_session = create_datastore_session(name="test-remote",
                                                  source="http://www.grid.ai/data.zip",
    ...
    @staticmethod
    def test_datastore_upload_compressed_file(monkeypatch):
        class MockClient:
            def execute(self, query, variable_values):
                if "createDatastore" in query:
                    assert variable_values == {'credentialId': 'cc-abcdef', 'name': 'test', 'source': 'data.zip'}
                ...
                if "GetMultiPartPresignedUrls" in query:
                    assert variable_values == {'datastoreId': 'abcde', 'count': 1, 'path': 'data.zip'}
                ...
                if "completeDatastoreUpload" in query:
                    assert variable_values == {'sizeMib': 0, 'datastoreId': 'abcde'}
        ...
        with NamedTemporaryFile(suffix=".zip") as tmp_file:
            ...
            session = DatastoreUploadSession(name="test", source_dir=tmp_file.name, credential_id="cc-abcdef")
            session.configure(client=client)
