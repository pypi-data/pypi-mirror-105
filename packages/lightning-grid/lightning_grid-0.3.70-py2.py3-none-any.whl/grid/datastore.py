...
def create_datastore(name: str, credential_id: str, client: Client, source: str = None):
    ...
    params = {'name': name, 'credentialId': credential_id, 'source': source}
...
def create_datastore_session(name: str, source: str, credential_id: str, compression: bool, client: Client):
    """Creates datastore session based on source"""
    check_source(source)

    if source.startswith("http"):
        return DatastoreRemoteUpload(name=name, source_url=source, credential_id=credential_id, client=client)
...
class DatastoreRemoteUpload:
    ...
    """
    def __init__(self, name: str, source_url: str, credential_id: str, client: Client):
        self.name = name
...
class DatastoreUploadSession(UploadProgressCallback):
    ...
    def recover(session_dir: str) -> "DatastoreUploadSession":
        ...
        with open(session_state_file, "r") as f:
            ...
            if datastore_version != DatastoreUploadSession.DATASTORE_VERSION:
                ...
                raise UnsupportedDatastoreImplementationError(f"Incomplete datastore session {session_name} is no " +
    ...
    def _create_datastore(self):
        ...
        datastore_id, datastore_version = create_datastore(name=self.name,
                                                           credential_id=self.credential_id,
                                                           client=self.client,
    ...
    def _upload_parts(self):
        ...
        uploader = self._create_uploader(presigned_urls=unuploaded_presigned_urls,
                                         already_uploaded_parts=already_uploaded_parts)
