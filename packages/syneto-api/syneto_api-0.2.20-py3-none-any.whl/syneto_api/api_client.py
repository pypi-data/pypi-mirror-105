...
class APIClientBase:
    ...
    def _request(
        ...
        **kwargs,
        ...
        url = make_url(self.url_base + "/" + relative_url, *args, query_args=query_args, **kwargs)
        ...
        try:
            ...
            subject = get_human_readable_entry_point(self)
            raise APIException(
                url=url,
                status_code=response.status_code,
                message=f"{subject} has failed ({response.status_code})",
                request_body=body,
                response=response,
            )
            ...
            raise APIException(
                url=url, message=f"{subject} has failed ({type(e)}: {hostname})", request_body=body,
            ) from e
