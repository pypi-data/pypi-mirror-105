"""Utilities for performing quadrature.


Sources

- @abramowitz2013handbook
- @casio2021guassian
...
from . import quad_simplex as simplex
...
class QuadratureScheme:
    def __init__(self, degree, points, weights):
        self.degree  = degree
        self.points  = points
        self.weights = weights


def _read_m228(filename):
...
def quad_points(n=None, deg=None, bounds=(-1.,1.), fam:str=None, closed=None, even=False, rule:str='mid', interpolator=None, error_term=False):
    """Generate weights and locations for a given quadrature scheme
    ...
    deg: int
        degree of highest order polynomial exactly integrated
    closed:
        True if endpoints are included
...
def newton_cotes(rn, equal=True, error_term=False):
    ...
    **This function is forked from the `scipy` library**
    ...
    where $\xi \in [x_0,x_N]$
    and $\Delta x = \frac{x_N-x_0}{N}$ is the average samples spacing.
    ...
    p1 = power+1
    fac = power*jnp.log(N) - scipy.special.gammaln(p1)
    fac = jnp.exp(fac)
    if error_term: return ai, BN*fac
    return ai

def gauss_quad(f:Callable, n: int, bounds:tuple=(-1,1), family='legendre'):
    """Integrate a function using Gaussian quadrature.
