...
def _keys_fn(self):
    return [key for key in self if key is not ...]

def _items_fn(self):
    return tuple(v for v in asdict(self).items() if v[0] is not ...)

def _values_fn(self):
    return tuple(v for v in asdict(self).values())

def _get_item_fn(self, name):
    if name is ...:
        return self._child
    else:
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen):
    ...
    _set_new_attribute(cls,'__getitem__',_get_item_fn)
