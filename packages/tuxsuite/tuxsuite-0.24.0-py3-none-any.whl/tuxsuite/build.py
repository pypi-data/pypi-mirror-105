...
class Plan(HeadersMixin):
    ...
    def watch(self):
        finished = {"builds": [], "tests": []}
        while True:
            plan = self.get_plan()
            for build in self.builds:
                if build.uid not in finished["builds"]:
                    ...
                    if state is not None:
                        yield state
                        if state.final:
                            finished["builds"].append(build.uid)
            for test in self.tests:
                if test.uid not in finished["tests"]:
                    ...
                    if state is not None:
                        yield state
                        if state.final:
                            finished["tests"].append(test.uid)
            if len(finished["builds"]) == len(self.builds) and len(
                finished["tests"]
            ) == len(self.tests):
                break
            time.sleep(5)
