...
def common_options(required):
    ...
    options = [
        ...
        option(
            "--toolchain",
            help="Toolchain [gcc-8|gcc-9|gcc-10|clang-10|clang-11|clang-12|clang-nightly|clang-android]",
        ),
        option(
            "--build-name",
...
def test(device, kernel, modules, tests, wait_for, no_wait):
...
def plan(
    ...
    **build_params,
    ...
    try:
        plan_config = tuxsuite.config.PlanConfig(name, description, config)
    ...
    try:
        plan.submit()
        info("Plan {}/plans/{}".format(plan.url, plan.plan))
    except tuxsuite.exceptions.BadRequest as e:
        raise (click.ClickException(str(e)))
