...
<Tooltip>
    ...
    has_tour: app.help_layout and help_id_tour_class(app.displayed_help_id) or False
    ...
    Label:
        ...
        markup: True
        padding: sp(12) if root.tap_thru else sp(6), sp(9)
        size_hint: None, None
...
<TourOverlay>
    ...
    BoxLayout:
        ...
        size_hint_y: None
        height: max(title_lbl.height, page_lbl.height) if app.landscape else title_lbl.height + page_lbl.height
        pos_hint: dict(x=0, y=root.buttons_pos_hint_y)
        canvas:
            Color:
                #rgba: Window.clearcolor[:3] + (0.48 if app.main_app.debug else 0.96, )
        ...
        Label:
            ...
            text: root.page_text
    Button:
        disabled: root.buttons_disabled or not root.prev_text
        opacity: 1.0 if root.prev_text else 0.0
        size_hint: None, None
        size: self.texture_size[0] + sp(18), root.label_height
        pos_hint: dict(x=0.06, top=root.buttons_pos_hint_y)
        font_size: root.font_height
        text: root.prev_text
        on_release: root.prev_page()
    StopTourButton:
        size_hint: None, None
        size: 2 * (root.label_height * (1.0 if root.prev_text or root.next_text else 1.5), )
        pos_hint: dict(center_x=0.501, top=root.buttons_pos_hint_y)
        source: app.main_app.img_file(id_of_flow('close', 'popup'))
        on_release: root.stop_tour()
    Button:
        disabled: root.buttons_disabled or not root.next_text
        opacity: 1.0 if root.next_text else 0.0
        size_hint: None, None
        size: self.texture_size[0] + sp(18), root.label_height
        pos_hint: dict(right=0.939, top=root.buttons_pos_hint_y)
        font_size: root.font_height
        text: root.next_text
...
class Tooltip(ScrollView):                                                           # pragma: no cover
    ...
    def on_touch_down(self, touch: MotionEvent) -> bool:
        ...
        if self.collide_tap_thru_toggler(*touch.pos):
            ...
            ret = App.get_running_app().main_app.starting_tour()
...
class TourOverlay(ModalBehavior, FloatLayout):
    """ tour overlay singleton view class. """
    buttons_disabled = BooleanProperty()        #: read-only flag disable back/next buttons in tour page setup
    tour_instance = ObjectProperty()            #: read-only TourBase instance, init. via instantiation/`start_tour`
    ...
    def page_updated(self):
        """ callback from :meth:`~TourBase.setup_layout` after update of tour layout/overlay for UI-specific setup. """
        ...
        but_pos = pos1 / 2.19 if pos1 > win_height - pos2 else (pos2 + win_height) / 1.89
        Animation(buttons_pos_hint_y=but_pos / win_height).start(self)
    ...
    def start_tour(self, tour_cls: Optional[Type[TourBase]] = None) -> bool:
        ...
        if tour_cls is None:
            tour_cls = help_id_tour_class(main_app.displayed_help_id) or OnboardingTour
