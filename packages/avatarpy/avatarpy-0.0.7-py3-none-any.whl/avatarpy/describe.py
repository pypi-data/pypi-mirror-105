...
class Describe:
    ...
    def __call__(self, indices=None, include=['corr', 'stat'], assign_ID=True):
        df = pd.concat([getattr(self, x)(indices=indices, assign_ID=assign_ID) for x in include]).reset_index(drop=True)
        return df
    ...
    def corr(self, indices=None, features=['velocity', 'acceleration', 'angle', 'angle_velocity', 'angle_acceleration', 'stretch_index'], 
        ...
        for feature in features:
            data =   self.__parent[feature].loc[indices] if indices else self.__parent[feature]
            corrs.extend([
        ...
        if assign_ID:
            return df.assign(ID=self.__parent.ID)
        return df
