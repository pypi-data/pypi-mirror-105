...
def init_sqlalchemy(app):
    ...
    from webcore.sql_encoder import SqlEncoder
    from sqlalchemy.exc import SQLAlchemyError
    db = SQLAlchemy(app)
    app.db = db
    app.register_encoder(SqlEncoder())
    app.register_error_handler(SQLAlchemyError, sql_error_handler)
...
def init_celery(app):
    ...
    celery = Celery(
        ...
        broker = app.config['CELERY']['celery_broker_url']
    )
