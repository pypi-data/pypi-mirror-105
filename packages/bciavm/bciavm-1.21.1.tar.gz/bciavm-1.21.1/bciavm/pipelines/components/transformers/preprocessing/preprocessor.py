...
from bciavm.pipelines.components.transformers.encoders import (
    OneHotEncoder
)
...
from bciavm.utils.gen_utils import calc_nearest_area_aggregate
import io
...
class PreprocessTransformer(Transformer) :
    ...
    def __init__(self, random_seed=0, target='Price_p', DATA_DICT = {
        ...
        for ct in range(1, 4):
            #d = io.BytesIO(pkgutil.get_data('bci_avm.data.lookup_table_parquet', 'lookup_table' + str(ct) + '.parquet'))
            d = 'https://bciavm.s3.amazonaws.com/dfPricesEpc_parquet/lookup_table'+str(ct)+'.parquet'
    ...
    def merge_tables(self, request_data):
        ...
        if 'POSTCODE' not in lookup_table.columns:
            ...
            for ct in range(1, 4):
                d = io.BytesIO(
                    pkgutil.get_data('bci_avm.data.lookup_table_parquet', 'lookup_table' + str(ct) + '.parquet'))
                self.lookup_table = self.lookup_table.append(pd.read_parquet(d))
            lookup_table = self.lookup_table
