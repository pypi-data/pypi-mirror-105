...
from unittest_reinvent.scoring_tests.fixtures import create_c_lab_component, create_predictive_property_component_regression
...
class Test_pip_with_parallel_additive(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        solph74.weight = 4
        solph74.specific_parameters[csp_enum.CLAB_INPUT_FILE] = f"{MAIN_TEST_PATH}/solph74.json"
        ...
        hlm_clint.weight = 4
        hlm_clint.specific_parameters[csp_enum.CLAB_INPUT_FILE] = f"{MAIN_TEST_PATH}/hlm_clint.json"
        ...
        azlogd74.weight = 4
        azlogd74.specific_parameters[csp_enum.CLAB_INPUT_FILE] = f"{MAIN_TEST_PATH}/azlogd74.json"
    ...
    def test_parallel_clab_1(self):
        smiles = ["O=S(=O)(c3ccc(n1nc(cc1c2ccc(cc2)C)C(F)(F)F)cc3)N" for i in range(3)]
        score = self.sf_instance.get_final_score(smiles=smiles)
        self.assertAlmostEqual(score.total_score[0], 0.330, 2)
