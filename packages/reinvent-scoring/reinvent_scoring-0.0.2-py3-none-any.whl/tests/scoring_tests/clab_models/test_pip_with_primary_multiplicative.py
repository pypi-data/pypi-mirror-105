...
from unittest_reinvent.scoring_tests import create_predictive_property_component_regression
...
class Test_pip_with_Product(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        solDD.specific_parameters[csp_enum.LOW] = 0.7
        solDD.specific_parameters[csp_enum.HIGH] = 2.3
        solDD.specific_parameters[csp_enum.TRANSFORMATION] = True
        solDD.weight = 4
        solDD.specific_parameters[csp_enum.CLAB_INPUT_FILE] = f"{MAIN_TEST_PATH}/solDD.json"
    ...
    def test_clab_with_primary_multiplicative_1(self):
        smiles = ["Cn1cnc2c(-c3cnc(Nc4ccc(C5CC5)cc4)c(C(N)=O)n3)c(-c3cn[nH]c3)ccc21" for i in range(3)]
