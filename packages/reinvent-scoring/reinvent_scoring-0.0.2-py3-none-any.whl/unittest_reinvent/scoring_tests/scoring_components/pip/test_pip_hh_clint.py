...
from reinvent_scoring.scoring.score_components import HHClintPiP
...
class Test_pip_hh_clint(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.expected_scores = [1.300, 1.346, 1.513]

        cls.component = HHClintPiP(parameters)
...
class Test_pip_hh_clint_transformation(unittest.TestCase):
    def setUp(cls):
        ...
        parameters.specific_parameters[csp_enum.LOW] = 1.4
        if not os.path.isdir(MAIN_TEST_PATH):
            os.makedirs(MAIN_TEST_PATH)

        cls.query_smiles = [celecoxib, 'CCC', "CCCCC"]
        cls.expected_raw_scores = [1.300, 1.346, 1.513]
