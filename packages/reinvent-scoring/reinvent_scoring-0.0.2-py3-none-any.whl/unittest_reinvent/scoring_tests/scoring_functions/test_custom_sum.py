...
class Test_custom_sum(unittest.TestCase):
    ...
    def test_special_selectivity_multiplicative_no_sigm_trans_1(self):
        score = self.sf_instance.get_final_score(smiles=["CC(O)(CO)c1ccc2nc(NC3CCN(S(C)(=O)=O)CC3)cc(C)c2c1"])
        self.assertAlmostEqual(score.total_score[0], 0.3158, 3)

    def test_special_selectivity_multiplicative_no_sigm_trans_2(self):
        score = self.sf_instance.get_final_score(smiles=["COc1cccc2cc(C(=O)NCCCCN3CCN(c4cccc5nccnc54)CC3)oc21"])
        self.assertAlmostEqual(score.total_score[0], 0.0888, 3)
    ...
    def test_special_selectivity_multiplicative_no_sigm_trans_4(self):
        ...
        self.assertAlmostEqual(score.total_score[0], 0.0735, 3)
