...
class ParserRunner(LogCollector):
    ...
    def __init__(self, manager_id: str, parser_type: EMailParserType):
        ...
        parsers_info: Optional[pd.DataFrame] = BasicDataApi().get_email_parser_info(manager_id)
        assert parsers_info is not None and not parsers_info.empty, f'get email parsers info of {manager_id} failed'
        print(parsers_info)

        # 只使用 valid parser
        parsers_info = parsers_info[parsers_info.is_valid]
        parsers_info = parsers_info[parsers_info.p_type == parser_type].p_id.to_list()

        parsers_code: Optional[pd.DataFrame] = BasicDataApi().get_email_parser_code(manager_id, parsers_info)
        assert parsers_code is not None and not parsers_code.empty, f'get email parsers code of {manager_id} failed'

        self._parsers: Dict[str, str] = parsers_code.set_index('p_id')['p_code'].to_dict()
