...
class BioLib:
    ...
    def run_local_docker_app(input_files, args, module, source_files_temp_dir):
        error_queue = Queue()
        biolib_docker_client = BiolibDockerClient(is_running_in_enclave=False, messages_to_send_queue=error_queue)
        biolib_docker_client.create_compute_container(
            image=module["image"],
        ...
        if input_files:
            biolib_docker_client.map_and_copy_input_files_to_container(input_files)

        if source_files_temp_dir:
            runtime_zip_data = open(f'{source_files_temp_dir.name}/biolib_run_dev.zip', 'rb').read()
            biolib_docker_client.map_and_copy_runtime_files_to_container(runtime_zip_data, remove_root_folder=False)
