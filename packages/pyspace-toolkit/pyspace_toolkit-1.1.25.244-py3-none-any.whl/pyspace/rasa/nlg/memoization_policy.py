...
class MemoizationPolicy(Policy):
    ...
    def __init__(self,policy_probability = 1.0,
        ...
        for ci in self.config:
            self.config_x.append( [ci[1], self._standard_featurizer(c[2])] )
    ...
    def predict_action_probabilities(
        ...
        for ci in self.config_x:
            ...
            logger.debug(f"Current tracker state {tracker_as_states}")
            logger.debug(f"Current tracker state {states}")
            ...
            if ci[0] == 'default':
                ...
                tracker_current = t.current_state()
                entities = tracker_current['latest_message']['entities']
                entities = set([e['entity'] for e in entities])

                tracker_state_entities = set([tas_k[7:] for tas in states for tas_k in tas if tas is not None and tas_k[:7] == 'entity_'])
