...
import os
import json
from t2wml.utils.debug_logging import basic_debug, t2wml_log
...
class Annotation():
    ...
    def _preprocess_annotation(self, annotations):
        ...
        t2wml_log.debug("first loop over blocks for preprocessing")
        ...
        t2wml_log.debug("finished first loop. second loop over blocks for preprocessing")
        for block in annotations:
        ...
        t2wml_log.debug("finished creating links")
...
class AnnotationNodeGenerator:
    ...
    def preload(self, sheet, wikifier):
        ...
        dataframe_rows=[]
        item_entities=set()
        t2wml_log.debug("ANG preload-- part one wikification")
        ...
        t2wml_log.debug("ANG preload-- create dataframe from wikfication")
        if dataframe_rows:
        ...
        t2wml_log.debug("ANG preload-- part 2 entities")
        ...
        t2wml_log.debug("ANG preload-- part 2 entities-- update cache")
        prov.update_cache(custom_nodes)
        
        if custom_nodes:
            t2wml_log.debug("ANG preload-- part 2 entities-- save stuff")
