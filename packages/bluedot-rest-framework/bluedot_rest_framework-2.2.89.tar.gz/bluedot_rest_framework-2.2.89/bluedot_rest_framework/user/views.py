...
from bluedot_rest_framework.utils.jwt_token import jwt_get_wechatid_handler, jwt_get_userinfo_handler
...
class UserView(CustomModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='check-user', url_name='check-user',)
    def check_user(self, request, *args, **kwargs):
        wechat_id = jwt_get_wechatid_handler(request.auth)
        queryset = self.model_class.objects.filter(wechat_id=wechat_id).first()
        if queryset:
