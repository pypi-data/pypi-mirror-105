...
class StateSpace:
    def __init__(self, m, size):
        ...
        self.buckets = None
        self.bin_limits = np.array([[[(0,1)]for j in range(self.states//2)]for i in range(self.m-1)]).reshape((self.m-1)*self.states//2,2)
        self.num_bins = tuple(np.array([[32] for i in range((self.m-1)*self.states//2)]).flatten())
        self.is_discrete = False
