...
def check_and_getsubNum(s, default_pre='Z', num_bit=5, drop_char=['.']):
    ...
    if not str.isalpha(s[0]):
        pre.join(default_pre)
    else:
        while str.isalpha(s[i]):
            pre.join(s[i])
            i += 1
...
def check_and_getmainNum_multi(s, default_pre='Z', num_bit=5, drop_char=['.']) -> (bool, List):
    ...
    if not isinstance(s, str):
        ...
        mainNumbers = []
        for a in arr:
            flag, num = check_and_getmainNum(a, default_pre, num_bit)
            check |= flag
            if flag:
...
def pathologyNum_mapto_sickNum(file_names1=[], file_names2=[], default_pre='Z', num_bit=5, col_name1=['病理号'],
                               sheet_names1=None, col_name2=['患者代码', '病理编号'], sheet_names2=None,
                               savename='pathology_mapto_sickno.csv', show_file_name=False):
    '''
    ...
    for i in tqdm(range(len(input1))):
        ...
        for j, line2 in enumerate(input2):
            s2 = line2[1]
            s2_file = names2[j]
            if match(s1, s2, default_pre, num_bit):
    ...
    for i, out in tqdm(range(len(output))):
        out_data.loc[i] = out[i]

    # 保存文件
    savename = 'pNum_mapto_sickNum_' + str(datetime.date.today()) + '.xlsx'
    ...
    for pathology in tqdm(list_pathology):
        ...
        for sub_dir in listdir_source:
            ...
            if check_and_getsubNum(pathology)==check_and_getsubNum(sub_dir):
                ...
                count += 1
    print('总数：',len(list_pathology),'成功拷贝：',count,'空病理号：',count_null,'其它：',len(list_pathology) - count -count_null)