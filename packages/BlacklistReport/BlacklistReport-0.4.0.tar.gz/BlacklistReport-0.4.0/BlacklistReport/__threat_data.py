...
class ThreatData:
    ...
    def fetch_GREYNOISE_summary(ip: str) -> (Optional[str], Optional[str], Optional[list]):
        classification, organization, tags = None, None, None
        ...
        if gn_ip_data.get('metadata', {}).get('organization'):
            organization = gn_ip_data['metadata']['organization']
        if gn_ip_data.get('classification') and gn_ip_data['classification'] != 'unknown':
            classification = gn_ip_data['classification'].upper()
        if gn_ip_data.get('tags'):
            tags = gn_ip_data['tags']
