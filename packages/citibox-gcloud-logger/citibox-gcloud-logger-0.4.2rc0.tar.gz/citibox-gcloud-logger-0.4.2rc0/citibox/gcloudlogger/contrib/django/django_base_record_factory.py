...
from citibox.gcloudlogger.src.base_record import BaseRecord, Request, Params, Body, RequestHeaders, \
...
class DjangoBaseRecordFactory(BaseLogRecordFactory):
    DJANGO_HEADER_PREFIX = "HTTP_"
    PUBSUB_USER_AGENT = "CloudPubSub-Google"

    def __init__(self, logger, request: HttpRequest, response: HttpResponse, view_kwargs: dict):
        super().__init__()
        self.logger = logger
    ...
    def _build_response_headers(self) -> ResponseHeaders:
        return ResponseHeaders(**self.response.items())
    ...
    def _build_response_body(self) -> Body:
        ...
        try:
            ...
            body = Body(
                **{"error_msg": f'Error serializing response: {e.msg}'}
            )
