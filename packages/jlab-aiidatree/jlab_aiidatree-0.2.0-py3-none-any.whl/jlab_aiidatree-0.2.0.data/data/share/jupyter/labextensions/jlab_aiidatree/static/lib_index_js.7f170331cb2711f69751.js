...
//# sourceMappingURL=d3graph_widget.js.map
...
//     factory: 'Editor'
// });
//# sourceMappingURL=json_widget.js.map
...
/* harmony export */   "queryOutgoing": () => (/* binding */ queryOutgoing),
/* harmony export */   "queryIncoming": () => (/* binding */ queryIncoming)
/* harmony export */ });
...
async function requestAPI(endPoint = '', init = {}) {
    ...
    const requestUrl = _jupyterlab_coreutils__WEBPACK_IMPORTED_MODULE_0__.URLExt.join(settings.baseUrl, 'jlab_aiidatree', // API Namespace
    endPoint);
    let response;
    try {
...
async function queryNode(pk, url = null) {
    ...
    let reply;
    try {
        reply = await requestAPI('querybuilder', {
            body: JSON.stringify(dataToSend),
            method: 'POST'
        });
    }
    catch (reason) {
        console.warn(`Error on POST /jlab_aiidatree/querybuilder ${dataToSend}.\n${reason}`);
...
async function queryProcesses(limit = 9999, orderBy = 'id', url = null) {
...
async function queryOutgoing(pk, url = null) {
...
async function queryIncoming(pk, url = null) {
    // Query all outgoing nodes for a certain node
    const dataToSend = {
        path: [
            { entity_type: '', tag: 'root' },
...
/* harmony import */ var three__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! three */ "webpack/sharing/consume/default/three/three");
...
class ThreeJSWidget extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_0__.Widget {
    constructor(id, label) {
        ...
        this.camera = new three__WEBPACK_IMPORTED_MODULE_2__.PerspectiveCamera(40, this.node.clientWidth / this.node.clientHeight, 1, 1000);
    ...
    initialise() {
        this.scene.background = new three__WEBPACK_IMPORTED_MODULE_2__.Color(0x999999);
        this.scene.add(new three__WEBPACK_IMPORTED_MODULE_2__.AmbientLight(0xffffff));
        this.camera.add(new three__WEBPACK_IMPORTED_MODULE_2__.PointLight(0xffffff, 0.8));
        this.camera.up.set(0, 0, 1);
        this.scene.add(this.camera);
        const grid = new three__WEBPACK_IMPORTED_MODULE_2__.GridHelper(200, 20, 0xffffff, 0x555555);
    ...
    create_atom(position, radius = 1, color = 'blue', opacity = 0.9) {
        const geometry = new three__WEBPACK_IMPORTED_MODULE_2__.SphereBufferGeometry(radius, 30, 30);
    ...
    renderStructureData(data) {
        ...
        const meshes = sites.reduce((a, site) => {
            a.push(this.create_atom({
                ...
                y: site['position'][1],
                z: site['position'][2]
            }, (0,lodash__WEBPACK_IMPORTED_MODULE_1__.get)(atomData, site['kind_name'], [1])[0], new three__WEBPACK_IMPORTED_MODULE_2__.Color(...(0,lodash__WEBPACK_IMPORTED_MODULE_1__.get)(atomData, site['kind_name'], [1, 0, 1, 0]).slice(1, 4))));
            return a;
        }, []);
        this.fitCameraToMeshes(meshes, this.camera, this.controls);
        this.scene.add(...meshes);
    ...
    fitCameraToMeshes(meshes, camera, controls) {
        ...
        const boundingBox = new three__WEBPACK_IMPORTED_MODULE_2__.Box3();
        // get bounding box of object - this will be used to setup controls and camera
        boundingBox.setFromPoints(points);
        const center = new three__WEBPACK_IMPORTED_MODULE_2__.Vector3();
        boundingBox.getCenter(center);
        const size = new three__WEBPACK_IMPORTED_MODULE_2__.Vector3();
...
//# sourceMappingURL=threejs_widget.js.map
...
class AiidaTreeWidget extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_0__.Widget {
    ...
    refreshSettings(loadedSettings) {
        this.dbSettings = {
            restUrl: loadedSettings.get('rest_url').composite
        };
    }
    async refresh() {
    ...
    async buildProcessesTable() {
        let processes = await (0,_rest__WEBPACK_IMPORTED_MODULE_5__.queryProcesses)(100, 'id', this.dbSettings.restUrl);
        ...
        for (const process of processes) {
            ...
            if (this.selectProcessState.value === 'all' ||
                ...
                const tr = this.buildTableRow([
                    process.id,
                    process.processLabel === null ? '-' : process.processLabel,
...
function createCoreCommands(app, widget) {
    ...
    app.commands.addCommand(_consts__WEBPACK_IMPORTED_MODULE_6__.CommandIDs.toggle + ':' + widget.id, {
        execute: async (args) => {
            ...
            if (row_element.nextElementSibling &&
                ...
                let next_element;
                const next_elements = [];
                const linksIncoming = await (0,_rest__WEBPACK_IMPORTED_MODULE_5__.queryIncoming)(pk, widget.dbSettings.restUrl);
                for (const linkIncoming of linksIncoming) {
                    next_element = createProcessLinkRow(widget, pk, linkIncoming);
                    next_elements.push(next_element);
                }
                const linksOutgoing = await (0,_rest__WEBPACK_IMPORTED_MODULE_5__.queryOutgoing)(pk, widget.dbSettings.restUrl);
                for (const linkOutgoing of linksOutgoing) {
                    next_element = createProcessLinkRow(widget, pk, linkOutgoing);
                    next_elements.push(next_element);
                }
    ...
    app.commands.addCommand(_consts__WEBPACK_IMPORTED_MODULE_6__.CommandIDs.inspectNode + ':' + widget.id, {
        execute: async (args) => {
            const pk = typeof args['pk'] === 'undefined'
                ? widget.selected_pk
                : args['pk'];
            const data = await (0,_rest__WEBPACK_IMPORTED_MODULE_5__.queryNode)(pk, widget.dbSettings.restUrl);
    ...
    app.commands.addCommand(_consts__WEBPACK_IMPORTED_MODULE_6__.CommandIDs.launchD3 + ':' + widget.id, {
        execute: async (args) => {
            ...
            const linksIn = await (0,_rest__WEBPACK_IMPORTED_MODULE_5__.queryIncoming)(pk, widget.dbSettings.restUrl);
            ...
            const linksOut = await (0,_rest__WEBPACK_IMPORTED_MODULE_5__.queryOutgoing)(pk, widget.dbSettings.restUrl);
...
function createProcessLinkRow(widget, pk, link) {
    // Create row for process table
    // Take the last part of the entry point
    const typeElements = link.nodeType.split('.');
    const name = typeElements[typeElements.length - 2];
    const icon = getIcon(link.nodeType);
    const arrow = link.linkDirection === 'incoming' ? '→' : '←';
    const next_element = widget.buildTableRow([link.nodeId, name, arrow], `${pk}/${link.nodeId}`, icon);
    next_element.className += ` aiidatree-link-item aiidatree-link-${link.linkDirection} aiidatree-item-${name}`;
...
//# sourceMappingURL=tree_widget.js.map
...
//# sourceMappingURL=lib_index_js.7f170331cb2711f69751.js.map