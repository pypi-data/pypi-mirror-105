...
def run_transform_string_inline(client, bucketname, prefix_in, prefix_trunc,
    ...
    if (look_for_transform(transform_string, TRANSFORM_RAW_PD)):
        ...
        logging.info("Running transform " + str(TRANSFORM_RAW_PD))
        ...
        logging.info("Running transform " + str(TRANSFORM_RAW_DS))
        # TODO: is xform_local_files_zip needed for this?
        ...
        logging.info("Running transform " + str(TRANSFORM_CSV_PD))
        # TODO: is xform_local_files_zip needed for this?
        return actually_run_transformation(client, True,\
        ...
        logging.info("Running transform " + str(TRANSFORM_CSV_DS))
        # TODO: is xform_local_files_zip needed for this?
        return actually_run_transformation(client, False,\
                bucketname, prefix_in, prefix_trunc, True, transform_string, zf,\
                **kwargs)
        ...
        logging.info("Running transform " + str(TRANSFORM_ONE_OBJ))
        ...
        if (client == None):
            raise ValueError('Error. Transform type ' + str(TRANSFORM_DIR_BY_DIR) + ' needs input_data')
        logging.info("Running transform " + str(TRANSFORM_DIR_BY_DIR))
        return read_and_xform_dir_by_dir(client, bucketname, prefix_in, prefix_trunc,
                transform_string, zf, **kwargs)
    elif (look_for_transform(transform_string, TRANSFORM_ALL_OBJECTS)):
        if (client == None):
            raise ValueError('Error. Transform type ' + str(TRANSFORM_ALL_OBJECTS) + ' needs input_data')
        logging.info("Running transform " + str(TRANSFORM_ALL_OBJECTS))
...
def run_transform(run_options:dict, **kwargs) -> str:
    ...
    if ('disable_cache' in run_options and
        ...
        logging.info('Not checking cache since disable_cache is set');
    else:
        found, run_id = check_in_cache(input_data_spec, xformname)
        if (found == True):
            logging.info("run_transform(): Transform not run again since it ran earlier: Found in cache. run_id=" + run_id)
            return run_id
        else:
            logging.info("Did not find data in cache. Running transform")
...
def run_transforms(transforms):
    try:
        ...
        logging.error("run_transforms(): error: caught exception: %s \ntransforms = %s", traceback.format_exc(), transforms)
