...
class App(StateMixin, LoggingMixin, RollbarMixin):
    ...
    def load(self, event_type: EventType, event: Event):
        ...
        self.event_records = self.filter_event_for_collections(self.event_type, valid_stream_collections, event)

        self.log_event(self.event_records, max_lookback_seconds)
    ...
    def run_modules(self):
        ...
        for module_type in self.get_active_modules():
            ...
            try:
                module.run(self.event_records)
            except Exception:
                message = f"Error in module {module_type.module_key}"
                self.track_error(message=message)
                raise

        last_processed_timestamp = self.event_records[-1].get('timestamp')
