...
def mkNCCommonUni_multiISO(output, dateTimeStart, dateTimeArr, isoArr, latArr, lonArr, dataClass4D=[], dataClass3D=[],
    ...
    try:
        if not dateTimeArr is None and not dateTimeStart is None:
            dataset.createDimension("time", len(dateTimeArr))
        if not isoArr is None:
            for idx, _ in enumerate(isoArr):
                dataset.createDimension(f"isobaric{idx+1}", len(isoArr[idx]))
        ...
        if not isoArr is None:
            for idx,_ in enumerate(isoArr):
                dataset.createVariable(f"isobaric{idx+1}", np.float32, (f"isobaric{idx+1}"), zlib=True)
        ...
        if not isoArr is None:
            for idx, _ in enumerate(isoArr):
                dataset.variables[f"isobaric{idx+1}"][:] = isoArr[idx]
                dataset.variables[f"isobaric{idx+1}"].units = "hPa"
                dataset.variables[f"isobaric{idx+1}"].positive = "up"
