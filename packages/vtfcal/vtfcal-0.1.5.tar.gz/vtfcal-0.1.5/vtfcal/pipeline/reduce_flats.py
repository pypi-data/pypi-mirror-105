...
def reduce_flats(data_tree, correction=True, fourier=True):
    ...
    for modstate in ["modstate0", "modstate1", "modstate2", "modstate3"]:
        ...
        plotprofile(
            asdf_file,
            [
                (f"reduced averaged flats {modstate}", "Raw flats"),
        ...
        if not correction:
            ...
            norm_frames = arr / np.nanmean(arr, axis=(1, 2))[:, np.newaxis, np.newaxis]
            norm_fnames = []
        ...
        asdf_file, daskarr, mean_profile = calculate_wl_shift(
        ...
        norm_frames = (
            shifted_frames
            / da.nanmean(shifted_frames, axis=(1, 2))[:, np.newaxis, np.newaxis]
        )
        norm_fnames = []
        for wl, frame in enumerate(norm_frames):
            fname = Path(outdir) / modstate / f"normalised_flat_l{wl:02}a0.FITS"
        ...
        plotprofile(
            asdf_file,
            [
                (f"corrected wl-shifted flats {modstate}", r"$\lambda$-shifted flats"),
                (mean_profile.reshape(len(norm_fnames)), "Average profile"),
                (f"corrected normalised flats {modstate}", "Normalised flats"),
            ],
