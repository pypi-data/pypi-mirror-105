...
class LebaiRobot:
    ...
    def move_pvats(self, pvat_iter):
        self.rcs.MovePVATStream((rc.PVATRequest(duration=s['t'], q=s['p'], v=s['v'], acc=s['a']) for s in pvat_iter))
    ...
    def set_fan(self, on):
        self._sync()
        self.rcs.SetFan(rc.FanStatus(fan=on))
    ...
    def add_signal(self, pin, delta):
        self._sync()
        self.rcs.AddSignal(rc.SignalValue(index=pin, value=delta))
