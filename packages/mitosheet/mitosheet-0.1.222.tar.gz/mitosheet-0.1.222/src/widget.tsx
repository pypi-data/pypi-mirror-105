...
// Components
import Mito, { ColumnSpreadsheetCodeJSON, SheetColumnFiltersArray, ColumnTypeJSONArray, DataTypeInMito } from './components/Mito';
...
export interface TourJSON {
    typeOfDataInTool: DataTypeInMito;
...
import { FeedbackOpenLocation } from './components/taskpanes/Feedback/FeedbackTaskpane';
...
export class ExampleView extends DOMWidgetView {
  ...
  */
    ...
    render(): void {  
        ...
        ReactDOM.render(
            <Mito 
                ...
                tourJSON={this.getTourJSON()}
                shouldUpgradeMitosheet={this.getShouldUpgradeMitosheet()}
                /* 
                    If we should open the feedback taskpane, then we set it open from
                    the start, so the user is prompted to give us feedback immediately
                */
                currOpenTaskpane={this.getShouldDisplayFeedback() 
                    ? {type: TaskpaneType.FEEDBACK, openLocation: FeedbackOpenLocation.AUTO_POPUP} 
    ...
    */
    ...
    getShouldDisplayFeedback(): boolean {
        return this.model.get('should_display_feedback');
    }
    ...
    getTourJSON(): TourJSON {
        const tourJSON = {
            typeOfDataInTool: DataTypeInMito.NONE,
            receivedTours: [],
            intendedBehavior: []
        }

        const typeOfDataInTool = this.model.get('data_type_in_mito');
        if (typeOfDataInTool === 'DataTypeInMito.NONE') {
            tourJSON['typeOfDataInTool'] = DataTypeInMito.NONE
        } else if (typeOfDataInTool === 'DataTypeInMito.PROVIDED') {
            tourJSON['typeOfDataInTool'] = DataTypeInMito.PROVIDED
        } else if (typeOfDataInTool === 'DataTypeInMito.TUTORIAL') {
            tourJSON['typeOfDataInTool'] = DataTypeInMito.TUTORIAL
        } else if (typeOfDataInTool === 'DataTypeInMito.PERSONAL') {
            tourJSON['typeOfDataInTool'] = DataTypeInMito.PERSONAL
        } else {
            tourJSON['typeOfDataInTool'] = DataTypeInMito.NONE
        }

        tourJSON['receivedTours'] = JSON.parse(this.model.get('received_tours'));
        tourJSON['intendedBehavior'] = JSON.parse(this.model.get('intended_behavior'));

        if (tourJSON['receivedTours'] === null) {
            tourJSON['receivedTours'] = []
        } 

        if (tourJSON['intendedBehavior'] === null) {
            tourJSON['intendedBehavior'] = []
        }
