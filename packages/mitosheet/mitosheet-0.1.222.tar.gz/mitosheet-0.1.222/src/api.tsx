...
export class MitoAPI {
    ...
    */
    ...
    /*
        If the user closes the feedback without giving any feedback, 
        we call this, as we don't want to keep opening the feedback
        and annoying the user :) 
    */
    async setClosedFeedback(): Promise<void> {
        this.send({
            'event': 'update_event',
            'type': 'set_user_field_update',
    ...
    */
    async sendCloseTour(tourNames: string[]): Promise<void> {
        this.send({
            ...
            'field': 'received_tours',
            'value': tourNames
        })
    }

    /* 
        Lets the backend know that new feedback was received. Note: the way
        we store the feedback in the backend is different than the way that
        we actually add params that get logged in this event. 

        This is because we cannot easily get the answers if they are in nested
        objects, so we promote them to the highest level, so that we can easily
        make reports on them.
    */
    async sendFeedback(feedback: Record<string, unknown>): Promise<void> {
        ...
        // We add the questions and answers to the highest level of the message
        // itself; see comment in send feedback
        let questionNum = 0;
        Object.keys(feedback).forEach(question => {
            message[`question_${questionNum}`] = question;
            message[`answer_${questionNum}`] = feedback[question];
            questionNum += 1;
        })
