...
test('Creates a filter group and then saves it properly when the filter modal is closed', async t => {
    await tryTest(
        t,
        code,
        async t => {
...
test('simple filter allows negative number or decimal', async t => {
    await tryTest(
        t,
