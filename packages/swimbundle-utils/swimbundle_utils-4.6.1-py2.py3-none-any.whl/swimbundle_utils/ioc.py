...
import urllib.parse
from ioc_finder import parse_domain_names
from ioc_finder import parse_email_addresses
...
class IOCParser(object):
    ...
    def parse_iocs(self, text, defang=False, whitelist_regex=''):
        ...
        am = "attack_mitigations"
        at = ['attack_techniques', 'attack_tactics']
        nestedKeys = ["enterprise", "mobile", "pre_attack"]
        for key in _IOC_TYPES:
            if key == am:
                _find_iocs[key] = {n: [] for n in nestedKeys[:-1]}
            elif key in at:
                _find_iocs[key] = {n: [] for n in nestedKeys}
            else:
                _find_iocs[key] = []
        ...
        for key in _IOC_TYPES:
            ...
            elif key == am:    # used later on in the class, dictionary with nested lists.
                for nk in nestedKeys[:-1]: # handle dicts with nested lists : attack_mitigations
                    _find_iocs[key][nk].extend(_found_iocs[key][nk])
            elif key in at:  # handle keys that dicts with nested lists.
        ...
        for k, v in result.items():
            if k == am: # handle dicts with nested lists,attack_mitigation
                temp_am = {}
                for nk,nv in result[k].items():
                    temp_am.update({nk:list(set(nv))})
                result[k] = temp_am
            elif k in at: # handle dicts with nested lists, 'attack_techniques' or  'attack_tactics'
                temp_at = {}
                for nk,nv in result[k].items():
                    temp_at.update({nk:list(set(nv))})
                result[k] = temp_at
            else: # handle lists
                result[k] = list(set(v))
