...
class LoggingConfiguration:
    ...
    def initLogging ( config : Config, loggingBlock : str = 'Logging', baseConfigBlock : str = None ):
        ...
        if incremental or not loggingConfig:
            ...
            logFile      = config.value( 'logfile',      DEFAULT_LOGFILE )
            logFormat    = config.value( 'logformat',    DEFAULT_LOGFORMAT )
            logLevel     = config.value( 'loglevel',     DEFAULT_LOGLEVEL )
            logToConsole = config.value( 'logToConsole', False)
