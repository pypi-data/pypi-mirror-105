...
class UpdateUser(BaseKeyCloak):
    ...
    """
    def __call__(self, email, first_name='', last_name='', enabled=True, is_staff=False, is_superuser=False):
...
class CreateUser(BaseKeyCloak):
    def __call__(self, email, first_name='', last_name='', enabled=True, email_verified=False, password=None, temporary_password=True, is_staff=False, is_superuser=False, send_email_to_user=False):
        self.keycloak_admin.refresh_token()
        ...
        if len(users) == 1:
            ...
            if send_email_to_user:
                mail.send_email(
                    ...
                    template_uuid=settings.MAIL_USER_CREATION_TEMPLATE_UUID,
                    brand_group_uuid=settings.EMAIL_BRAND_GROUP_UUID,
                    subject=settings.MAIL_USER_CREATION_SUBJECT,
                    context={
...
class UpdatePassword(BaseKeyCloak):
    def __call__(self, email, password, temporary=False, send_email_to_user=False):
        self.keycloak_admin.refresh_token()
        user_id_keycloack = self.keycloak_admin.get_user_id(email)
        ...
        if send_email_to_user:
            mail.send_email(
                email=email,
                from_email=settings.DEFAULT_FROM_EMAIL,
                template_uuid=settings.MAIL_USER_PASSWORD_NOTIFICATION_TEMPLATE_UUID,
                brand_group_uuid=settings.EMAIL_BRAND_GROUP_UUID,
                subject=settings.MAIL_USER_PASSWORD_NOTIFICATION_SUBJECT,
