...
def _write_per_twitch_metric_labels(
    ...
    for (
        iter_width_percent,
        iter_metric_name,
    ) in TWITCH_WIDTH_METRIC_DISPLAY_NAMES.items():
    ...
    for (
        iter_width_percent,
        iter_metric_name,
    ) in CONTRACTION_COORDINATES_DISPLAY_NAMES.items():
    ...
    for (
        iter_width_percent,
        iter_metric_name,
    ) in RELAXATION_COORDINATES_DISPLAY_NAMES.items():
        if iter_width_percent in twitch_coordinate_values:
            curr_sheet.write(curr_row, 0, f"{iter_metric_name} (seconds)")
            curr_row += 1
            curr_sheet.write(curr_row, 0, f"{iter_metric_name} (μN)")
    ...
    for (
        iter_width_percent,
        iter_metric_name,
    ) in CONTRACTION_TIME_DIFFERENCE_DISPLAY_NAMES.items():
    ...
    for (
        iter_width_percent,
        iter_metric_name,
    ) in RELAXATION_TIME_DIFFERENCE_DISPLAY_NAMES.items():
        if iter_width_percent in twitch_time_diff_values:
            curr_sheet.write(curr_row, 0, iter_metric_name)
            curr_row += 1
    return curr_row
        ...
        if iter_width_percent in twitch_time_diff_values:
            for iter_twitch_index in range(number_twitches):
                ...
                if not isinstance(coordinates, tuple):
                    raise NotImplementedError(
                        f"The coordinate value under key {coord_uuid} must be a tuple."
                    )
                x_value: float
...
class PlateRecording(FileManagerPlateRecording):
    ...
    def _write_xlsx_aggregate_metrics(
        self, twitch_width_values: Tuple[int, ...]
    ) -> None:
            ...
            for well_index in well_indices:
                ...
                try:
                    ...
                    if iter_sub_metric_name == "Mean":
                        ...
                        if (
                            metrics_dict["std"] is not None
                            and metrics_dict["mean"] is not None
                        ):
                    ...
                    if iter_metric_uuid in (
                        ...
                        if (
                            iter_sub_metric_name != "CoV"
                        ) and value_to_write is not None:  # coefficients of variation are %, not a raw time unit
                            value_to_write /= CENTIMILLISECONDS_PER_SECOND
                    if iter_metric_uuid in (
                        CONTRACTION_VELOCITY_UUID,
                        RELAXATION_VELOCITY_UUID,
                ...
                try:
                    curr_sheet.write(
                        curr_row, 2 + well_index, value_to_write, cell_format
                    )
                except TypeError:
                    pass
