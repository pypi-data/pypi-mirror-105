...
# ⋅                                   see format in annotatedstring.py
# ⋅
# ⋅o  "textrefs limits": (None or a TextRef* object) every textrefs stored in
# ⋅                      the AnnotatedText has to be inside
# ⋅                      .options["textrefs limits"]
...
class AnnotatedText:
    """
        ...
        ⋅.options["TextRef class(type)" .
        ⋅
        ⋅You can discard references outside certain limits: use
        ⋅.options["textrefs limits"] to accept only the textual references
        ⋅that are inside .options["textrefs limits"] .
    ...
    def _get_textref_object(self,
        ...
        """
            ...
            is stored in .options["TextRef class(type)"]
        ...
        """
        if source_string:
            res = self.options["TextRef class(type)"]()
            res.init_from_str(source_string)
            return res

        return self.options["TextRef class(type)"]()
    ...
    def add_str(self,
        ...
        # If .options["textrefs limits"] is definied, we check that <textref> is
        # inside .options["textrefs limits"]:
        if self.options["textrefs limits"] is not None and \
           not textref.is_inside(self.options["textrefs limits"]):
            return False
    ...
    def set_limits(self,
        ...
        """
            ...
            Initialize self.options["textrefs limits"].
            ...
            ARGUMENTS:
            o  limits: TextRef* object to be directly stored in self.options["textrefs limits"]
                It is assumed and checked that the TextRef* object in <limits> is a
                .options["TextRef class(type)"] object .
        ...
        """
        if limits:
            if limits.__class__.__name__ != self.options["TextRef class(type)"].__class__.__name__:
                ...
                # ⋅to atext.options["TextRef class(type)"].
                # ⋅In other words you can't set limits using a TextRefOEVerses object to an
                # ⋅AnnotatedText object whose .options["TextRef class(type)"] is
                # ⋅TextRefDefault .
                error = MusaMusaError()
                ...
                error.msg = "Can't set limits to an AnnotatedText object: " \
                    ...
                    "defined in self.options['TextRef class(type)'] " \
                    f"(={self.options['TextRef class(type)']}"
                self.errors.append(error)
                return False

            self.options["textrefs limits"] = limits
            return True
        ...
        if limits_str:
            ...
            if _limits.errors.zero_error_or_warning():
                self.options["textrefs limits"] = _limits
                return True
            return False

        # can't set self.options["textrefs limits"]:
        return False
