...
# SonyCP.py
#
# This file contains the code to talk to a Sony Projector. It was updated from the original to
...
class Projector:
    ...
    # Return true or false if the projector is powered on
    ...
    # Return the actual status of the projector
    
    def get_power_status(self):
        data = self._send_command(action=ACTIONS["GET"], command=COMMANDS["GET_STATUS_POWER"])

        # Find value and return the key
    ...
    # Change the lens position... Sony calls this "picture position", but in fact you are moving the
    # lens from 1.85.1 to 2.35.1 or some custom layouts
    
    def set_picture_position(self, pic_pos: str):

        # Make sure the argument is valid

        try:
            lens_pos = PICTURE_POSITION[pic_pos]
        except BaseException as excpt:
            return False
        
        data = self._send_command(action=ACTIONS["SET"], command=COMMANDS["PICTURE_POSITION"],
                                  data=lens_pos)
        return data

    # Get the lens position (Sony calls it "picture position").
    
    def get_picture_position(self):
        data = self._send_command(action=ACTIONS["GET"], command=COMMANDS["PICTURE_POSITION"])

        # Find value and return the key

        for key, value in PICTURE_POSITION.items():
            if value == data:
                return key
