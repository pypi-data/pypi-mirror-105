...
class TqApi(object):
    ...
    def get_quote_list(self, symbols: List[str]) -> List[Quote]:
        ...
        """
        if self._loop.is_running():
            self.create_task(self._get_quote_async(symbols))  # 协程中需要等待合约信息，然后发送订阅请求
            ...
            if set(symbols) - self._requests["quotes"]:
                ...
                while any([quote["datetime"] == "" for quote in quote_list]):
                    ...
                    if not self.wait_update(deadline=deadline):
                        raise Exception(f"获取 {symbols} 的行情信息超时，请检查客户端及网络是否正常")
            return quote_list
    ...
    def _ensure_symbol(self, symbol: Union[str, List[str]]):
        ...
        if self._stock is False:
            ...
            self.create_task(self._ensure_symbol_async(symbol_list))
        else:
