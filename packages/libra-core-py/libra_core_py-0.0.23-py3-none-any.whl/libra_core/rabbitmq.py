...
class RabbitmqClient(object):
    ...
    def consume_loop(self, callback, no_ack=True):
        while True:
            self._consume_once(callback, no_ack)

    def _consume_once(self, callback, no_ack=True):
        while True:
            try:
                while not self._connect():
                    sleep(0.1)
                if not self._queue:
                    ...
                    self._queue = q.method.queue
                self._channel.basic_qos(prefetch_count=1)
                mframe, hframe, body = self._channel.basic_get(queue=self._queue, no_ack=no_ack)
                if body is not None:
                    callback(body)
                    if not no_ack:
