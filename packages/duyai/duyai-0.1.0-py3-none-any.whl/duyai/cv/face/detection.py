...
def mtcnn_detect(frame, align=False):
    ...
    if mtcnn_model is None:
        mtcnn_model = model_utils.load_mtcnn_model()
    ...
    return dlib_align(frame, boxes) if align is True else boxes
...
def test(id_cam=0, func='ssd', align=False, show=True):
    ...
    while ret==True :
        ...
        if func == 'ssd':
            ...
            boxes = mtcnn_detect(frame2, align=align)
        elif func== 'dlib':
            boxes = dlib_hog_detect(frame2, align=align)
        else:
...
if __name__ == "__main__":
    test(show=True, func='ssd', align=False)