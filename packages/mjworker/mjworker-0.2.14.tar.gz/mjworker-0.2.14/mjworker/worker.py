...
def run(server, workerid, sessionid, timeout):
    ...
    task_proc = Process(target=task_runner, args=(master_data,))
    publish_proc = Process(target=publish_runner, args=(master_data,))
    sync_proc = Process(target=sync_runner, args=(master_data,))
    task_proc.daemon = True
    publish_proc.daemon = True
