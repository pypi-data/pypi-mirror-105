...
from mjworker.variables import pid_files, sync_pidfile
from mjworker.sync import sync_status
...
def run(server, workerid, sessionid, timeout):
    ...
    task_process = Process(target=task_runner, args=(master_data,))
    publish_process = Process(target=publish_runner, args=(master_data,))
    task_process.start()
