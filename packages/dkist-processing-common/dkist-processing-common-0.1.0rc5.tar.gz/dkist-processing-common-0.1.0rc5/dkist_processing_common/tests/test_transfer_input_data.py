...
def transfer_l0_data(recipe_run_id, tmp_path):
    ...
    task._scratch.scratch_base_path = tmp_path
    ...
    input_dataset = {
        ...
        "frames": [
            (tmp_path / "objectkey1").as_posix(),
            (tmp_path / "objectkey2").as_posix(),
            (tmp_path / "objectkey3").as_posix(),
        ],
    }
...
def test_format_transfer_items(transfer_l0_data):
    ...
    for item in task.format_transfer_items():
        assert item.source_path.as_posix() in input_dataset["frames"]
        assert item.destination_path.as_posix() in filenames
