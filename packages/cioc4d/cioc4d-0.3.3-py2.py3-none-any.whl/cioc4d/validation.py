    ...
    def run(self, _):
        ...
        for gpath in asset_cache.data(self._submitter):
            ...
            if asset_path.startswith(dest_path):
                ...
                self.add_error(
                    "The destination directory for rendered output ({}) contains assets that are in the upload list. This can cause your render to fail. See the script editor for details.".format(dest_path)
                )
                break
...
class ValidateAbsoluteWindowsPaths(Validator):
    def run(self, _):
        ...
        if bad_assets:
            msg = "Some assets contain Windows drive letters. "
            msg += "Asset paths should be specified by file basename only (if they are in the tex folder). "
            msg += "If they are not under the tex folder, please specify a path relative to the C4D file, or consider using UNC paths. "
            msg += "See the console for details."
            self.add_warning(msg)
            c4d.WriteConsole("-" * 30)
            c4d.WriteConsole("The following asset paths should be made relative or use UNC paths.")
