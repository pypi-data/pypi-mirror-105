...
import warnings
import zlib
from typing import Any, Callable, Dict, Iterable, NamedTuple, Sequence, Tuple, Type, Union

from .backend import AbstractBackend


__all__ = 'Hermes', 'Mangler', 'Cached', 'Serialiser', 'Compressor'


class Serialiser(NamedTuple):
...
class Cached:
  ...
  _keyFunc: Callable = None
  '''Key creation function.'''

  _tags: Sequence[str] = None
...
class Hermes:
  ...
  :argument cachedcls:
    Optional, typically a subclass, cache-point class
  :argument ttl:
  ...
  def __init__(
    ...
    *,
    mangler: Mangler = None,
    cachedcls: Type[Cached] = Cached,
    ttl: int = 3600,
    **backendconf
  ):
    self.ttl = ttl
    ...
    if isinstance(backend, AbstractBackend):
      ...
      raise ValueError('Expected class or instance of AbstractBackend')

    assert issubclass(cachedcls, Cached)
    self.cachedcls = cachedcls

  def __call__(self, *args, ttl: int = None, tags: Sequence[str] = None, key: Callable = None):
    '''
    ...
    if args:
      ...
      if callable(args[0]) or inspect.ismethoddescriptor(args[0]):
        return self.cachedcls(self, args[0])
      else:
        raise TypeError('First positional argument must be callable or method descriptor')
    else:
      # @cache()
      return lambda fn: self.cachedcls(self, fn, ttl = ttl, tags = tags, key = key)

  def clean(self, tags: Sequence[str] = None):
    '''
