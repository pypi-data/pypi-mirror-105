...
from ..backend import AbstractLock, redis
...
class TestRedisLock(test.TestCase):
  ...
  def setUp(self):
    cache = Hermes(redis.Backend, host = os.getenv('TEST_REDIS_HOST', 'localhost'))
    cache.clean()
  ...
  def testWith(self):
    with self.testee:
      ...
      client  = redis.Backend(Mangler(), host = os.getenv('TEST_REDIS_HOST', 'localhost')).client
