...
class TestManualSnippets(unittest.TestCase):
  ...
  def testNonTagged(self):
    import hermes.backend.dict  # noqa
...
class TestFacade(unittest.TestCase):
  ...
  def testBackendInstanceInstantiation(self):
    mangler = hermes.Mangler()
    backend = hermes.backend.AbstractBackend(mangler)
    testee = hermes.Hermes(backend, mangler = mangler)
  ...
  def testBackendInstanceWarning(self):
    ...
    with self.assertWarns(Warning) as ctx:
      testee = hermes.Hermes(backend, mangler = mangler, someBackendOpt = 13)
    self.assertEqual(
...
class TestDefaultMangler(unittest.TestCase):
  ...
  testee: hermes.Mangler

  def setUp(self):
    self.testee = hermes.Mangler()
  ...
  def testSerialisationCustom(self):
    self.testee.serialiser = hermes.Serialiser(lambda obj: json.dumps(obj).encode(), json.loads)
  ...
  def testSerialisationCompressorCustom(self):
    self.testee.compressor = hermes.Compressor(lzma.compress, lzma.decompress, lzma.LZMAError, 100)
...
class TestCachedWrapping(test.TestCase):
  ...
  def setUp(self):
    mangler = hermes.Mangler()
    backend = hermes.backend.dict.Backend(mangler, ttlWatchSleep = 0.001)
    self.testee = hermes.Hermes(backend, mangler = mangler)
  ...
  def testUncallable(self):
    ...
    self.assertEqual(
      'First positional argument must be callable or method descriptor', str(ctx.exception)
    )

  def testClassAccess(self):
    self.assertIsInstance(self.fixture.__class__.simple, hermes.Cached)
    self.assertIsInstance(self.fixture.__class__.simple._callable, types.FunctionType)

    self.assertIsInstance(self.fixture.simple, hermes.Cached)
...
class CustomMangler(hermes.Mangler):
  ...
  compressor = hermes.Compressor(zlib.compress, zlib.decompress, zlib.error, 0)
...
class TestDictCustomMangler(test.TestCase):
  ...
  def setUp(self):
    self.testee = hermes.Hermes(
      hermes.backend.dict.Backend, mangler = CustomMangler(), ttl = 360, ttlWatchSleep = 0.01
    )
    self.addCleanup(self.testee.backend.stop)
...
class TestDictCustomCached(test.TestCase):
  ...
  def setUp(self):
    self.testee = hermes.Hermes(
      BackendThatIsDown, cachedcls = CustomCached, ttl = 360, ttlWatchSleep = 0.01
    )
    self.fixture = test.createFixture(self.testee)
