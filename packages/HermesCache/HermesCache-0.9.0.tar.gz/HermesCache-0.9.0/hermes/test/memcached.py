...
from .. import Hermes, Mangler, test
from ..backend import AbstractLock, memcached
...
class TestMemcached(test.TestCase):
  ...
  def setUp(self):
    self.testee = Hermes(
      memcached.Backend,
      servers = [os.getenv('TEST_MEMCACHED_HOST', 'localhost')],
      ttl = 360,
    )
  ...
  def testLazyInit(self):
    ...
    Hermes(
      memcached.Backend,
      servers = ['{}:{}'.format(os.getenv('TEST_MEMCACHED_HOST', 'localhost'), server.port)],
    )
...
class TestMemcachedLock(test.TestCase):
  ...
  def setUp(self):
    self.cache = Hermes(
      memcached.Backend, servers = [os.getenv('TEST_MEMCACHED_HOST', 'localhost')]
    )
    self.cache.clean()
  ...
  def testWith(self):
    with self.testee:
      ...
      client = memcached.Backend(
        Mangler(), servers = [os.getenv('TEST_MEMCACHED_HOST', 'localhost')]
      ).client
