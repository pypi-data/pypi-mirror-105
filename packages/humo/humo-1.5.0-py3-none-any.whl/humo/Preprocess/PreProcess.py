...
#*NumberOfMuscle, EMGdevicename
    ...
    def convert2pickle(self, **kwargs):
        ...
        data = {
            "device":[self.device, list(self.deviceheader)],
            "model":[self.model, list(self.modelheader)],
            "marker":[self.marker, list(self.markerheader)],
            "spep":self.spep,
            "MMT":self.MMT,
            "rawMMT":self.rawMMT,
            "ID":self.id,
        ...
        if not kwargs:
            ...
            fileType = [("", "*.pkl")]
            startdir = pathlib.Path.home()
            ret = tkinter.filedialog.asksaveasfilename(defaultextension="pkl", filetype=fileType, initialdir=startdir, title="Save as...")
            p = pathlib.Path(ret)
            files = []
