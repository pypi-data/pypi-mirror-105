			...
			if adjustframes == True:
				if type(ColNumber) == list:
					...
					for i in ColNumber:
						...
						elif "unit" in kwargs.keys():
							if kwargs["unit"] == "degree":
								data.append(args[0]._model.iloc[1:-1,i:i+3].values)
							elif kwargs["unit"] == "radian":
								data.append(np.radians(args[0]._model.iloc[1:-1,i:i+3].values))
						else:
							data.append(args[0]._model.iloc[1:-1,i:i+3].values)
					try:
						f = args[0].fadjust
					...
					if not kwargs:
						...
						if kwargs["unit"] == "degree":
							...
							data = np.radians(args[0]._model.iloc[1:-1,ColNumber:ColNumber+3].values)
					else:
						data =  args[0]._model.iloc[1:-1,ColNumber:ColNumber+3].values
					try:
						f = args[0].fadjust
				...
				if type(ColNumber) == list:
					...
					for i in ColNumber:
						if not kwargs:
							...
							if kwargs["unit"] == "degree":
								...
								data.append(np.radians(args[0]._model.iloc[:,i:i+3].values))
							else:
								data.append(args[0]._model.iloc[:,i:i+3].values)
						else:
							data.append(args[0]._model.iloc[:,i:i+3].values)
					...
					if not kwargs:
						...
						if kwargs["unit"] == "degree":
							...
							data = np.radians(args[0]._model.iloc[:,ColNumber:ColNumber+3].values)
						else:
							data = args[0]._model.iloc[:,ColNumber:ColNumber+3].values
					else:
						data = args[0]._model.iloc[:,ColNumber:ColNumber+3].values
