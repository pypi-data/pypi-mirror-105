...
import { recordingViewPlugins, sortingUnitViewPlugins, sortingViewPlugins } from "../../pluginInterface";
...
const ViewLauncher = ({ onLaunchSortingView, onLaunchRecordingView, onLaunchSortingUnitView, selection, hasSorting }) => {
    const plugins = usePlugins();
    const sortingUnitViewPlugin = sortingUnitViewPlugins(plugins).filter(p => (p.name === 'MVSortingUnitView'))[0];
    return (React.createElement(Fragment, null,
        React.createElement("div", { key: "recordingViews", style: { flexFlow: 'wrap' } }, sortByPriority(recordingViewPlugins(plugins)).filter(p => (p.name !== 'MVRecordingView')).map(rv => (React.createElement(LaunchRecordingViewButton, { key: rv.name, plugin: rv, onLaunch: onLaunchRecordingView })))),
        hasSorting && (React.createElement("div", { key: "sortingViews", style: { flexFlow: 'wrap' } }, sortByPriority(sortingViewPlugins(plugins)).filter(p => (p.name !== 'MVSortingView')).map(sv => (React.createElement(LaunchSortingViewButton, { key: sv.name, plugin: sv, onLaunch: onLaunchSortingView }))))),
...
const LaunchRecordingViewButton = ({ plugin, onLaunch }) => {
    const handleClick = useCallback(() => {
        onLaunch(plugin);
    }, [onLaunch, plugin]);
    return (React.createElement("button", { onClick: handleClick, style: buttonStyle },
