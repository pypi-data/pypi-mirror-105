...
class QRCodeAdmin(admin.ModelAdmin):
    ...
    def scan_view(self, request):
        if request.method == 'POST':
            uuid = request.POST['short_uuid']
            try:
                code = QRCode.objects.get(uuid)
                return redirect('admin:qrtoolkit_core_qrcode_change', kwargs={'object_id': code.pk})
            except QRCode.DoesNotExist:
                self.message_user(request, f'Qr code with uuid {uuid} not found.', level=messages.WARNING)
