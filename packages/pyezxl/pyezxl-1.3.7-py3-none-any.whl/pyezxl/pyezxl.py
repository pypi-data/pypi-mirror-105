...
class pyezxl:
    ...
    def copy_range_x(self, sheet_name1, sheet_name2, cut_num, paste_num):
        # 현재는 1개의 가로열만 가능하도록 하였다
        sheet1 = self.check_sheet_name(sheet_name1)
        sheet2 = self.check_sheet_name(sheet_name2)

        sheet1.Rows(str(cut_num) + ':' + str(cut_num)).Select()
        sheet1.Rows(str(cut_num) + ':' + str(cut_num)).Copy()
        sheet2.Select()
        sheet2.Rows(str(paste_num) + ':' + str(paste_num)).Select()
        sheet2.Paste()

    def copy_range_y(self, sheet_name1, sheet_name2, cut_num, paste_num):
        # 세로의 값을 복사합니다
        # 현재는 1개의 세로행만 가능하도록 하였다
        sheet1 = self.check_sheet_name(sheet_name1)
        sheet2 = self.check_sheet_name(sheet_name2)

        cut_num_char = self.change_num_char(cut_num)
        paste_num_char = self.change_num_char(paste_num)

        sheet1.Rows(str(cut_num_char) + ':' + str(cut_num_char)).Select()
        sheet1.Rows(str(cut_num_char) + ':' + str(cut_num_char)).Copy()
        sheet2.Select()
        sheet2.Rows(str(paste_num_char) + ':' + str(paste_num_char)).Select()
        sheet2.Paste()
    ...
    def delete_x_value(self, sheet_name, num):
        # 한줄값만 삭제하기
        #num = self.change_char_num(num)
        sheet = self.check_sheet_name(sheet_name)
        sheet.Rows(str(num) + ':' + str(num)).ClearContents()

    def delete_y_value(self, sheet_name, num):
        # 한줄값만 삭제하기
        sheet = self.check_sheet_name(sheet_name)
        num_check = self.change_num_char(num)
        sheet.Columns(str(num_check) + ':' + str(num_check)).ClearContents()
    ...
    def insert_line_x(self, sheet_name, num):
        # 가로열을 한줄삽입하기
        sheet = self.check_sheet_name(sheet_name)
        sheet.Range(str(num) + ':' + str(num)).Insert()

    def insert_line_y(self, sheet_name, num):
        # 세로행을 한줄삽입하기
        sheet = self.check_sheet_name(sheet_name)
        num_r1 = self.change_num_char(num)
        sheet.Columns(str(num) + ':' + str(num)).Insert()
    ...
    def set_range_name(self, name, xyxy):
        #영역에 이름을 지정한다
        self.write_range_name(name, xyxy)
