...
class TextQueryOp:
    ...
    def __init__(self, op):
        if op not in ['OR', 'AND', 'NOT']:
            logging.error(f'TextQueryOp has been called with an invalid argument: {op}')
        self.op = op
            ...
            for i, subquery in enumerate(query.subqueries):
                if type(subquery) == TextQueryOp and subquery.op == 'NOT':
                    ...
                    try:
                        ...
                        logging.error(f'Invalid query: Found NOT at the end of a query: {query.to_str()}')
...
def smart_expand_query(query_str: str, related_terms_map: Dict) -> TextQuery:
    """Expands the query with the given related terms map.
    It only solves one problem: if we use the approach in TextQuery itself,
    it doesn't replace synonym queries with other synonyms. For example:
    For the query 'hydrogen mobility' we might have the synonym
    'hydrogen AND fuel cells'; but then we wouldn't expand 'hydrogen' with
    the term-synonym '5fb1b72a2ac8af76ac7d751f' (its label).
    This could be left out as well but it does change the results and it seems
    more intuitive and expected to add this behavior here.
    NOTE: this does not work 100% yet because we actually have to generate the
    synonyms in query-resolution as well. If we don't even look for any labels
    for the single term 'hydrogen', we can't add it here."""
    # First, we need to parse the strings in related_terms_map because otherwise
    # we won't know if they are queries or not.
    parsed_synonyms = [
        {
            "original": synonym["original"],
            "term": TextQuery.from_str(synonym["term"]),
            "source": synonym["source"]
        }
        for synonym in related_terms_map["synonyms"]
    ]
    parsed_queries_map = {
        "synonyms": [
            synonym for synonym in parsed_synonyms
            if len(synonym['term'].get_terms()) >= 2
        ]
    }
    # Create a TextQuery object and expand with query-synonyms
    query = TextQuery.from_str(query_str)
    if len(parsed_queries_map['synonyms']) >= 1:
        query.add_related_terms_from_map(parsed_queries_map)
        # Create a copy with the expanded query synonyms
        query = TextQuery.from_str(query.to_str())
    parsed_terms_map = {
        "synonyms": [
            synonym for synonym in parsed_synonyms
            if len(synonym['term'].get_terms()) == 1
        ]
    }
    query.add_related_terms_from_map(parsed_terms_map)
    return query
