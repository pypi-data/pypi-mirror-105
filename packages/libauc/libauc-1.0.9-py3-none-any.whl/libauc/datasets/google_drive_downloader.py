# credit for https://github.com/ndrplz/google-drive-downloader
...
class GoogleDriveDownloader:
    ...
    def download_file_from_google_drive(file_id, dest_path, overwrite=False, unzip=False, showsize=False):
        ...
        unziped_dest_path = dest_path.replace('.tar.gz', '').replace('.zip', '')
        if not exists(destination_directory):
            makedirs(destination_directory)                
        if not exists(unziped_dest_path):
            makedirs(unziped_dest_path)  
                ...
                try:
                    ...
                    if '.zip' in dest_path:
                        with zipfile.ZipFile(dest_path, 'r') as z:
                            z.extractall(unziped_dest_path)
                    elif '.tar.gz' in dest_path or '.tar' in dest_path:
                        tarfile.open(dest_path, 'r:gz').extractall(unziped_dest_path)
                    else:
                        warnings.warn('Ignoring `unzip` since "{}" does not look like a valid zip/tar file'.format(file_id))
                    print('Done.')
