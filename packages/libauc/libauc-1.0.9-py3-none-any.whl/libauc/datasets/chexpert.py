...
class CheXpert(Dataset):
    def __init__(self, 
        ...
        '''
        TODO: support training for 5 classes
        '''
        # load data from csv
        ...
        if class_index == -1: # 5 classes
            print ('Multi-class mode: True, Number of classes: [%d]'%len(train_cols))
    ...
    def image_augmentation(self, image):
        img_aug = tfs.Compose([tfs.RandomAffine(degrees=(-15, 15), translate=(0.05, 0.05), scale=(0.95, 1.05), fillcolor=128)])
        image = img_aug(image)
        return image
...
if __name__ == '__main__':
    ...
    root = '/scratch/Users/zhuoning/dataset/imagenet_data/CheXpert-v1.0-small/'
    root = '/Users/zhuoning/Experiment/dataset/CheXpert-v1.0-small/'
    ...
    # from libauc.losses import AUCMLoss, CrossEntropyLoss, FocalLoss
    # from libauc.optimizers import PESG, PESG_v2, SGD
    ...
    # model = DenseNet121(pretrained=True, last_activation='sigmoid', activations='relu', num_classes=1)
    # model = model.cuda()
    
    # Loss = AUCMLoss(imratio=0.1)
    # optimizer = PESG_v2(model, 
    #                      a=Loss.a, 
    #                      b=Loss.b, 
    #                      alpha=Loss.alpha, 
    #                      imratio=0.1, 
    ...
    for idx, data in enumerate(trainloader):
        ...
        #inputs = inputs.cpu().numpy()[0].transpose(1,2,0)
