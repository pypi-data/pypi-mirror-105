...
class BinaryMinBarStream(object):
    ...
    def __init__(self, market, instrument_id, file_path):
        ...
        if create_new:
            self._file_stream = open(file_path, 'w')
    ...
    def read_trading_days(self, bar_series, begin_trading_day, end_trading_day):
        ...
        reader = BinaryReader(self._file_stream)
        self._file_header.read(reader)
        if (end_trading_day < self._file_header.begin_trading_day) or (
                begin_trading_day > self._file_header.end_trading_day):
            return False
    ...
    def _update_file_header(self, bar_series, start, length):
        ...
        if self._file_header.begin_time == datetime.datetime(2999, 1, 1) or start_bar.begin_time < self._file_header.begin_time:
            self._file_header.begin_time = start_bar.begin_time
            self._file_header.mark_as_dirty(True)

        if self._file_header.end_time == datetime.datetime(2999, 1, 1) or start_bar.end_time > self._file_header.end_time:
            self._file_header.end_time = end_bar.end_time
            self._file_header.mark_as_dirty(True)
        self._file_header.bar_count += length

    def _write_interval(self, bar_series: BarSeries, start: int, length: int, offset: int):
        if bar_series is None:
        ...
        self._update_file_header(bar_series, start, length)
        writer = BinaryWriter(self._file_stream)
        writer.stream.seek(offset, 0)
        self._write_bars(writer, bar_series, 0, length)
        self._file_header.write(writer)

    def _write_bars(self, writer, bar_series, start, length):
        for i in range(start, length, 1):
            bar = bar_series[i]
            self._write_bar(writer, bar)

    def _write_bar(self, writer, bar):
    ...
    def write_bar_series(self, bar_series: BarSeries, start: int, length: int):
        self._write_interval(bar_series, start, length, FileHeader.FileHeaderTotalSize)
        self._file_stream.flush()
