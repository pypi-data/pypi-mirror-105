...
class Template:
    ...
    def _source_map_to_etree_map(
        ...
        cache = {}
        for key in smap:
            # FIXME: the fuck is this? We want to cache the thing itself; this will never trigger since we're iterating over a set (dict keys)!
            #   what we want to do is see if we've called labellike_to_etree on the thing before, and if so, use the cached object instead...
            if key in cache:
                smap[key] = cache[key]
            else:
                try:
