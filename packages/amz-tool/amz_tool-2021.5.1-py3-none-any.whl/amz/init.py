...
def amz_repo_init(verbose=False):
    ...
    # Check if actually inside AMZ Repo and set root dir
    amz_root_dir = os.getcwd()
    if not os.path.exists('amz.yaml'):
        # Check root of git repo.
        try:
            ...
            if p.returncode == 0 and os.path.exists(f"{p.stdout.decode()[:-1]}/amz.yaml"):
                amz_root_dir = p.stdout.decode()[:-1]
            else:
                print("You are not inside amz repo! Run the command from inside repo")
                l.end(1)
                return
        except:
            print("Error finding amz.yaml! Run the command from repo root")
            l.end(1)
            return
    ...
    try:
        ...
        # Check if it's autonomous repository
    ...
    # Ask if user wants to set this as default AMZ autonomous repo
    if IS_AUTO_REPO:
        l.chain("Do you wish to set this as the default AMZ Autonomous Repo? ", "\t")
        l.end(-3)

        while True:
            SET_DEFAULT_REPO = input("\t\t (y/n): ").lower()
            if SET_DEFAULT_REPO not in ['y', 'n']:
    ...
    try:
        ...
        gi_files = open(f'{amz_root_dir}/.gitignore').readlines()
        if not set(config_files).issubset(set(gi_files)):
            with open(f'{amz_root_dir}/.gitignore', 'a') as gi:
