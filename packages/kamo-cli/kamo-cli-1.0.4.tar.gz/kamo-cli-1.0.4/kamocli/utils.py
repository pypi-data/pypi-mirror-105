...
def check_profile(ctx):
    if not ctx.obj.client:
        abort(
            "You have no active profile or selected profile is invalid. Run 'kamo profile add [name]'"
        )
...
def check_tenant(func):
    ...
    def _check(*args, **kwargs):
        ...
        if not tenant:
            abort(
                "Before using this command please set a tenant by running: kamo tenants set [name]"
            )
        else:
...
def get_builds_from_s3_url(url, num=10):
    ...
    for p in paths[:num]:
        ...
        f.seek(0)
        c = json.load(f)
        c["s3_path"] = f"s3://{bucket_name}/{p}"
        builds.append(c)
        #logger.debug(p)
    versions = {}
    builds.sort(key=itemgetter("version_string"), reverse=True)
    for build in builds:
        versions[build["version_string"]] = build
    return versions