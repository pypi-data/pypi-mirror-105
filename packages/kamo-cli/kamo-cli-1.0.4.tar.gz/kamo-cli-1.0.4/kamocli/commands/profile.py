...
def add_profile(
    ctx: Context,
    profile: str = Option(..., "-p", "--domain", prompt="What would you like to call this profile?"),
    domain: str = Option(
        ..., "-d", "--domain", prompt="Domain name of the service (e.g. kamo.io)"
    ),
    #tenant: str = Option(..., "-t", "--tenant", prompt="Default tenant (e.g. staging)"),
    ...
    set_default_profile(profile)
    echo(
        "Profile has been added and is now active. If you want to edit it please run: kamo profile edit."
    )
    secho("Please select a tenant with 'kamo tenants pick'")
