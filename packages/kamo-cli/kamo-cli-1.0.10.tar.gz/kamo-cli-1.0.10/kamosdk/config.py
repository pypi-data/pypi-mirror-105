...
from kamosdk.utils import domain_from_api_key
...
def _prepare_profile(profile):
    ...
    try:
        ...
        if profile.get("domain"):
            ret["domain"] = profile["domain"]
        else:
            ret["domain"] = domain_from_api_key(ret["api_key"])
    except Exception as e:
        log.error(f"Error preparing profile {profile}: {e}")
        return None
    return ret


def create_profile(name: str, api_key: str, domain: Optional[str] = None, skip_auth: bool = False) -> None:
    """
    ...
    """
    contents: Dict = {"api_key": api_key, "domain": domain}
    if skip_auth:
