...
@app.command("ensure")
@check_tenant
def ensure(ctx: Context, 
    num: int = Argument(..., help="Number of instances"),
    instance_type: str = Option(
        DEFAULT_INSTANCE_TYPE, "--type", "-t", help="Type of EC2 instance to launch"
    ),
    is_wait: bool = Option(
        False, "-w", "--wait", is_flag=True, help="Wait until instance is ready"
    ),
    ):
    """
        ...
        for node in nodes:
            ...
            if node["status"] == "unknown":
                ...
                try:
                    ...
                    abort(f"Could not remove broken instance {node['node_id']}. Please delete it manually before continuing.")
        nodes = find_nodes(ctx, tenant)
...
def find_nodes(ctx, tenant=None):
    ...
    nodes = content["nodes"]
    if tenant:
        nodes = [n for n in content["nodes"] if n.get("tenant") == tenant]
    return nodes
...
def list_nodes(
    ...
    node_list = []
    columns = ["node_id", "status", "registered", "last_modified", "instance_type", "tenant"]
    for node in nodes:
        node_row = []
        for k in columns:
            v = node.get(k)
...
def terminate_all(
    ctx: Context,
    is_all: bool = Option(
        False, "-a", "--all", is_flag=True, help="Teminate all nodes for all tenants"
    ),
