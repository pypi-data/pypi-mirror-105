...
class ServiceSession(requests.Session):
    ...
    def __init__(self, domain = None, api_key = None, profile=None, tenant=None, *args, **kwargs):
        ...
        if self.profile_name:
            ...
            if not api_key:
                raise InvalidProfile(
                    "No profile specified and KAMO_API_KEY not set in environment"
                )
    ...
    def _initialize(self) -> None:
        ...
        if self.api_key:
            try:
                self.token = get_access_token(self.api_key, self.root_info["config"]["auth_client_id"], self.root_info["config"]["auth_url"])
                self.headers["Authorization"] = "Bearer {}".format(self.token)
            except Exception as ex:
                log.warning("Error fetching access token: %s", str(ex))
                self.api_key = ""
                self._save()
        self._save()
    ...
    def _do_request(self, method, retry=True, *args, **kwargs):
        ...
        self.headers.update({"Kamo-Api-Key": self.api_key})
        if method == "get":
    ...
    def fetch_root_info(self) -> Dict:
        log.debug(
            "fetch_root_info(): domain: {0}, headers: {1}".format(
                self.domain, self.headers
            )
        )
        ...
        try:
            ...
            raise ServerError(
                f"Could not reach server {root_url} ({ex})"
            ) from None
        ...
        if "application/json" not in r.headers["Content-Type"]:
            raise ServerError(
                "Unexpected response: %s" % r.text, url=root_url
            ) from None
        ret = r.json()
        self.root_info = ret
        return ret
