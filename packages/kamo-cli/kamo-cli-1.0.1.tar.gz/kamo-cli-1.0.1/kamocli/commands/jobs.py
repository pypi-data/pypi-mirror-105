...
def list_jobs(
    ...
    is_all: bool = Option(
        False, "-a", "--all", is_flag=True, help="Show all jobs regardless of status"
    ),
):
    """List server jobs"""
    ...
    for job in resp.json()["jobs"]:
        ...
        lst.append(
            [
                ...
                job["node_id"],
                address,
                fmt_date(job["created"]),
                fmt_date(job["last_modified"]),
                job.get("result"),
                heartbeat_txt
            ]
        )
...
def shutdown_job(
    ...
    resp = ctx.obj.session.patch(url, json={"shutdown": True})
...
def shutdown_all(
    ...
    is_force: bool = Option(False, "-y", is_flag=True, help="Force a shutdown without a prompt"),
    is_wait: bool = Option(
        False, "-w", "--wait", is_flag=True, help="Wait until jobs have been shut down"
    ),
):
...
def cleanup(
    ...
    resp = ctx.obj.session.get(jobs_url, params={"tenant": tenant, "start": 0, "end": 100, "status": "running,pending,processing"})
    n = 0
    for job in resp.json()["jobs"]:
        node_id = job["node_id"]
        url = ctx.obj.session.knode_url + f"/nodes/{node_id}"
...
def create_job(
    ...
    version: str = Option(
        None, "-v", "--version", help="Version to deploy (or latest)"
    ),
    ...
    executable: str = Option(
        None, "-e", "--executable", help="Executable name and path (if not standard)"
    ),
    map_name: str = Option(
        None,
        "--map",
    ...
    custom_args: str = Option(
        "?listen,-server,-NullRHI", "-a", "-args", help="Override default arguments"
    ),
    ...
    if process_per_volume:
        ...
        if map_name and not volumes:
            ...
            if map_name not in volumes_per_map:
                abort(f"Map '{map_name}' not found on DB, only these exist: {list(volumes_per_map.keys())}.\nTry deploy without the --ppv switch to populate the DB.")

            volumes = " ".join([volume or "main" for volume in volumes_per_map[map_name]])

    executable = executable or "LinuxServer/KamoOneProject/Binaries/Linux/KamoOneProjectServer"
    ...
    if not s3_path:
        ...
        if version != "latest":
            if version not in versions:
                abort(f"Version '{version}' not found for tenant '{tenant}'.\nAvailable versions: {', '.join(versions.keys())}")
        else:
        ...
        try:
            ...
            abort(f"Could not load buildinfo.json from {s3_path}. Was the path correct?")
    ...
    if not is_force:
        confirm(
            f"Do you want to create {len(launches)} jobs with version '{version}' from '{s3_path}' on the '{tenant}' tenant?",
            abort=True,
        )
