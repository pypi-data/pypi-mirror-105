...
def list_profiles(ctx: Context):
    ...
    for profile_name in sorted(profiles):
        root_url = profiles[profile_name]["root_url"]
        col = ""
        if get_default_profile() == profile_name:
            col = "green"
            root_url += " (selected)"
        secho(f"{profile_name:10} > {root_url}", fg=col)
    echo("")
...
def add_profile(
    ctx: Context,
    profile: str = Argument(..., help="Profile name to add"),
    domain: str = Option(
        ..., "-d", "--domain", prompt="Domain name of the service (f.ex. kamo.io)"
    ),
    api_key: str = Option(None, "-k", "--api-key", help="API Key (if not set a browser will open)"),
    ...
    if not skip_auth:
        if not api_key:
            url = f"https://kamo.{domain}/"
            try:
                resp = requests.get(url, timeout=2)
                resp.raise_for_status()
            except Exception:
                secho("%s is not reachable." % url, fg="red")
                if not confirm("Are you sure you want to add this domain?"):
                    return

    try:
        create_profile(profile, api_key=api_key, domain=domain, skip_auth=skip_auth)
    except InvalidProfile as ex:
        abort(ex)
    set_default_profile(profile)
    echo(
        "Profile has been added and is now active.\n"
        "If you want to edit it please run: kamo profile edit."
    )
