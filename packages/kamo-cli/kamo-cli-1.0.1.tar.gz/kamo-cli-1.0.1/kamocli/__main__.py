...
def cli(ctx: Context,
    ...
    if ctx.obj is None:
        ...
        try:
            ...
            log.debug(ex)
    try:
        out(f"Tenant: {ctx.obj.session.profile.tenant} - Profile: {ctx.obj.session.profile_name} - Endpoint: {ctx.obj.session.profile.domain}")
    except:
        pass
...
def version(ctx: Context):
    ...
    ret["knode_build_info"] = (
        requests.get(ctx.obj.session.get_url("knode")).json().get("build_info")
    )
...
from kamocli.commands import tenants, profile, login, db, nodes, builds, jobs
