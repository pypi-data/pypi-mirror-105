...
class LocalDev(KamoWorld):
    ...
    def update_backend(self):
        backend_url = "git@github.com:directivegames/kamo-backend.git"

        project_root = pathlib.Path(self.project_root).resolve().parent
        backend_root = project_root.joinpath("kamo-backend")
    ...
    def start_local_backend(self):
        ...
        backend_root = pathlib.Path(self.project_root).resolve().parent.joinpath("kamo-backend")
        ...
        subprocess.Popen(
            "start pipenv run knode jobs process -d", cwd=backend_root.as_posix(), shell=True
        )
    ...
    def start_servers(self):
        ...
        cmd_args = [arg.strip() for arg in list(filter(None, cmd_args.split(",")))] + [
            f"-LogCmds={log_cmds}"
        ]
        ...
        if sys.platform == "darwin":
            if use_debug:
                executable_name = (
                    "UE4Editor-Mac-DebugGame.app/Contents/MacOS/UE4Editor-Mac-DebugGame"
                )
            else:
