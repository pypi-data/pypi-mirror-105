...
from kamocli.utils import dumps, abort, print_tab, fmt_date, check_tenant, print_table, out, success, get_builds_from_s3_url
...
@app.command("path")
def set_builds_path(ctx: Context, url: str = Argument(..., help="Full s3 url to builds. e.g. s3://at-teamcity-artifacts/Existence/Existence_BuildServer/")):
    ...
    if not builds:
        abort(f"No builds found in {url}")
    else:
...
def list_builds(ctx: Context, num: int = Option(10, help="Number of the most recent builds to view")):
    ...
    for build in builds:
        lst.append([build["version_string"], fmt_date(build["timestamp"]), build["branch"], build["s3_path"]])
    echo(tabulate(lst, headers=headers))


@app.command("view")
def view_build(ctx: Context, version: str = Argument(..., help="Build version to view"), is_wait: bool = Option(False, "-w", "--wait", is_flag=True, help="Wait for the build to be ready if it is not found")):
