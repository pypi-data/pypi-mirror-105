...
def edit_object(
    ctx: Context,
    key: str = Option(None, "-k", "--key", help="Edit a specific key")
):
    """
...
def create_region(
    ...
    state: str = Option(
        '{"handler": ""}', "-s", "--state", show_default=True, help="Region initial state"
    ),
):
    """
...
def region_create_object(ctx: Context, class_name: str, kamo_id: str = Option(None, "-k", "--kamo-id", help="Unique kamoID (optional)"), state: str = Option("{}", "-s", "--state", help="Initial kamo state for the object")):
    """
