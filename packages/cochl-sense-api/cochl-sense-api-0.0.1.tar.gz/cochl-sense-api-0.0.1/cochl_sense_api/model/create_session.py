...
class CreateSession(ModelNormal):
    ...
    def __init__(self, content_type, type, *args, **kwargs):  # noqa: E501
        ...
        Args:
            content_type (str): type of audio to send. Check  [content-type documentation](https://docs.cochl.ai/todo) for an exaustive list of available content type 
            type (AudioType):
        ...
        Keyword Args:
            ...
            total_size (int): if set, it allows to automatically change the state of the session to `readonly` when at least `total_size` bytes of audio chunk have  been uploaded.   For stream, this can be used to limit the maximum duration of the  session.  For a file, this allows to automatically start inferencing when the  whole file has been sent. We recommand to set the size for files as  it allows to do one less API call to start the inferencing . [optional]  # noqa: E501
        """
