...
def validate(file, rename):
    ...
    if diff_def["deleted_files"]:
        ...
        report.write('-------\n')
        for location, file_set in diff_def['deleted_files'].items(): # Todo if a folder is deleted or is from the previous FT to the latest then 664 need an if statement to see if it is still there
