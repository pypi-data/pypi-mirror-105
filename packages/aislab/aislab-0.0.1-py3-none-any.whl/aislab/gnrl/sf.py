...
def inv2(X, X_1, pInv):
    ...
    D = X[n1:n+1, n1:n+1]
    T1 = X_1@B
    Ai = X_1 + T1@nsinv(D - C@T1, s_min)@T1.T
...
def nsinv(X, s_min=1e-6, met='EVD', mtype='PDMR'):
    if met=='SVD':
        ...
        if np.max(d) == 0:
            print('Matrix is not invertible.') # todo: make it error
            return None # return Xinv = None
        if mtype=='PDMR':   d = np.real(d) 	# eigenvalues should be real & positive, like us all :)
        elif mtype=='NDMR': d = -np.real(d)	# eigenvalues should be real & negative
        elif mtype=='NDM': 	d = -d			# eigenvalues should be negative
        ind = np.asarray( np.nonzero(d/(np.max(d) + 1e-6) > s_min)).flatten() # ind = find(abs(d)/(max(abs(d)) + 1e-6) > s_min);
