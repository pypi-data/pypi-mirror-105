...
# def gswlinr():
###################################################################################
def bElimin(model=None, st0=None, par=None):
    Nw, z, FY, FF, wF, met, cnames, s_min, SLE, SLS, iterate, crit_nbm, ivi, ivo, n1, n2 = parin(st0, model[-1], par)
    # ----- Calculate significance of factors in the model -----
    if met == 'FR' and n2 == z:
        par['iterate'] = 0
        return model, st0, par
    ivi0 = ivi
    ivo0 = ivo
    if ivi.shape[0] > 1:
        n1 = ivi.shape[0]
        ...
        for i in np.arange(n1):
            ...
            pmi, P = mdl(submat(FF, ivi-1, ivi-1), submat(FY, ivi-1), s_min)
            pm[:, i] = pmi[:,0]
            st = st + [stmdl('BE', model[-1], submat(FY, ivi-1), P, submat(wF, ivi-1), n2, n1, st0)]
    ...
    if met == 'FR': return model, st0, par
    if flg:
        p_Fpi, pmi, sti, ind = nextmdl('BE2', model, st, pm, crit_nbm)
        if p_Fpi > SLS:
            ivi = delrc(ivi0, ind)
            ivo = np.vstack((ivo, ivi0[ind]))
            model = model + [htmdl('BE1', model[-1], 'BE', n2, ivi, ivo, cnames, ivi0[ind], pmi, sti)]
            # visualz(model, par)
        else:
            if met == 'BR': iterate = 0
            else: model[-1]['FB'] = 'BE_stop'
    elif met == 'BR': iterate = 0
    else: model[-1]['FB'] = 'BE_stop'
    par['iterate'] = iterate
    return model, st0, par
###################################################################################
def firstmdl(st0=None, par=None): 
    if par['mtp'] == 'full':
        z = st0['z']
        FF = st0['FF']
        FY = st0['FY']
        ...
        n2 = z
        pm, P = mdl(FF, FY, par['s_min'])
        st_ext = stmdl('BE1', [], FY, P, wF, n2, [], st0)
        # st_ext2 = stmdl('FS1', [], FY, P, wF, n2, [], st0)
        model = [htmdl('BE1', [], 'BE', n2, ivi, ivo, par['cnames'], [], pm, st_ext['ovr'])]
        # model[-1] = htmdl('FS1', model, 'FS', n2, ivi, ivo, par['cnames'], [], pm, st_ext['ovr'])
    elif par['mtp'] == 'empty':
        z = st0['z']
        ...
        model = [htmdl('FS1', [], 'FS', n2, ivi, ivo, par['cnames'], [], pm, st_ext['ovr'])]
    elif par['mtp'] == 'init':
        z = st0['z']
        ivi = c_(np.arange(1, int(np.floor(z/2)) + 1))
        ivo = c_(np.arange(int(np.floor(z/2)) + 1, z + 1))
        n2 = ivi.shape[1-1]
        FF = submat(st0['FF'], ivi-1, ivi-1)
        FY = submat(st0['FY'], ivi-1)
        wF = submat(st0['wF'], ivi-1)
        pm,P = mdl(FF, FY, par['s_min'])
        st_ext = stmdl('FS1', [], FY, P, wF, n2, [], st0)
        model = [htmdl('FS1', [], 'FS', n2, ivi, ivo, par['cnames'], [], pm, st_ext['ovr'])]
    else:
    ...
    return model, par
###################################################################################
def fSelect(model=None, st0=None, par=None): 
    # if len(model) > 1 and len(model[-1]['st']['in']) == 0: return model, st0, par # las1
    if model[-1]['FB'] == 'BE': 
        if par['iterate'] == 0: par['iterate'] = -1
        return model, st0, par # las1t step was BE
    ...
    if not len(ivo) == 0:
        n1 = ivi.shape[0]
        n2 = n1 + 1
        ivi0 = np.hstack((np.matlib.repmat(ivi[:, 0], z - n1, 1), ivo))
        pm = nans((n2, len(ivo)))
        st = []
        for i in np.arange(z - n2 + 1):
            ...
            pmi, P = mdl(submat(FF, ivi-1, ivi-1), submat(FY, ivi-1), s_min)
            pm[:, i] = pmi[:,0]
            st = st + [stmdl('FS',model[-1], submat(FY, ivi-1), P, submat(wF, ivi-1),n2, n1, st0)]
...
def htmdl(mode=None, model0=None, FB=None, n2=None, ivi=None, ivo=None, cnames=None, ind2=None, pm=None, st={}):
    ...
    if ind2 is not None and len(ind2) > 0: ind2 = ind2 - 1
    # Model history
    if mode == 'FS1' or mode == 'BE1':
        model['cname_i'] = np.array(cnames)[ivi - 1]
        model['cname_o'] = np.array(cnames)[ivo - 1]
        model['FB'] = FB
        model['n'] = n2
...
def parin(st0, model, par):
    ...
    SLS = par['SLS']
    iterate = par['iterate']
    crit_nbm = par['crit_nbm']
    ivi = model['ivi']  
    ivo = model['ivo']
    n1 = len(ivi) - 1
    n2 = n1 + 1
    return Nw, z, FY, FF, wF, met, cnames, s_min, SLE, SLS, iterate, crit_nbm, ivi, ivo, n1, n2
###################################################################################
def stats(x=None, y=None, w=None, cnames=None, par=None): 
    N, z = x.shape
    Nw = sum(w)
    FF = x.T@(np.matlib.repmat(w, 1, z)*x)
    FY = x.T@(w*y)
    wF = w.T@x
    mF = x.T@w/Nw
    my = y.T@w/Nw
...
def stmdl(FB=None, model=None, FY=None, P=None, wF=None, n2=np.nan, n1=np.nan, st0=None): 
    ...
    sse = np.max([0, ssy - YFPFY])
    ssm = np.max([0, YFPFY])
    ssr = np.max([0, YFPFY + Nw*my**2 - 2*my*wF@P@FY])
    v1o = n2
    v2 = N - n2
    Fo = ssr/sse*v2/v1o
    ...
    if FB == 'FS':
        R2prt = R2 - R2_1
        v1p = np.abs(n2 - n1)
        t2ss = np.max([0, ssm - ssm_1])
        Fp = t2ss/sse*v2/v1p
        p_Fp = pvalF(Fp, v1p, v2, 'ot')
    elif FB == 'BE':
        R2prt = R2_1 - R2
        v1p = np.abs(n2 - n1)
        t2ss = np.max([0, ssm_1 - ssm])
    ...
    return st
###################################################################################
def swlinr(x=None, y=None, w=None, par=None): 
    N = x.shape[0]
    if par['pm0'] == 1:
    ...
    # par['iterate'] = 1
    st0 = stats(x, y, w)
    model, par = firstmdl(st0, par)
    par['iterate'] = 1
    while par['iterate'] > 0:
        model, st0, par = bElimin(model, st0, par)
        model, st0, par = fSelect(model, st0, par)
        visualz(model, par)
    return model
###################################################################################
def visualz(model=None, par=None):
    mode = par['dsp']
    if mode == 'no': return
    ...
    for step in np.arange(len(model0) - 2 + i1, len(model0) - i2):
        model = model0[step]
        print('Step: ', step, '  =================================================================================================================')
        if model['FB'] == 'FS' and step != 0: print('Added factor: ', model['xio'])
        elif model['FB']=='BE' and step != 0: print('Removed factor: ', model['xio'])
        elif par['mtp'] == 'init':            print('Initial model: ', model['cname_i'].flatten())
        elif model['FB']=='FS' and step == 0: print('Initial model: intercept')
        elif model['FB']=='BE' and step == 0: print('Initial model: full')
        elif model['FB']=='BE' and step == 0: print('Initial model: full')
        
        if 'Fo' in model['st']['ovr'] and mode == 'all' or mode == 'ovr':
            ...
            print('------------------------------------------------------')
            print(' ')    
        if 'st' in model and 'in' in model['st'] and mode == 'all':
            print('--- Partial model measures ---')
            Variable = model['cname_i']
            Estimates = model['pm']
            st = model['st']['in']
            if st is not None:
                n = len(st)
            ...
            print('------------------------------------------------------')
            print(' ')    
        if 'st' in model and 'out' in model['st'] and mode == 'all':
            print('--- Measures of eligible to enter variables ---')
            Variable = model['cname_o']
            st = model['st']['out']
            n = len(st)
            T2SS = nans((n, 1))
            PartialF = nans((n, 1))
