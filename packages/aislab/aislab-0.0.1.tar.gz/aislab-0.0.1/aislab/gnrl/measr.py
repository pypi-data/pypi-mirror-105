...
###################################################################################
def gamln(a):
    cof = c_(np.array([76.18009172947146, -86.50532032941677, 24.01409824083091, -1.231739572450155, 0.1208650973866179e-2, 0.5395239384953e-5]))
...
def gamq(a, x):
    ...
    for i in range(0, len(gq.flatten())):
        ...
        if x[i] < a[i] + 1: gq[i] = 1 - gser(a[i], x[i])
        else:               gq[i] = gcf(a[i], x[i])
    return gq
###################################################################################
def gbi(g, b):
    ...
    ind = gbo >= GBO;  gbi[ind] = gbo[ind]/GBO[ind]*100
    gbo[gbo < 1e-12] = 1e-12
    ind = gbo < GBO;   gbi[ind] = -GBO[ind]/gbo[ind]*100
    gbi = gbi.reshape(m, n)
...
def gser(a, x):
    ...
    gln = gamln(a)
    gamser = 0
    ap = a;  inva = 1/a;  s = inva
    for n in range(0, maxIter):
        ap = ap + 1;  inva = inva*x/ap;  s = s + inva
...
def pvalF(F, v1, v2, tp='ot'):
    ...
    F = c_(F)
    if v2 <= 0 or v2 <= 0: print('ERROR: SpecialFunctions, pvalF():', 'Degrees of freedom must be positive...'); return []
    if (F < 0).any():      print('ERROR: SpecialFunctions, pvalF():', 'x must be non negative...'); return []
    if tp == 'ot': p = betai(v2/2, v1/2, v2/(v2 + F*v1))
    else:          p = 2*betai(v2/2, v1/2, v2/(v2 + F*v1))
