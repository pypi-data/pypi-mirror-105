...
##############################################################################
def c_(x): 
    if isinstance(x, (int, float, str)): x = np.array([x])
...
def find(x, n=None, pos=None):
    if len(x.shape) == 1:
        ...
        if n is None:
            ...
            if pos is None: res = ind[:n+1]
            elif pos == 'first': res = ind[:n+1]
...
def min1(x, axis=0, naskip=False):
    ...
    if naskip: x[np.isnan(x)] = max(x) + 1
    xmin = np.amin(x, axis=axis)
    ind = np.where(x == xmin)[0][0]
    return xmin, ind
##############################################################################
def nans(size):
...
##############################################################################
def r_(x): return np.reshape(x, (1, len(x)))
##############################################################################    
