...
class Widget(object):
    ...
    def __init__(self,default_con_file=""):

        if len(default_con_file) == 0:
            return 

        if os.path.exists(default_con_file):
            with open(default_con_file,'r') as filer:
                self.default_con = json.load(filer)
    ...
    def create_text(self, text_id_name, x_, y_, width_, height_):
        ...
        if text_id_name in self.default_con:
            self.set_text(text_id_name,self.default_con[text_id_name])
    ...
    def add_comboxlist(self, comboxlist_id_name, item,
        ...
        if comboxlist_id_name in self.default_con:
            select_item = int(self.default_con[comboxlist_id_name])
            if select_item <0 or select_item > len(item):
                raise Exception("default select_item out of range")
    ...
    def create_checkbox(self, text_, checkbox_id_name,
        ...
        selected = 0
        if checkbox_id_name in self.default_con:
            if self.default_con[checkbox_id_name] == "True":
