...
class OpenSprinkler(object):
    ...
    def getData(self):
        ...
        if not response:
            raise Exception('Could not contact OpenSprinkler host')
    ...
    def parse_api_data(self,response):
        ...
        utcTimeLastBoot = datetime.fromtimestamp(settings['lupt'], utc_tz)
        lastBootTime = utcTimeLastBoot.strftime("%d/%m %H:%M")
        lastBootReason = settings['lrbtc']


        p_status = settings['ps']
        rain_sensor = settings['sn1']
        sensor2 = settings['sn2']
        ...
        my_dict['lastBootTime'] = lastBootTime
        my_dict['lastBootReason'] = lastBootReason

        my_dict['rain_sensor'] = rain_sensor
        my_dict['sensor2'] = sensor2
