...
class TextProcessor(object):
    ...
    def __init__(self, sentence_re=None, clause_re=None):
        if clause_re is None:
            clause_re = self.CLAUSE_DELIMITERS
        if sentence_re is None:
            sentence_re = self.SENTENCE_DELIMITERS
        self.clause_regex_punctuation = re.compile(f"([{''.join(map(re.escape, clause_re))}]+)")
        ...
        self.sentence_regex_punctuation = re.compile(f"([{''.join(map(re.escape, sentence_re))}]+)")
        self.sentence_regex = re.compile('|'.join(map(re.escape, self.SENTENCE_DELIMITERS)))

    @staticmethod
    def split_text(text_re, my_text):
        sentences = []
        temp = ''
        for i, sentence in enumerate(re.split(text_re, my_text)):
    ...
    def split_clauses(self, text, punctuation=0):
        if not punctuation:
            clauses = self.sentence_regex.split(text)
            # 只保留非空的句子
            clauses = [s for s in clauses if s != '']
        else:
            clauses = self.split_text(self.clause_regex_punctuation, text)
        return clauses

    def split_sentences(self, text, punctuation=0):
        '''
        这样分句保留了标点符号,并且对分句的句子做了strip的字符过滤
        '''
        if not punctuation:
            ...
            sentences = self.split_text(self.sentence_regex_punctuation, text)
        return sentences
...
class TokenMatcher(object):
    ...
    def __init__(self, text, clause_delimiter=None, sentence_delimiter=None, tokens=None):
        self.text = text
...
if __name__ == '__main__':
    TEXT_PROCESSOR = TextProcessor(sentence_re=[".", "?", "!", ";", "？", "！", "：", "。", "；", '\n'])

    text = '''
    \n妊娠糖尿病5.11日忙碌的产检日 到现在才有空来记录今天！累！乱！忙！\n昨晚房间混进一只蚊子，整夜围着我咬，又不能喷药[捂脸R][捂脸R][捂脸R]困得找不到，打不死[哭惹R]！终于熬到五点上了个洗手间，头晕眼花�，心情烦躁，睡不着�好抓狂�啊！翻来翻去，不知道用了多长时间感觉像是睡着了！昏昏沉沉的！在醒来一看手机，快要九点了�，今天产检啊���，迅速叫醒老公和仔，洗漱，随便带了点吃的就出门了！\n到医院，先去找医生给加号，上次产检完忘记让医生帮我约好这次的，加好立马冲去抽血和验尿！但是。。。但是。。这次抽血要空腹�，完蛋，我完全忘记，已经在车上吃了东西�，问了一下验尿不需要空腹，还好，至少能做一个检查！搞定这个，时间快要到b超时间了，正往b超区赶，医院电话来了，问我是否来产检，立即告知已经在往b超区走了，走的太急，感觉心跳好快，就差小跑了[笑哭R]终于顺利签到做完了！带着报告单去产科签到，然后候诊，屁股还没坐热，尿检报告出来了，又下楼去拿报告单[笑哭R]，真是楼上楼下跑了好几趟，感觉自己是今日最忙的孕妇�了[笑哭R]\n例行产检：医生看了报告说都挺好，宝宝也没有因为控糖偏小，说抽血没做成就两周后产检那天在做，不用明天再跑一趟了，还好还好��\n到家已经一点多了，简单做了点吃的，散步完开始洗澡，然后房间里面放药灭蚊，带仔仔出门玩儿[捂脸R]其实很累，超级想躺着，但是小伙伴约玩儿还是出去了[笑哭R]\n分割线！！！！！！！！！！！！！！！！！！！！！\n今日空腹：没测，起晚了着急出门\n✡早餐：饼干➕牛奶\n到医院产检\n加餐：饼干➕牛奶�\n✡午餐：西葫芦洋葱炒西红柿，炒鸡蛋一份，半根玉米，一片黑麦面包�\n餐2：5.6☑\n下午加餐：牛奶�➕一片黑麦面包�\n✡晚餐：西芹蘑菇炒肉➕炒苦瓜➕玉米一根（来不及煮饭就蒸了玉米�）➕烤肠一根\n餐2：6.2☑，烤肠不应该吃的，太油估计，运动的也少，今天累，走不动！\n  洗洗睡觉���\n    \n
    '''
    rs = TEXT_PROCESSOR.split_sentences(text=text, punctuation=1)
