...
from rapyuta_io.utils.constants import NON_PLAN_ATTRIBUTES
from rapyuta_io.utils.utils import is_empty, remove_attributes
...
class Plan(ObjDict):
    ...
    :ivar singleton: Boolean representing whether the plan is singelton or not.
    :ivar inboundROSInterfaces: Dictionary containing inbound ROS interfaces information.
    :ivar dependentDeployments: List of other dependent deployments.
    :ivar components: Dictionary containing components such as executables.
    :ivar internalComponents: Dictionary containing internal components information.
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'planId' in self:
            ...
            self.planId = self.id
        remove_attributes(self, NON_PLAN_ATTRIBUTES)
