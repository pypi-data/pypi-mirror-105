...
class Optimization(LCA_matrix):
    ...
    def worker(optObject, bnds, x0, iteration):
        ...
        if res.success and False:
            x0_ = deepcopy(res.x)
            x0_[x0_ > 0.95] = 1
            x0_[x0_ < 0.05] = 0
            if not np.all(x0_ == res.x):
                res_ = minimize(optObject._objective_function, x0_, method='SLSQP', bounds=bnds, constraints=optObject.cons)
                if not res_.success:
                    print(f"Iteration: {iteration} Revised x0 results in failed.")
                elif res_.fun < res.fun:
                    print(f"Iteration: {iteration} Results improved!")
                    res = res_
                elif res_.fun == res.fun:
                    print(f"Iteration: {iteration} Results didn't changed!")
                    res = res_
                else:
                    print(f"Iteration: {iteration} Results got worsed!")
