import argparse
import os
from certora_cli.certoraUtils import debug_print_, get_version, sanitize_path, check_results, \
...
def check_results_from_web(output_URL: str, max_attempts: int, delay_between_attempts_seconds: int) -> bool:
    ...
    if actual is None:
        ...
        return False
    return check_results(cast(dict, actual.json()))
...
def save_features_json_from_web(output_URL: str, max_attempts: int, delay_between_attempts_seconds: int) -> None:
    ...
    with open("featuresReport.json", 'w') as featuresReportFile:
        featuresReportFile.write(features_json_content.text)
...
class CloudVerification:
    ...
    def __set_url(self, url_attr: str, index: str, user_id: int, current_job_anonymous_key: str,
                  requested_resource: str = None, debug: bool = False) -> None:
        """
    ...
    def cli_verify(self, args: argparse.Namespace, arg_str: str, debug: bool = False) -> bool:
        ...
        if args.coinbaseMode:
            self.set_requested_resource_url(userId, 'featuresResults', 'featuresResults.json', anonymousKey, debug)
