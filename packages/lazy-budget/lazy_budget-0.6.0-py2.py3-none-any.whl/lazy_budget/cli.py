...
from lazy_budget.charts import burndown_chart
...
def show_burndown_chart(args):
    budget = _get_budget(args)
    ops = _get_history(args)
    burndown_chart(budget, ops, show_zero=args.show_zero)
...
class CLIConfig:
    ...
    def get_args(self, argv=None):
        ...
        parser_categories = subparsers.add_parser(
            "categories", aliases=["cats"], help="display spendings by category"
        )
