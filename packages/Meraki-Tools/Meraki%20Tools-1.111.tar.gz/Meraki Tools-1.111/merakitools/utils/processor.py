...
def _config_tags(device,sdk):
    switch_port_cfg = []
    tags = {"AP": [16,17,18,19,20,21,22,23,24],"Admin": [1,2,3,4,5,6], "Classroom": [7,8,9,10], "IOT":[11,12,13,14,15]}
    for tag in tags:
                    ...
                    if port == switch_port['portId']:
                        ...
                        if not await lib.compare(approved_port_config,
                            ...
                            switch_port_cfg.append(sdk.batch.switch.updateDeviceSwitchPort(device,
                                                                          switch_port['portId'],
                                                                          **model.config_manager[golden_tag].approve_golden_config[tag]))
                            print(f"{lib.bc.WARNING} Updating-{lib.bc.OKBLUE}Orginization: {model.meraki_nets[org_id].name} "
...
async def check_last_change(change, dashboard_change):
    if change != dashboard_change or change is not None:
        return True
    else:
        return False
