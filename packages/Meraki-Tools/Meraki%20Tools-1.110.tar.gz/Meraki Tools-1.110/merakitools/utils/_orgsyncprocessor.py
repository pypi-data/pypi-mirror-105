...
class Orgsyncprocessor(threading.Thread):
    ...
    def run(self):
        ...
        if const.appcfg.use_cache:
            lib.load_cache(self.org_id, model.meraki_nets[self.org_id], 'autosync')
        has_golden = False
        remove_list = []
        for net in nets:
            ...
            elif const.appcfg.tag_target in net['tags'] and net['id'] not in model.meraki_nets[self.org_id].networks.keys():
                model.meraki_nets[self.org_id].networks.update(
                        {net['id']: model.MNET(net)})
            found = False
            for net_id in model.meraki_nets[self.org_id].networks.keys():
                if (net_id == net['id'] and const.appcfg.tag_target in net['tags']) or const.appcfg.tag_override:
                    found = True
                    continue
            if not found and net['id'] in model.meraki_nets[self.org_id].networks.keys():
                    remove_list.append(net['id'])
        for nid in remove_list:
            model.meraki_nets[self.org_id].networks.pop(nid)
