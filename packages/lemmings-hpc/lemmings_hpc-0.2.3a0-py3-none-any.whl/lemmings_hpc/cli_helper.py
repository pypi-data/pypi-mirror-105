...
def get_workflow_py(workflow):
    ...
    *Get the path of the {workflow}.py file.*
    ...
    if catch is False:
        wf_py_path = Path(resource_filename('lemmings', "")) / "chain" / "workflows" /  str(workflow + ".py") #pylint: disable=line-too-long
        if Path(wf_py_path).is_file():
            catch = True
        else:
...
def find_all_wf():
    ...
    *Find all available workflows.*
    ...
    path_folder_wf = Path(resource_filename('lemmings', "")) / "chain" / "workflows"
