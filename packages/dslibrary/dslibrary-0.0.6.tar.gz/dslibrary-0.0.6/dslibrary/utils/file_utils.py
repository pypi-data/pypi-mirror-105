...
def connect_to_filesystem(uri: str, for_write: bool=False, **kwargs):
    ...
    :param uri:             A partial URI like "s3://bucket".
    :param for_write:       Whether to allow write access.
    :param kwargs:          Additional arguments, namely credentials.
    :return:        An open() method that lets you access files in the remote filesystem, for read or write.
    """
    # uri can just be a local path
    if "://" not in uri:
        uri = os.path.abspath(uri)
        return lambda filename, mode: open(os.path.join(uri, filename.strip("/")), mode)
    protocol, bucket = uri.strip("/").split("://", maxsplit=1)
    if "/" in bucket or ":" in bucket:
        raise ValueError("Invalid URI for external filesystem")

    def opener(filename, mode):
        if "w" in mode and not for_write:
            raise PermissionError("write not allowed on this filesystem")
        uri = f"{protocol}://{bucket}/{filename.strip('/')}"
        return FileOpener().open(uri, mode, **kwargs)
