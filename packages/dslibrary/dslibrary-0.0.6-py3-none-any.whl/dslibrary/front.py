...
class DSLibrary(object):
    ...
    def _xlt_resource(self, resource_name, _mode="inputs", **kwargs):
        """
    ...
    def open_resource(self, resource_name: str, mode: str='rb', **kwargs) -> io.RawIOBase:
        ...
        proto_handler = load_protocol_handler(uri)
        if proto_handler:
    ...
    def _get_system_connection(self, system_type, resource_name: str, for_write: bool=False, **kwargs):
        ...
        handler = load_protocol_handler(uri)
        if handler:
        ...
        # TODO implement timeseries connection
    ...
    def get_filesystem_connection(self, resource_name: str, for_write: bool=False, **kwargs):
        """
        Returns an 'open(filename, mode)' method for a given external filesystem, like s3, abs, gcs, etc..
    ...
    def get_timeseries_db_connection(self, resource_name: str, for_write: bool=False, **kwargs):
        ...
        :return:    A connection, as a yet-to-be-determined interface that supports a range of popular time series
                    databases.
        """
        return self._get_system_connection("timeseries", resource_name, for_write, **kwargs)
    ...
    def _write_content(self, opener, resource_name: str, content, append: bool=False, **kwargs):
        ...
        # FIXME use the URL extension splitter!
    ...
    def load_dataframe(self, resource_name: str, **kwargs):
        ...
        if open_args.get("nosql_collection"):
            collection = open_args["nosql_collection"]
        ...
        # FIXME use the URL splitter, not this one!
        file_extension = os.path.splitext(uri)[1].strip(".")
        if not file_extension:
