...
class DatabricksBundle(Bundle):
    ...
    def boot(self, container: ContainerInterface):
        ...
        if (
            ...
            spark = container.get("databricksbundle.spark")

            if spark.conf.get("spark.databricks.workspace.multipleResults.enabled") == "false":
                logger.warning("spark.databricks.workspace.multipleResults.enabled is set to false!")
                logger.warning("Logs and displayed DataFrames might not show properly!")
