...
class OMFITkeqdsk(OMFITnamelist):
    ...
    def compare_omas_constraints(self, ods, time_index=None, time=None):
        """
        ...
        for k, (check, case) in enumerate(checks.items()):
            try:
                odata = ods[case['omas']]
                n = len(odata)
                ...
                ax.hist((odata - kdata) / abs(kdata) * 100, np.linspace(-15, 15, 31))
                ax.set_aspect(1.0 / ax.get_data_ratio(), adjustable='box')
                ax.set_xlabel('% relative error')
            except Exception as _excp:
                print(f"error in comparison of {check}: " + repr(_excp))
