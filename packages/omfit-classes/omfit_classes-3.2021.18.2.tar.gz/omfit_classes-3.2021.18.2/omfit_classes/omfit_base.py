...
class OMFITtree(SortedDict):
                    ...
                    if only is None or any(entryID.startswith(k) for k in tolist(only)):
                        ...
                        if isinstance(me[kid], _OMFITnoSave):
                            ...
                            if not saved and isinstance(
                                ...
                                if os.environ.get('USER', '') in ['meneghini', 'smithsp', 'eldond']:  # Show all objects
                                    pickled_objects.append(entryID)
                                elif entryID != "['__GUISAVE__']":  # Don't warn about __GUISAVE__
                                    pickled_objects.append(entryID)
                                try:
        ...
        try:
            with open(file, 'w') as f:
                ...
                if pickled_objects:
                    ...
                    print('The following objects were pickled:')
                    pprint(pickled_objects)
    ...
    def _load(
        ...
        quiet=False,
        developerMode=False,
        lazyLoad=bool(ast.literal_eval(os.environ.get('OMFIT_LAZYLOAD', 'False'))),
    ):
        global OMFITexpressionsReturnNone
        tmpExp = OMFITexpressionsReturnNone
        OMFITexpressionsReturnNone = True
