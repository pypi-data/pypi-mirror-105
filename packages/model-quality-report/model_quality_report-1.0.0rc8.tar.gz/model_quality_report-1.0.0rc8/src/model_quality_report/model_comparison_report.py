...
class ModelComparisonReport:
    ...
    def get_true_and_predicted_data(self, reports: ModelComparisonReportType) -> pd.DataFrame:
        ...
        results = list()
        for quality_report_instance, quality_report in zip(self._quality_reports, reports):
            report_df = quality_report_instance.get_true_and_predicted_data(report=quality_report[1]).assign(
                **quality_report[0]
            )
            results.append(report_df)
        return pd.concat(results)
