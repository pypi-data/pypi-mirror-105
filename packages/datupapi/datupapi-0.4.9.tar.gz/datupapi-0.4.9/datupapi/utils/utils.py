...
class Utils(Config):
    ...
    def delete_datalake_objects(self, datalake=self.datalake, datalake_path=None):
        """
        Return the list of deleted objects in the specified datalake and path

        :param datalake: Datalake's name to delete the stored objects. Default datalake in config.yml
        ...
        >>> response = list_datalake_objects(datalake='mydatalake', datalake_path='path/to/objects')
        >>> response = {}
        """
        ...
        try:
            list_objects = client.list_objects_v2(Bucket=datalake, Prefix=datalake_path)
            if objects['KeyCount'] is not 0:
                for object in objects['Contents']:
                    response = client.delete_objects(Bucket='cnch-datalake-datup',
