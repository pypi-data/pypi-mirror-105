...
class Results:
    ...
    Attributes
    ----------
    time : :obj:`numpy.ndarray` of :obj:`numpy.float32` (time_index)
    ...
    state : :obj:`numpy.ndarray` of :obj:`numpy.complex64` (time_index, magnetic_quantum_number)
    ...
    spin : :obj:`numpy.ndarray` of :obj:`numpy.float32` (time_index, spatial_direction)
    ...
    spin_calculator : :obj:`callable`
        ...
        * **state** (:obj:`numpy.ndarray` of :obj:`numpy.complex64` (time_index, magnetic_quantum_number)) - The quantum state of the spin system over time, written in terms of the eigenstates of the spin projection operator in the z direction.

        Returns:
        
        * **spin** (:obj:`numpy.ndarray` of :obj:`numpy.float32` (time_index, spatial_direction)) - The expected spin projection (Bloch vector) over time.
    """
...
class Simulator:
    ...
    get_time_evolution_raw : :obj:`callable`
        ...
        * **time_coarse**   (:obj:`numpy.ndarray` of :obj:`numpy.float32` (time_index)) - The times that `state` was evaluated at.
        * **time_end_points** (:obj:`numpy.ndarray` of :obj:`numpy.float32` (start/end)) - The time offset that the experiment is to start at, and the time that the experiment is to finish at. Measured in s.
    ...
    def compile_time_evolver(self, get_field, spin_quantum_number, device, use_rotating_frame = True, integration_method = IntegrationMethod.MAGNUS_CF4, exponentiation_method = None, trotter_cutoff = 28, threads_per_block = 64, max_registers = 63):
        ...
        @jit_device_template("(float32[:], complex64[:, :], complex64[:, :])")
        def append_exponentiation(field_sample, time_evolution_fine, time_evolution_coarse):
            if device_index == 0:
                time_evolution_old = np.empty((dimension, dimension), dtype = np.complex64)
            elif device_index == 1:
                time_evolution_old = cuda.local.array((dimension, dimension), dtype = np.complex64)
            elif device_index == 2:
                time_evolution_old_group = roc.shared.array((threads_per_block, dimension, dimension), dtype = np.complex64)
        ...
        if use_rotating_frame:
            if dimension == 3:
                @jit_device_template("(float32[:], float32, complex64)")
        ...
        if integration_method == IntegrationMethod.MAGNUS_CF4:
            @jit_device_template("(float32, float32, float32, float32, float32[:, :], float32, complex64[:])")
            ...
            @jit_device_template("(complex64[:, :], complex64[:, :], float32[:, :], float32, float32, complex64[:])")
        ...
        @jit_device_template("(int64, float32[:], float32, float32, float32[:], complex64[:, :, :], float32)")
        def get_time_evolution_loop(time_index, time_coarse, time_step_output, time_step_integration, time_end_points, time_evolution_coarse, sweep_parameter):
            # Declare variables
            if device_index == 0:
                time_evolution_fine = np.empty((dimension, dimension), dtype = np.complex64)

                field_sample = np.empty((sample_index_max, lie_dimension), dtype = np.float32)
                rotating_wave_winding = np.empty(sample_index_end, dtype = np.complex64)
            elif device_index == 1:
                time_evolution_fine = cuda.local.array((dimension, dimension), dtype = np.complex64)

                field_sample = cuda.local.array((sample_index_max, lie_dimension), dtype = np.float32)
                rotating_wave_winding = cuda.local.array(sample_index_end, dtype = np.complex64)
            elif device_index == 2:
                time_evolution_fine_group = roc.shared.array((threads_per_block, dimension, dimension), dtype = np.complex64)
                time_evolution_fine = time_evolution_fine_group[roc.get_local_id(1), :, :]

                field_sample_group = roc.shared.array((threads_per_block, sample_index_max, lie_dimension), dtype = np.float32)
                field_sample = field_sample_group[roc.get_local_id(1), :, :]
                rotating_wave_winding_group = roc.shared.array((threads_per_block, sample_index_end), dtype = np.complex64)
        ...
        @jit_host("(float32, float32[:], float32[:], float32, float32, complex64[:, :, :])", max_registers)
        def get_time_evolution(sweep_parameter, time_coarse, time_end_points, time_step_integration, time_step_output, time_evolution_coarse):
            ...
            time_coarse : :class:`numpy.ndarray` of :class:`numpy.float32` (time_index)
            ...
            time_end_points : :class:`numpy.ndarray` of :class:`numpy.float32` (start time (0) or end time (1))
            ...
            time_evolution_coarse : :class:`numpy.ndarray` of :class:`numpy.complex64` (time_index, bra_state_index, ket_state_index)
        ...
        @jit_host("(complex64[:, :], float32[:, :])", max_registers = max_registers)
        def get_spin(state, spin):
            ...
            Parameters
            ----------
            state : :class:`numpy.ndarray` of :class:`numpy.complex64` (time_index, state_index)
            ...
            spin : :class:`numpy.ndarray` of :class:`numpy.float32` (time_index, spatial_index)
        ...
        def spin_calculator(state):
            ...
            """
            if device.index == 0:
                spin = np.empty((state.shape[0], 3), np.float32)
                get_spin(state, spin)
            elif device == Device.CUDA:
                spin = cuda.device_array((state.shape[0], 3), np.float32)
                ...
                spin = roc.device_array((state.shape[0], 3), np.float32)
                blocks_per_grid = (state.shape[0] + (threads_per_block - 1)) // threads_per_block
    ...
    def evaluate(self, sweep_parameter, time_start, time_end, time_step_integration, time_step_output, state_init):
        ...
        state_init : :obj:`numpy.ndarray` of :obj:`numpy.complex64` (magnetic_quantum_number)
        ...
        time_end_points = np.asarray([time_start, time_end], np.float32)
        state_init = np.asarray(state_init, np.complex64)

        time_index_max = int((time_end_points[1] - time_end_points[0])/time_step_output)
        if self.device.index == 0:
            time = np.empty(time_index_max, np.float32)
            time_evolution_coarse = np.empty((time_index_max, self.spin_quantum_number.dimension, self.spin_quantum_number.dimension), np.complex64)
            ...
            time = cuda.device_array(time_index_max, np.float32)
            time_evolution_coarse = cuda.device_array((time_index_max, self.spin_quantum_number.dimension, self.spin_quantum_number.dimension), np.complex64)
            ...
            time = roc.device_array(time_index_max, np.float32)
            time_evolution_coarse = roc.device_array((time_index_max, self.spin_quantum_number.dimension, self.spin_quantum_number.dimension), np.complex64)
        ...
        state = np.empty((time_index_max, self.spin_quantum_number.dimension), np.complex64)
    ...
    def get_state(state_init, state, time_evolution):
        ...
        Parameters
        ----------
        state_init : :class:`numpy.ndarray` of :class:`numpy.complex64`
        ...
        time_evolution : :class:`numpy.ndarray` of :class:`numpy.complex64` (time_index, bra_state_index, ket_state_index)
            The evaluated time evolution operator between each time step. See :ref:`architecture` for some information.
        """
...
sqrt2 = math.sqrt(2)
sqrt3 = math.sqrt(3)
machine_epsilon = np.finfo(np.float32).eps*1000
...
class Utilities:
    ...
    conj(z) : :obj:`callable`
        ...
        * **z** (:class:`numpy.complex64`) - The complex number to take the conjugate of.
        
        Returns

        * **cz** (:class:`numpy.complex64`) - The conjugate of z.
    ...
    complex_abs(z) : :obj:`callable`
        ...
        * **z** (:class:`numpy.complex64`) - The complex number to take the absolute value of.
        
        Returns

        * **az** (:class:`numpy.float32`) - The absolute value of z.
    ...
    norm2(z) : :obj:`callable`
        ...
        * **z** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (index)) - The vector to take the 2 norm of.

        Returns
        
        * **nz** (:class:`numpy.float32`) - The 2 norm of z.
    ...
    inner(left, right) : :obj:`callable`
        ...
        * **left** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (index)) - The vector to left multiply in the inner product.
        * **right** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (index)) - The vector to right multiply in the inner product.
        
        Returns
        
        * **d** (:class:`numpy.complex64`) - The inner product of l and r.
    ...
    set_to(operator, result) : :obj:`callable`
        ...
        * **operator** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix to copy from.
        * **result** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix to copy to.
    ...
    set_to_one(operator) : :obj:`callable`
        ...
        * **operator** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix to set to :math:`1`.
    ...
    set_to_zero(operator) : :obj:`callable`
        ...
        * **operator** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix to set to :math:`0`.
    ...
    matrix_multiply(left, right, result) : :obj:`callable`
        ...
        * **left** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix to left multiply by.
        * **right** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix to right multiply by.
        * **result** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - A matrix to be filled with the result of the product.
    ...
    adjoint(operator) : :obj:`callable`
        ...
        * **operator** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The operator to take the adjoint of.
        * **result** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - An array to write the resultant adjoint to.
    ...
    matrix_exponential_analytic(field_sample, result) : :obj:`callable`
        ...
        * **field_sample** (:class:`numpy.ndarray` of :class:`numpy.float32`, (y_index, x_index)) - The values of x, y and z respectively, as described above.
        * **result** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix which the result of the exponentiation is to be written to.
    ...
    matrix_exponential_lie_trotter(field_sample, result) : :obj:`callable`
        ...
        * **field_sample** (:class:`numpy.ndarray` of :class:`numpy.float32`, (y_index, x_index)) - The values of x, y and z (and q for spin one) respectively, as described above.
        * **result** (:class:`numpy.ndarray` of :class:`numpy.complex64`, (y_index, x_index)) - The matrix which the result of the exponentiation is to be written to.
    ...
    def __init__(self, spin_quantum_number, device, threads_per_block):
        ...
        if spin_quantum_number == SpinQuantumNumber.HALF:
            ...
            def matrix_exponential_lie_trotter(field_sample, result, trotter_cutoff):
                ...
                if device_index == 0:
                    temporary = np.empty((2, 2), dtype = np.complex64)
                elif device_index == 1:
                    temporary = cuda.local.array((2, 2), dtype = np.complex64)
                elif device_index == 2:
                    temporary_group = roc.shared.array((threads_per_block, 2, 2), dtype = np.complex64)
            ...
            def matrix_exponential_lie_trotter(field_sample, result, trotter_cutoff):
                ...
                if device_index == 0:
                    temporary = np.empty((3, 3), dtype = np.complex64)
                elif device_index == 1:
                    temporary = cuda.local.array((3, 3), dtype = np.complex64)
                elif device_index == 2:
                    temporary_group = roc.shared.array((threads_per_block, 3, 3), dtype = np.complex64)
                    temporary = temporary_group[roc.get_local_id(1), :, :]
                for power_index in range(hyper_cube_amount):
                    matrix_multiply(result, result, temporary)
                    matrix_multiply(temporary, temporary, result)
