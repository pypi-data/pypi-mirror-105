...
def entmax15(inputs, axis=-1):
    ...
    def _entmax_inner(inputs):
        with tf.name_scope('entmax'):
            ...
            inputs -= tf.reduce_max(
                inputs, axis, keepdims=True)  # subtract max for stability
...
def sparse_entmax15_loss_with_logits(labels, logits):
    ...
    with tf.name_scope('entmax_loss'):
        ...
        omega_entmax15 = (
            1 - (tf.reduce_sum(p_star * tf.sqrt(p_star), axis=-1))) / 0.75
        p_incr = p_star - tf.one_hot(
            labels, depth=tf.shape(logits)[-1], axis=-1)
...
def top_k_over_axis(inputs, k, axis=-1, **kwargs):
    ...
    with tf.name_scope('top_k_along_axis'):
        ...
        input_perm_sorted, sort_indices_perm = tf.nn.top_k(input_perm,
                                                           k=k,
                                                           **kwargs)
...
def gather_over_axis(values, indices, gather_axis):
    ...
    for axis_i in range(ndims):
        if axis_i == gather_axis:
            ...
            index_i = tf.range(tf.cast(shape[axis_i], dtype=indices.dtype),
                               dtype=indices.dtype)
            index_i = tf.reshape(
                index_i, [-1 if i == axis_i else 1 for i in range(ndims)])
            index_i = tf.tile(
                index_i,
                [shape[i] if i != axis_i else 1 for i in range(ndims)])
...
def entmax_threshold_and_support(inputs, axis=-1):
    ...
    with tf.name_scope('entmax_threshold_and_support'):
        ...
        inputs_sorted, _ = top_k_over_axis(inputs,
                                           k=num_outcomes,
                                           axis=axis,
                                           sorted=True)
        ...
        support_size = tf.reduce_sum(tf.cast(tf.less_equal(tau, inputs_sorted),
                                             dtype=tf.int64),
                                     axis=axis,
                                     keepdims=True)
