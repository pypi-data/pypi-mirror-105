...
try:
    ...
    wr("Getting Connection With Redis Database")
    time.sleep(6)
...
try:
    ...
    ok = []
    wr("Can't connect to Redis Database.... Restarting....")
    for x in range(1, 6):
        redis_info = Var.REDIS_URI.split(":")
        udB = StrictRedis(
            host=redis_info[0],
        ...
        try:
            ...
            wr(f"Connection Failed ...  Trying To Reconnect {x}/5 ..")
    if not ok:
        wr("Redis Connection Failed.....")
        exit()
