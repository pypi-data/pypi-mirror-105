...
async def autobot():
    if Var.BOT_TOKEN and not udB.get("BOT_TOKEN"):
        udB.set("BOT_TOKEN", str(Var.BOT_TOKEN))
        return
    async with ultroid_bot.conversation("botfather") as conv:
        await conv.send_message("/newbot")
        await asyncio.sleep(1)
        await conv.get_response()
        await conv.send_message("ULTROID ASSISTANT")
        await asyncio.sleep(0.8)
        await conv.send_message(f"ultroid{ultroid_bot.uid}bot")
        res = await conv.get_response()
        token = res.message.split(":\n")[1].split("\n")[0]
        udB.set("BOT_TOKEN", str(token))  # Extract Bot token and set
...
async def autopilot():
    ...
    await ultroid_bot(
        EditAdminRequest(chat_id, asst.me.username, newrights, "Assistant Admin")
    )
    pfpa = await ultroid_bot.download_profile_photo(udB.get("LOG_CHANNEL"))
    if not pfpa:
        ...
        await ultroid_bot(
            EditPhotoRequest(udB.get("LOG_CHANNEL"), InputChatUploadedPhoto(ll))
        )
