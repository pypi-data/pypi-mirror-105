...
class CloudPodManager(klFOK):
 BACKEND='_none_'
 def __init__(self,pod_name=klFOc,config=klFOc):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise klFOE('Not implemented')
 def pull(self):
  raise klFOE('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert klFOp(statuses)==klFOp(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise klFOE('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @klFOY
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise klFOE('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==klFOp([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=klFOA)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise klFOE('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=klFOA)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=klFOo(self,'clone_dir',klFOc)
...
class PodConfigManagerMeta(klFOD):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=klFOc
   for manager in cls.CHAIN:
    try:
     tmp=klFOo(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif klFOP(tmp,klFOt)and klFOP(result,klFOt):
       result.extend(tmp)
    except klFOE:
     pass
   if result is not klFOc:
    return result
   raise klFOE('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(klFOK,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @klFOY
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise klFOE('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(klFOK):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=klFOA).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=klFOp([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=klFOa):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except klFOE:
   if safe:
    return{}
   raise klFOE('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(klFOK):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise klFOE('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise klFOE('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise klFOE('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
