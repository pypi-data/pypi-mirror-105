...
class CloudPodManager(wXRQA):
 BACKEND='_none_'
 def __init__(self,pod_name=wXRQq,config=wXRQq):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise wXRQo('Not implemented')
 def pull(self):
  raise wXRQo('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert wXRQa(statuses)==wXRQa(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise wXRQo('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @wXRQF
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise wXRQo('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==wXRQa([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=wXRQE)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise wXRQo('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=wXRQE)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=wXRQm(self,'clone_dir',wXRQq)
...
class PodConfigManagerMeta(wXRQx):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=wXRQq
   for manager in cls.CHAIN:
    try:
     tmp=wXRQm(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif wXRQn(tmp,wXRQK)and wXRQn(result,wXRQK):
       result.extend(tmp)
    except wXRQo:
     pass
   if result is not wXRQq:
    return result
   raise wXRQo('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(wXRQA,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @wXRQF
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise wXRQo('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(wXRQA):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=wXRQE).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=wXRQa([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=wXRQb):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except wXRQo:
   if safe:
    return{}
   raise wXRQo('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(wXRQA):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise wXRQo('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise wXRQo('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise wXRQo('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
