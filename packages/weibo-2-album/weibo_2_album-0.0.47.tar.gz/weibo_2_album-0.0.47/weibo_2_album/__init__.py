...
def getAdditionalInfo(json):
	...
	if getUserInfo(user_info):
		result += 'user: %s ' % getUserInfo(user_info)
	retweeted_user_info = json.get('retweeted_status', {}).get('user', {})
	if getUserInfo(retweeted_user_info):
		result += 'retweeted_from: %s ' % getUserInfo(retweeted_user_info)
	video = getVideo(video)
	if video: 
		result += 'video: %s ' % video
	imgs = [x['url'] for x in json.get('pics', [])]
	if imgs:
