...
def to_str(var: any,
    ...
    if isinstance(var, torch.Tensor):
        ...
        if fp is not None and fp >= 0 and (var.dtype == torch.float32 or var.dtype == torch.float64):
            if len(var.shape) == 1:  # hack for 1d - nicer print
                f_format = '{:,.%xf}' % fp
...
def model_info(model: nn.Module, title: str = 'model', wv: bool = False, chars: int = 2, tabs: int = 1):
    """
    ...
    var_meta = '{}\t{:' + str(longest_name) + 's}: {}({:,.0f} params), trainable:{}, is_cuda:{}'
    sum_params = 0
    for name, param in model.named_parameters():
        layer_params = total_size(param, ignore_first=False)
        sum_params += layer_params
        print(var_meta.format(tabs * '\t', name, size_s(param), layer_params, is_trainable(param), is_cuda(param)))
