...
def display_open_cv_image(
        ...
        title: str = '',
        loc: (tuple, str) = (0, 0),
        resize_scale_percent: float = None
) -> None:
    """
    ...
    :param resize_scale_percent: None for original size. else img_size *= resize (resize > 0)
...
def display_open_cv_images(
    ...
    if imgs_n > 0:
        ...
        big_img = unpack_list_imgs_to_big_image(imgs, grid=grid, resize=resize_scale_percent)
        display_open_cv_image(big_img, ms, title, loc, resize_scale_percent=None)
    return
