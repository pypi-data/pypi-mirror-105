...
def save_load_model_test():
    ...
    mt.get_function_name(ack=True, tabs=0)
    net1 = Net()
    tt.model_info(net1, title='net1', wv=True, chars=100, tabs=1)
    path = './mnist.pt'
    tt.save_model(net1, path=path)

    net2 = Net().cuda()
    tt.model_info(net2, title='net2 - new values', wv=True, chars=100, tabs=1)
    tt.load_model(net2, path)
    tt.model_info(net2, title='net2 - post loading net1 saved weights', wv=True, chars=100, tabs=1)
    mt.delete_file(path)
    return
