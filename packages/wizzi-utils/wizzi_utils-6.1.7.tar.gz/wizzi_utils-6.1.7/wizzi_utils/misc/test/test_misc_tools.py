...
def to_str_test():
    ...
    print(mt.to_str(var=li, title='\t1d list(rounded 7 digits)', fp=7))  # TODO NOT WORKING

    li = [12, 1.2323, 'HI']
    print(mt.to_str(var=li, title='\t1d mixed list'))

    li = [4e-05, 6e-05, 4e-05, 5e-05, 4e-05, 7e-05, 2e-05, 7e-05, 5e-05, 8e-05]  # TODO NOT WORKING
    ...
    ni = np.array([[1231.123122, 15.9], [3.0, 7.55]])
...
def round_list_test():
    ...
    new_li = mt.round_list(li, fp=1)
    print(mt.to_str(var=new_li, title='\tfp=1', fp=None))
    new_li = mt.round_list(li, fp=3)
    print(mt.to_str(var=new_li, title='\tfp=3', fp=None))
    return


def round_tuple_test():
    mt.get_function_name(ack=True, tabs=0)
    t = (1.23123123, 12.123123123123123123, 1.2, 1.0)
    print(mt.to_str(var=t, title='\torigin', fp=None))
    new_t = mt.round_tuple(t, fp=1)
    print(mt.to_str(var=new_t, title='\tfp=1', fp=None))
    new_t = mt.round_tuple(t, fp=3)
    print(mt.to_str(var=new_t, title='\tfp=3', fp=None))
    return
