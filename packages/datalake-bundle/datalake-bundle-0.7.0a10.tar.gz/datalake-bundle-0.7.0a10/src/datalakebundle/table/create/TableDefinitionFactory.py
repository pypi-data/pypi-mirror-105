...
from datalakebundle.table.schema.SchemaClassGenerator import SchemaClassGenerator
...
class TableDefinitionFactory:
    ...
    def __init__(self, logger: Logger, table_parameters_manager: TableParametersManager, schema_class_generator: SchemaClassGenerator):
        self.__logger = logger
        self.__table_parameters_manager = table_parameters_manager
        self.__schema_class_generator = schema_class_generator
    ...
    def create_from_dataframe(self, table_identifier: str, df: DataFrame, decorator_name: str):
        self.__logger.warning(
            ...
            f"you can define schema as\n\n{self.__schema_class_generator.generate(table_identifier, df)}\n"
            f"Usage: @{decorator_name}({table_identifier.split('.')[1]})"
        )
