...
class BaseGAV3Reader:
    ...
    def normalize_report(reports):
        """
        Converts a ga report into a flat file structure.
        :return: dict. ga report.
        """
        data_set = []
        for row in reports['rows']:
            profile_info = reports['profileInfo']
            metrics = reports['totalsForAllResults']
            row_data = profile_info
            row_data.update(metrics)
            dimen_headers = [name['name'] for name in reports['columnHeaders']]
            dimensions = dict(zip(dimen_headers, row))
            ...
            for date in range_of_dates:
                for view_id in self.config.get('view_ids'):
                    ...
                    while True:
                        ...
                        yield self.normalize_report(response)
