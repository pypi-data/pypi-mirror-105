...
class CustomOneSignalReader:
    ...
    def get_csv_url_data(self, url):
        ...
        # so increment wait time until csv is generated
        attempts = 0
        sleep_time = 10
        while True:
            ...
            if csv_response.status_code == 403:
                print(f'Csv url response: {csv_response} '
                      f'attempt: {attempts}/10 '
                ...
                if attempts == 10:
                    raise EnvironmentError(f'Failed to retrieve csv data '
                                           f'after 10 attempts from {url}.')

                attempts += 1  # count attempts
                sleep_time += 2  # increment wait time
