...
class SingleValueNullableAggregationOperationVisitor(AggregationOperationVisitor):
    ...
    def _get_contribute_source_code(  # pylint: disable=no-self-use
        self, operation_element: JavaOperationElement
    ) -> str:
        numeric_code = (
            self._get_contribute_numeric_code(operation_element)
    ...
    def _get_decontribute_source_code(  # pylint: disable=no-self-use
        self, operation_element: JavaOperationElement  # pylint: disable=unused-argument
    ) -> Optional[str]:
        # Single value cannot be de-aggregated
        return None

    def _get_merge_source_code(  # pylint: disable=no-self-use
        self, operation_element: JavaOperationElement
    ) -> str:
        if is_numeric_type(operation_element.output_type):
            output_type = operation_element.output_type
    ...
    def _get_terminate_source_code(  # pylint: disable=no-self-use
        self, operation_element: JavaOperationElement
    ) -> str:
        if is_numeric_type(operation_element.output_type):
