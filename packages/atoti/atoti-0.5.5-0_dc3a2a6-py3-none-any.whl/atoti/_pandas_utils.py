...
def _flatten_dataframe(dataframe: pd.DataFrame):  # type: ignore
    ...
    new_columns = []
    for columns in dataframe.columns.to_flat_index():
        if isinstance(columns, tuple):
            ...
            column = (level for level in columns if not pd.isnull(level))
            new_columns.append(COLUMN_LEVEL_SEPARATOR.join(map(str, column)))
        else:
