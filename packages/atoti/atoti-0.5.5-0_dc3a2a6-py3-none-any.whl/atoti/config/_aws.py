...
def create_aws_config(
    *,
    ...
    Args:
        client_side_encryption: The `client side encryption <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingClientSideEncryption.html>`__ mechanism to use when loading data from AWS.
        region: The AWS region to interact with.
            Required for client side encryption.

            AWS KMS CMK must have been created in the same AWS region as the destination bucket (cf. `AWS documentation <https://docs.aws.amazon.com/AmazonS3/latest/dev/replication-config-for-kms-objects.html>`__).
