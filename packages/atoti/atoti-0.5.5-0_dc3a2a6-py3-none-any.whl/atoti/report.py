...
Each store has a global :attr:`~atoti.store.Store.loading_report` made of several individual loading reports.
...
def _warn_new_errors(errors: Dict[str, int]):
    ...
    for store, error_count in errors.items():
        if error_count > 0:
            message = (
                f"{error_count} error(s) occurred while feeding the store {store}."
            )
