...
# ------------------------------------------------------------------------------
# Script Name: Pydoni CLI App
# Script Author: Andoni Sooklaris
# Date: 2020-03-21
#
# Command line app interface for Pydoni programs.
# ------------------------------------------------------------------------------
...
logger = pydoni.logger_setup(name='pydoni-cli', level=logging.WARN)
...
class AppNavigation(object):
    ...
    def homepage(self, figlet=True):
        ...
        # pydoni.vb.echo('%s Initializing Pydoni CLI...' % click.style('==>', fg='magenta'))
...
class AppInput(object):
    ...
    def get_major_appcmds(self):
        ...
        """
        return {k: v for k, v in app_cli.cli.commands.items() if k != 'app'}
    ...
    def read_override_params(self, app_overrides_file):
        ...
        if os.path.isfile(app_overrides_file):
            ...
            logger.warn(f'App override parameters file not found. Expected at {app_overrides_file}')
            return {}
    ...
    def prompt_user_fill_out_param_values(self, appcmd_params_dict):
        ...
        if len(choices):
            ...
            for pname in edit_param_names:
                ...
                testval = pydoni.test(value, dtype=necessary_dtype)

                while not testval:
                    value = pydoni.get_input('Incorrect datatype! Please re-enter')
                    testval = pydoni.test(value, dtype=necessary_dtype)

                coerced_val = pydoni.test(value, dtype=necessary_dtype, return_coerced_value=True)
...
def app(major, minor, verbose, dry_run, debug, logfile):
    ...
    app_overrides_file = os.path.join(os.path.dirname(__file__), 'app_reference', 'app_param_overrides.yaml')
