...
def define_views(pg, vb, pg_schema, verbose=1, safe=False, dry_run=False):
    ...
    def execute_view_def(vw_sql, vw_name, safe, verbose, dry_run):
    ...
    vw_sql_fpaths = pydoni.listfiles(vw_sql_dpath, full_names=True)
    ...
    for vw_basename in independent_vw_fnames + dependent_vw_fnames_ordered + dependent_vw_fnames_rest:
        ...
        vw_name = splitext(basename(vw_fpath))[0]
        vw_sql = read_view_sqlfile(vw_fpath)
        if not dry_run:
            execute_view_def(vw_sql, vw_name, safe, verbose, dry_run)
