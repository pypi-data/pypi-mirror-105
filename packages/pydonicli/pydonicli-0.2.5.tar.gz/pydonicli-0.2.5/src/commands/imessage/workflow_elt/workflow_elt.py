...
def print_startup_message():
    ...
    with open(os.path.join(os.path.dirname(__file__), 'elt_logging', 'startup_message.txt'), 'r') as f:
        msg = f.read()
...
@click.option('--chat-db-path', type=str, default=os.path.expanduser('~/Library/Messages/chat.db'),
...
@click.command()
def workflow_elt(chat_db_path, pg_schema_load, pg_schema_live, full_refresh, verbose, dry_run, no_startup_message):
    """
    ...
    chat_db_extract = extract_sqlite(
        pg=pg,
        vb=vb,
