...
class BaseAPI(ABC):
    ...
    @abstractmethod
    def get_available_sessions_by_pincode(self, pincode: str, date: datetime.date):
...
class Cowin(BaseAPI):
    ...
    def get_available_sessions_by_pincode(self, pincode, date=None):
        if not date:
...
class AsyncCowin(BaseAPI):
    ...
    async def get_available_sessions_by_pincode(self, pincode, date=None):
        if not date:
            date = datetime.date.today().strftime(DATE_FORMAT)
    ...
    async def confirm_otp(self, txn_id: str, otp: str):
        headers = {'User-Agent': UserAgents.random()}
        payload = {
            'otp': hash_otp(otp),
            'txnId': txn_id
