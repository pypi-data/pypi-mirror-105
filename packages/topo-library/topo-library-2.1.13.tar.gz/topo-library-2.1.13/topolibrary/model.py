...
class CrawlerHistory(Document):
    """ Crawler History  """
    timestamp = DateTimeField(required=True)
    url = StringField(required=True)
    sysetm = StringField(max_length=120)
    run_time = FloatField()
    number_of_events = IntField()
    logs = StringField(max_length=10000)
    result = StringField(max_length=50)

class Systems(Document):
    """ System from setting page  """
    url = StringField(max_length=120, required=True)
    name = StringField(required=True)
    system_type = StringField(required=True)
