...
class CircularArc(Curve):
    ...
    def __init__(self, id: str, origin:Vector2, destination:Vector2, center: Vector2):
        """ Create a circle given a center, start, and end. The arc is assumed to be the clockwise movement from origin to
    ...
    @property
    def angle_rads(self):
        return (self.origin - self.center).angle_from((self.destination - self.origin))
    ...
    def as_circular_arcs(cls, curve, tolerance:float = 10):
        ...
        if len(inflection_points) > 0:
            ...
            if deviation < tolerance:
                return [CircularArc(id=str(uuid4()),
                                   origin=curve.origin,
                                   destination=curve.end_point,
