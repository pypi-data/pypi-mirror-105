...
def _process_grid_blocks(block_list, band_parameter_dict, time, feature, method, product_code, owner, aggregation_statistic, units, data_license, attribution, encoding, description, frequency_interval='T', utc_offset='0H', x_name='x', y_name='y', run_date=None, height=0):
    ...
    for band in bp_bands:
        ...
        for block in block_list:
            ...
            block2 = block1.to_dataset()
            block2 = block2.assign_coords({'time': timestamp})
            block2 = block2.assign_coords({'height': height})
            block2 = block2.expand_dims(['time', 'height'])

            ## Package the results
            prepare_results(data_dict, ds_list, block2, run_date_key)

    return data_dict, dataset_list
...
class Raster(object):
    ...
    def process_save_results(self, remote, x_size, y_size, band_parameter_dict, time, feature, method, product_code, owner, aggregation_statistic, units, data_license, attribution, encoding, description, frequency_interval='T', utc_offset='0H', run_date=None, height=0, public_url=None, threads=30, cores=2):
        """
        ...
        for tif in images:
            ...
            data_dict, dataset_list = _process_grid_blocks(block_list, band_parameter_dict, time, feature, method, product_code, owner, aggregation_statistic, units, data_license, attribution, encoding, description, frequency_interval, utc_offset, self.x_name, self.y_name, run_date, height)
            run_date_dict = {ds: run_date_key for ds in data_dict}
            update_results_s3(6, data_dict, run_date_dict, remote, threads, public_url)
        ...
        #     datasets = executor.map(do_the_work, images)

        ## Update the datasets and station jsons
        print('Aggregating dataset and station data.')
        s3 = s3_connection(remote['connection_config'], 50)

        for ds in datasets:
            ds_new = put_remote_dataset(s3, remote['bucket'], ds)
            ds_stations = put_remote_agg_stations(s3, remote['bucket'], ds['dataset_id'], 50)

        # Aggregate all datasets for the bucket
        ds_all = put_remote_agg_datasets(s3, remote['bucket'], 50)
