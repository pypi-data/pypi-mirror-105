...
def get_hilltop_results(param, station_mtype_corrections=None, quality_codes=True, public_url=None, save_interval_hours=336, modified_date=True):
    ...
    try:
        ...
        for meas in datasets:
            ...
            ex_ds = datasets[meas][0]['dataset_id']
            tethys = Tethys([file_remote])
            remote_stns = tethys.get_stations(ex_ds)
            remote_stns1 = pd.DataFrame(remote_stns)[['station_id', 'ref']]

            ## Assign station_ids
            stns2 = pd.merge(stns2, remote_stns1, on='ref', how='left')

            stns2.loc[stns2['station_id'].isnull(), 'station_id'] = tu.processing.assign_station_ids_df(stns2[stns2['station_id'].isnull()])

            ## Assign geometries
            stns2['geometry'] = tu.processing.create_geometry_df(stns2, to_wkb_hex=True, precision=6)
            ...
            if not mtypes_df.empty:
                ...
                for i, row in mtypes_df.iterrows():
                    ...
                    stn = stns3[stns3['station_id'] == row['station_id']].to_xarray()
        ...
        tu.email_msg(param['remote']['email']['sender_address'], param['remote']['email']['sender_password'], param['remote']['email']['receiver_address'], 'Failure on Hilltop extraction for ' + param['remote']['bucket'], traceback.format_exc(), param['remote']['email']['smtp_server'])
    ...
    try:
        ...
        s3 = tu.s3.s3_connection(remote['connection_config'], 50)

        for ds in dataset_list:
            ds_new = tu.s3.put_remote_dataset(s3, param['remote']['bucket'], ds)
            ds_stations = tu.s3.put_remote_agg_stations(s3, param['remote']['bucket'], ds['dataset_id'], 50)

        ### Aggregate all datasets for the bucket
        ds_all = tu.s3.put_remote_agg_datasets(s3, param['remote']['bucket'], 50)
