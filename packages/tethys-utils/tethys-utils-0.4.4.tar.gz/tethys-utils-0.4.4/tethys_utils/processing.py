...
def package_xarray(data, parameter, attrs, encoding, run_date=None, compression=False, compress_level=1):
    ...
    if grouping == 'blocks':
        ...
        data1 = data1.assign_coords([extent_hex])
    ...
    if isinstance(attrs, dict):
        attrs1.update(copy.deepcopy(attrs))
    ...
    if isinstance(encoding, dict):
        encoding1.update(copy.deepcopy(encoding))
        ...
        if freq_code == 'T':
            ...
            for av in ancillary_variables:
                if qual_col == av:
                    ...
                    qual1 = grp_ts_agg(ts_data2[qual_cols].reset_index(), other_index, 'time', freq_code, 'min')
