        ...
        if not raw:
            ...
            try:
                ...
                if detail:
                    ...
                    join = "\n" if "\n" in detail else " - "
                    msg += f"{join}{detail}"
                # also capture any json we sent when the server says it's on us
                if err is ClientError and kwargs.get("json") is not None:
                    payload = truncate(pprint.pformat(kwargs["json"]))
                    join = "\n" if "\n" in payload else " "
                    msg += f"\n{method} payload was:{join}{payload}"
...
def truncate(data: t.AnyStr, max_length: int = 256) -> t.AnyStr:
    """Truncate a (byte)string to the specified length."""
    if len(data) > max_length:
        dots = "..." if isinstance(data, str) else b"..."
        data = data[: max_length - 3] + dots
    return data
