...
def tune_cv(args: argparse.Namespace) -> None:
    ...
    def define_train_loop(trial: Trial, n_folds: int, base_serialization_dir: str) -> Dict[int, TrainModel]:

        model_per_fold: Dict[int, TrainModel] = dict()

        for fold in range(n_folds):
            os.environ['FOLD'] = str(fold)
            _serialization_dir = os.path.join(base_serialization_dir, "fold_{}".format(fold))
            params = load_params(trial, config_file)
            model_per_fold[fold] = TrainModel.from_params(
                params=params,
                serialization_dir=_serialization_dir,
                local_rank=0,
            )

        return model_per_fold
    ...
    def _objective_cv(trial: Trial,
        ...
        model_per_fold = define_train_loop(trial, n_folds, optuna_serialization_dir)
        scores = []
        for fold in range(n_folds):
            os.environ['FOLD'] = str(fold)
            train_loop = model_per_fold[fold]
