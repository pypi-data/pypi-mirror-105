...
def tune_cv(args: argparse.Namespace) -> None:
    ...
    def load_params(trial: Trial, config_file: str) -> Params:
        ...
        system_attrs = {
            ...
            _STORAGE_NAME: trial.study._storage.url,
            _MONITOR: metrics,
    ...
    def define_train_loop(trial: Trial, _serialization_dir: str) -> TrainModel:

        params = load_params(trial, config_file)

        return TrainModel.from_params(
            params=params,
    ...
    def objective(trial: Trial,
        ...
        optuna_serialization_dir = os.path.join(serialization_dir, "trial_{}".format(trial.number),
                                                "fold_{}".format(fold))
        train_loop = define_train_loop(trial, optuna_serialization_dir)
        loop_metrics = train_loop.run()
        train_loop.finish(loop_metrics)
        return loop_metrics[metrics]
    ...
    def _objective_cv(trial: Trial,
        ...
        for fold_idx in range(n_folds):
            os.environ['FOLD'] = str(fold_idx)
            score = objective(trial, fold_idx)
            scores.append(score)
