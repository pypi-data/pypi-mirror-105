...
class PD_input:
    ...
    def protein_rollup(self,input_file,method='sum'):
        ...
        MPA=MPA[0]
        if method == 'sum':
            input_file = input_file.groupby([MPA]).sum()
            input_file.index = input_file[MPA]
            result = input_file[channels]
        elif method == 'median':
            input_file = input_file.groupby([MPA]).median()
            input_file.index = input_file[MPA]
            result = input_file[channels]
        elif method == 'mean':
            input_file = input_file.groupby([MPA]).mean()
            input_file.index = input_file[MPA]
            result = input_file[channels]
        else:
            print('Problem detected: no correct method set. Used default')
            input_file = input_file.groupby([MPA]).sum()
            input_file.index = input_file[MPA]
            result = input_file[channels]
