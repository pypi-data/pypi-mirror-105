...
def valid_apply_ref(ref):
    if isinstance(ref, str):
        parts = ref.split("/")
        return len(parts) == 2 and parts[0] == "apply" and valid_id(parts[1])
    return False
...
def valid_configuration_changes(changes):
    ...
    for change in changes:
        if not isinstance(change, dict):
            return False
        if "rid" not in change or "content" not in change:
            return False
        content = change["content"]
        if isinstance(content, dict) and "provisioner" in content.keys():
            return False
    return True
