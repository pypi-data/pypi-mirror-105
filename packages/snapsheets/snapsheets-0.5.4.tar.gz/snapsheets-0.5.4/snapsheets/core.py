...
class Config:
    ...
    def check_path(path: str) -> str:
        ...
        if not _path.is_dir():
            path = "."
            warning = f"Directory {_path} nod found. Switch to current directory."
            ic(warning)
        return path
    ...
    def load_toml(self) -> Dict[Any, Any]:
        ...
        """
        config: Dict[Any, Any] = {}
        config.setdefault("tool", None)
        config.setdefault("sheets", None)
        ...
        ic(info)
        for i, fname in enumerate(fnames):
            info = f"Loaded TOMLs [{i+1}/{n}]: {fname}"
            ic(info)
