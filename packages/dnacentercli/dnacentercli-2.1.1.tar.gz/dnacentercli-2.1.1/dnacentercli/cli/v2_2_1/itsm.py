...
def itsm(ctx, obj):
    ...
    Wraps the DNA Center ITSM API and exposes the API as native Python commands.
...
@click.option('--instance_id', type=str,
              help='''Instance Id of the failed event as in the Runtime Dashboard.''',
              show_default=True)
...
@click.pass_obj
def get_failed_itsm_events(obj, pretty_print, beep,
                           instance_id,
                           headers):
    ...
    try:
        ...
        result = obj.get_failed_itsm_events(
...
@click.option('--status', type=str,
              help='''Supported values are "Success","Failed" and "Unknown". Providing other values will result in all the available sync job status.''',
              show_default=True)
@click.option('--date', type=str,
              help='''Provide date in "YYYY-MM-DD" format.''',
              show_default=True)
@click.option('--headers', type=str, help='''Dictionary of HTTP Headers to send with the Request.''',
              default=None,
              show_default=True)
...
@click.option('--beep', is_flag=True, help='''Spinner beep (on)''')
@click.pass_obj
def get_cmdb_sync_status(obj, pretty_print, beep,
                         status,
                         date,
                         headers):
    ...
    try:
        if headers is not None:
            headers = json.loads(headers)
        result = obj.get_cmdb_sync_status(
            status=status,
            date=date,
            headers=headers)
        stop_spinner(spinner)
...
@click.option('--beep', is_flag=True, help='''Spinner beep (on)''')
@click.pass_obj
def retry_integration_events(obj, pretty_print, beep,
                             headers,
                             payload,
                             active_validation):
    ...
    try:
        ...
        result = obj.retry_integration_events(
            headers=headers,
            payload=payload,
