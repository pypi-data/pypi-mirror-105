...
@click.option('--beep', is_flag=True, help='''Spinner beep (on)''')
@click.pass_obj
def get_sda_fabric_count(obj, pretty_print, beep,
                         headers):
    ...
    try:
        if headers is not None:
            headers = json.loads(headers)
        result = obj.get_sda_fabric_count(
            headers=headers)
        stop_spinner(spinner)
        opprint(result, indent=pretty_print)
    except Exception as e:
...
@sda.command()
@click.option('--device_management_ip_address', type=str,
              help='''Device Management IP Address.''',
              required=True,
              show_default=True)
@click.option('--headers', type=str, help='''Dictionary of HTTP Headers to send with the Request.''',
              default=None,
...
@click.option('--beep', is_flag=True, help='''Spinner beep (on)''')
@click.pass_obj
def get_device_role_in_sda_fabric(obj, pretty_print, beep,
    ...
    try:
        if headers is not None:
            headers = json.loads(headers)
        result = obj.get_device_role_in_sda_fabric(
