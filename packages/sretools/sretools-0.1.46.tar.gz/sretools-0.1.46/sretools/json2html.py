    ...
    def jsonize(p,depth=9999) :
        ...
        if type(p) is dict :
            for k,c in p.items() :
                if type(c) is str :
                    try :
                        p[k] = JsonConverter.jsonize(json.loads(c),depth=depth-1)
                    except :
                        pass
                else :
                        p[k] = JsonConverter.jsonize(p[k],depth=depth-1)
        elif type(p) is list :
            for i,k in enumerate(p) :
                if type(k) is str :
                    try :
                        p[i] = JsonConverter.jsonize(json.loads(k),depth=depth-1)
                    except :
                        pass
                else :
                        p[i] = JsonConverter.jsonize(p[i],depth=depth-1)
        elif type(p) is str :
            try :
                p = JsonConverter.jsonize(json.loads(p),depth=depth-1)
            except :
                pass
        else :
            pass
