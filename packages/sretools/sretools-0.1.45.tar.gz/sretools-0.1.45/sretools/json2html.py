    ...
    def jsonize(p) :
        if type(p) is dict :
            for k,c in p.items() :
                if type(c) is str :
                    try :
                        p[k] = JsonConverter.jsonize(json.loads(c))
                    except :
                        pass
                else :
                        p[k] = JsonConverter.jsonize(p[k])
        elif type(p) is list :
            for i,k in enumerate(p) :
                if type(k) is str :
                    try :
                        p[i] = JsonConverter.jsonize(json.loads(k))
                    except :
                        pass
                else :
                        p[i] = JsonConverter.jsonize(p[i])
        elif type(p) is str :
            try :
                p = JsonConverter.jsonize(json.loads(p))
            except :
                pass
        else :
            pass
