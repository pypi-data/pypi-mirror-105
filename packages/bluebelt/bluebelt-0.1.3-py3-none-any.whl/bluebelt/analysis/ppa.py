...
class ProcessCapability():
    ...
    def calculate(self):
        ...
        # basic statistics
        self.min = self.series.values.min()
        self.max = self.series.values.max()
        self.mean = self.series.values.mean()
        self.std = self.series.values.std()
        self.size = len(self.series)

        # _lsl, _usl and midpoint to calculate performance
        self._lsl = self.lb or self.lsl or self.min
        self._usl = self.ub or self.usl or self.max
        self._midpoint = (self._usl + self._lsl) / 2

        # performance
        self.lt_lsl = (self.series[self.series < self._lsl].count() / self.size)
        self.gt_usl = (self.series[self.series > self._usl].count() / self.size)
        
        self.observed_performance = self.series[(self.series >= self._lsl) & (self.series <= self._usl)].count() / self.size
        
        self.pp = self.get_pp(tolerance=self.tolerance)
        self.ppl = self.get_ppl(tolerance=self.tolerance)
        self.ppu = self.get_ppu(tolerance=self.tolerance)
        self.ppk = self.get_ppk(tolerance=self.tolerance)
        self.cpm = self.get_cpm(tolerance=self.tolerance)


    def get_pp(self, tolerance=6):
        # Pp = (USL – LSL) / tolerance * sigma
        return (self._usl - self._lsl) / (tolerance * self.std)

    def get_ppl(self, tolerance=6):
        return (self.mean - self._lsl) / ((tolerance / 2) * self.std)

    def get_ppu(self, tolerance=6):
        return (self._usl - self.mean) / ((tolerance / 2) * self.std)

    def get_ppk(self, tolerance=6):
        return min(self.get_ppl(tolerance=tolerance), self.get_ppu(tolerance=tolerance))

    def get_cpm(self, tolerance=6):
        if not self.target:
            return None
        
        else:
            if not (self.ub or self.usl):
                # LSL and target only
                numerator = self.target - (self.lb or self.lsl)
            elif not (self.lb or self.lsl):
                # USL and target only
                numerator = (self.ub or self.usl) - self.target
            else:
                numerator = min(self.target - (self.lb or self.lsl), (self.ub or self.usl) - self.target)
        ...
        denominator = tolerance * ((sum([sum((dfx[col].dropna()-target)**2) for col in dfx]) / series.sum())**0.5)
    ...
    def __repr__(self):
        ...
        return (f'{self.__class__.__name__}(n={self.size}, target={self.target}, {lsl_text}={self.lb or self.lsl}, {usl_text}={self.ub or self.usl}, observed={self.observed_performance:1.4f})')
    
    def df(self):
        df_md = pd.DataFrame({
            'metric': ["target", "LB" if self.lb else "LSL", "UB" if self.ub else "USL", "% < LSL", "% > USL", "Observed Performance", "Pp", "Ppk"],
            'value': [self.target, self.lb or self.lsl, self. ub or self.usl, self.lt_lsl, self.gt_usl, self.observed_performance, self.pp, self.ppk],
            }
        )
    ...
    def plot(self, **kwargs):
        ...
        histogram_points = [x for x in [self._lsl, self._usl, self.target] if x is not None]
        ...
        # LSL, USL
        if self.lb or self.lsl:
            ax.axvline(x=self.lb or self.lsl, ymin=0, ymax=1, **style.process_capability.sl_axvline)
            lsl_text = 'LB' if self.lb else 'LSL'
            ax.text(self.lb or self.lsl, ylims[1]*0.9, lsl_text, **style.process_capability.sl_text)
        
        if self.ub or self.usl:
            ax.axvline(x=self.ub or self.usl, ymin=0, ymax=1, **style.process_capability.sl_axvline)
            usl_text = 'UB' if self.ub else 'USL'
            ax.text(self.ub or self.usl, ylims[1]*0.9, usl_text, **style.process_capability.sl_text)

        # change xlim if needed
        xlims_min = min(self.series.values.min(), self._lsl, self._usl, self.target)
        xlims_max = max(self.series.values.max(), self._lsl, self._usl, self.target)
