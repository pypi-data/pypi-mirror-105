...
class ArkitektModelException(Exception):
    pass

class ModelDoesNotExistError(ArkitektModelException):
    pass

class ArnheimModelConfigurationError(ArkitektModelException):
    pass


ModelType = TypeVar("ModelType", bound="ArkitektModel", covariant=True)

class ArkitektModelManager(BaseManager, Generic[ModelType]):
...
class ArkitektAsyncModelManager(BaseManager, Generic[ModelType]):
...
class ArkitektModelMeta(ModelMetaclass):
    ...
    def __new__(mcls, name, bases, attrs):
        ...
        return super(ArkitektModelMeta, mcls).__new__(mcls, name, bases, attrs)


    @property
    def objects(cls: Type[ModelType]) -> ArkitektModelManager[ModelType]:
        return cls.__objects

    @property
    def asyncs(cls: Type[ModelType]) -> ArkitektAsyncModelManager[ModelType]:
        return cls.__asyncs


    def __init__(self, name, bases, attrs):
        super(ArkitektModelMeta, self).__init__(name, bases, attrs)
        if attrs["__qualname__"] != "ArkitektModel":
            ...
            if managerClass:
                ...
                self.__objects = ArkitektModelManager[ModelType](model=self,meta=meta)

            asyncmanagerClass = self.overriden_async_manager 
            if asyncmanagerClass:
                self.__asyncs = asyncmanagerClass(model=self,meta=meta)
            else:
                self.__asyncs = ArkitektModelManager[ModelType](model=self,meta=meta)
