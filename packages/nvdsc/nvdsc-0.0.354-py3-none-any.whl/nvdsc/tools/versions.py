...
class VersionChecker:
  ...
  PIP_NVDSC_UPGRADE_CMD = "pip3 install -U nvdsc"
...
#      self.local['kaggle'] = subprocess.run([os.environ['HOME'] + "/.local/bin/kaggle", "--version"], stdout=subprocess.PIPE).stdout.split()[-1].decode('utf-8').split(',')[0]
  ...
  def check_dss_local(self):
    ...
    try:
      self.local['dss'] = subprocess.run(["/bin/sh", "-c", "grep '^STACK_VERSION=' " + os.environ['HOME'] + "/data-science-stack/data-science-stack"], stdout=subprocess.PIPE).stdout.decode('utf-8').rstrip().split('=')[1]
    except:
  ...
  def check_pip_remote(self):
    out = None
    try:
      out = subprocess.run(["/bin/sh", "-c", "pip3 list -o --format json"], stdout=subprocess.PIPE).stdout      
    except:
    ...
    if out is not None:
      self.parse_pip_output(out)

  def parse_pip_output(self, out):
    try:
      for j in json.loads(out):
        if j['name'] == 'nvdsc':
          self.remote['nvdsc'] = j['latest_version']
          self.local['nvdsc'] = j['version']
...
#    out = subprocess.run(["/bin/bash", "-c", os.environ['HOME'] + "/.local/bin/iudss.sh"], capture_output=True)
  ...
  def update_nvdsc(self):
    log.debug("nvdsc update starting")
    out = subprocess.run(["/bin/sh", "-c", "pip3 install -U nvdsc" ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if out.returncode == 0:
      log.info("nvdsc updated")
      log.debug(out.stdout.decode('utf-8'))
      log.debug(out.stderr.decode('utf-8'))
    else:
      log.warning("unable to upgrade nvdsc:")
      log.warning(out.stdout.decode('utf-8'))
      log.warning(out.stderr.decode('utf-8'))    
    log.debug("nvdsc update done")  

    return out.stdout.decode('utf-8'), out.stderr.decode('utf-8'), out.returncode

  def get_nvdscd_start_cmd(self):
    if self.script_dir is not None:
      return self.script_dir + "/nvdscd"
    else:
      return os.environ['HOME'] + "/.local/bin/nvdscd"

  def launch_nvdsc(self):
    log.debug("nvdsc launch starting")
    # subprocess.Popen
    if self.script_dir is not None:
      proc = subprocess.Popen(self.script_dir + "/nvdsc", shell = True)
    else:
      proc = subprocess.Popen(os.environ['HOME'] + "/.local/bin/nvdsc", shell = True)
    log.debug("nvdsc launch sleeping 5")
    time.sleep(5)
    log.debug("nvdsc launch polling")
    rc = proc.poll()
    # None is good

    log.debug("nvdsc launch returning")
