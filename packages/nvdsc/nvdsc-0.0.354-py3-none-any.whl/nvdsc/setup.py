...
# handler = TimedRotatingFileHandler(LOG_FILE, when="d", interval=config['MAIN']['LOG_ROLL_DAYS'], backupCount=config['MAIN']['LOG_BACKUPS'])
...
def gpu_present():
  if subprocess.run(["/bin/sh", "-c", "lspci | grep 'VGA compatible controller: NVIDIA Corporation'"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.decode('utf-8') == '':
    return False
  else:
    return True
...
def install_autostart():
  ...
  # need to copy nvdsc.desktop to ~/.config/autostart
  ...
  src_file = SCRIPT_DIR + '/nvdsc.desktop'
  tgt_file = astart_dir + '/nvdsc.desktop'
 
  try:
    shutil.copy(src_file, tgt_file) 
    LOG.info("installed autostart entry to " + astart_dir + '/nvdsc.desktop')
  except:
    LOG.error("could not copy " + src_file + " to " + tgt_file)
    return False
 
  with open(tgt_file, 'a') as f:  
    print("Exec=" + SCRIPT_DIR + '/nvdsc', file = f)
...
#  v.check_remote()
  ...
  if v.local['driver'] != 'not found' and sverc(v.local['driver'], config['MAIN']['MIN_DRIVER_VERSION']) >=0 and \
      ...
      v.local['dss'] != 'not found' and sverc(v.local['dss'], config['MAIN']['LATEST_DSS_VERSION']) == 0:
