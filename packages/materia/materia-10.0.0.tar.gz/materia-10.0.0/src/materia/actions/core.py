...
class InsertTasks(Action):
    def __init__(self, *tasks, requires_kw=None) -> None:
        # NOTE: the first task in tasks must be the head
        # and the last task in tasks must be the tail!
        self.tasks = copy.deepcopy(tasks)
        self.requires_kw = requires_kw

    def run(self, node, tasks, links, done: Dict[int, bool]) -> None:
        head_task_node = len(tasks)
        # this value is safe to use in the scope of this method
        # because only the producer runs actions,
        # and there is only one producer at a time
...
class Modify(Action):
    def modify(self, task: materia.Task) -> materia.Task:
        # NOTE: this should return the modified task
        # this can be a copy or it can modify task in
        # place and return it, either way
