...
def install():
    ...
    def set_code(reader, request, type="tenant_code"):
        ...
        # 设置用户自定义字段值
        if type == "tenant_code" and not config.custom_tenant_code:
            config.custom_tenant_code = code
        elif type == "user_code" and not config.custom_user_code:
            config.custom_user_code = code
        elif type == "env_code" and not config.custom_env_code:
