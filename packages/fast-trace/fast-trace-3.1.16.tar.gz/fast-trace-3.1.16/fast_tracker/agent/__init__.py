...
def start():
    print("FAST: Config.Enable is %s " % config.enable)
    # 如果探针开关关闭,则不执行探针的初始化操作，不启动探针
    if not config.enable:
        return False

    if not config.product_code or not config.app_code:
        print("FAST: 缺少product_code或app_code字段值，探针初始化失败")
        return False
    ...
    try:
        ...
        print("FAST: 未发现使用gevent，如果确实未使用gevent，请忽略。")
