...
class Requests(object):
    ...
    def _request(self, method, url, **kwargs):
        ...
        logger = LoggerWrapper(job_id=job_id)
        logger.info(f"{method.upper()} {url}")
        logger.info(f"kwargs: {kwargs}")
        try:
            ...
            logger.info("\n\n\n")
            return None  # 请求异常时 response 为 None
        except Exception:
            logger.error(format_exc())
            raise  # 代码异常时打印日志并抛出异常



class LoggerWrapper:

    def __init__(self, job_id, logger=logger):
        self.job_id = job_id
        self.logger = logger
    
    def __getattr__(self, attr):
        if attr in ("critical", "error", "warning", "info", "debug", "notset"):
            return self.wapper(getattr(self.logger, attr))

    def wapper(self, method):
        def inner(msg, *args, **kwargs):
            msg = self._generate_log_msg(msg)
            return method(msg, *args, **kwargs)
        return inner

    def _generate_log_msg(self, msg):
        return f"{self.job_id}: {msg}"
