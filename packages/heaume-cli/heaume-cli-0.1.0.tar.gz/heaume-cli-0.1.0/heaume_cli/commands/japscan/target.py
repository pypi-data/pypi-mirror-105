...
@click.command()
@click.option("--email", help="The email of the japascan user", required=True)
@click.option(
    "--type",
...
def target(email, type, add, delete):
    """
    ...
    if not add and not delete:
        response = requests.get(f"{endpoint}/target?email={email}")
    else:
        if add:
            response = requests.post(f"{endpoint}/target?email={email}", json={"targets": list(add)})
        if delete:
            response = requests.delete(f"{endpoint}/target?email={email}", json={"targets": list(delete)})        
