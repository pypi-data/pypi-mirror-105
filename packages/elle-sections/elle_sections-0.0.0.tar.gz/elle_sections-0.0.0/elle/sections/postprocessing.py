...
def surface_points(Qm, fy=1.0, yref=0., norm=False, scale=None, **kwds):
    to_float = True
    nIP = len(Qm[0])

    f_n = [[fy*(-1)**(j>i) for i in range(1,nIP+1)] for j in range(1,nIP+1)] + \
...
def plot_strain(yi,e,ax=None,mpl={},x_scale=1.0,**kwds):
    ...
    epsi = []
    def eps(y,epsa,kappa): return epsa -y*kappa 
...
def My_abd(k, yref, ymf, d, bf, tf, tw, fy, E):
    ...
    Mu =-(bf-tw)*((-6*d*fy*h + 3*(fy*h**2) - 3*c**2*fy + 3*(d**2*fy) + 3*(c**2*(E*(h*k))) + 3*(d**2*(E*(h*k))) - E*h**3*k + 2*(c**3*(E*k)) - 2*d**3*(E*k))/6)
...
def yield_points(yref, ymf, d, bf,  tw, fy, E,**kwds):
    ...
    kap = onp.linspace(0., 2.*fy/E/d, n_abd)
