...
class TaskDB:
    ...
    def que_tasks(self, result, batch_size=3000):
        """
        查询任务
...
class MainPath:
    def __init__(self, main_path: str, root='.', new=True):
        """
        用于主目录的处理, 通过warp实现同一个文件夹下不会出现一摸一样的文件夹
        :param main_path: str; 主目录, 不支持从根目录开始
        :param root: str; 主目录所在的目录, 支持 ~
        :param new: bool; 是否会创建包裹后的 main_path, 不会覆盖(倒也不会重复)
        """
        assert main_path[0] != '/' and main_path[0] != '\\' and main_path[0] != '~', 'main_path 不支持从根目录开始!'
        self._origin = main_path.rstrip('/\\')
        self._father = f"{datetime.datetime.now().strftime('%Y%d%m_%H%M%S')}_{random.random()}"
        self._root = os.path.expanduser(root.rstrip('/\\'))
        self._all_path = os.path.join(self._root, self._father, self._origin)
        if new and not os.path.isdir(self._all_path):
            os.makedirs(self._all_path)

    @property
    def m(self):  # origin, main_path 名
        return self._origin

    @property
    def f(self):  # father, main_path 的随机生成的父目录名
        return self._father

    @property
    def fm(self):  # father/origin
        return os.path.join(self._father, self._origin)

    @property
    def rfm(self):  # root/father/origin
        return self._all_path

    @property
    def rf(self):  # root/father
        return os.path.join(self._root, self._father)
    ...
    @property
    def rm(self):  # root/origin, 不包裹的 main_path 目录
        return os.path.join(self._root, self._origin)


class TaskDBapi:
    @staticmethod
    def app_run(db_dirs: list, port=19999, log_path=f'log/TaskDBapi.app_run.log', mongo_url=None):
        """
    ...
    def request_api(request_data: dict, url='http://127.0.0.1:19999', sleep=0.5, try_times=5, mp: MainPath = None,
        ...
        :param mp: MainPath() or None; 用于将完成的文件移动到子文件夹, MainPath()且是已完成任务才会移动
            main_path 会从 mp.rf 移动到 mp.r/__completed, 然后删除 mp.rf
        ...
        if mp and 'status' in response and response['status'] >= 1 and request_data['type'] == 'complete':
            ...
            assert os.path.isdir(mp.rfm), '没有生成包裹的主目录 main_path: ' + mp.rfm
            assert not os.path.isdir(mp.rm), '生成了无包裹的主目录 main_path: ' + mp.rm
            # 生成完成目录
            completed_path = f'{mp.r}/__completed'
            if not os.path.exists(completed_path):
                os.makedirs(completed_path)
            # 移动任务目录
            if not os.path.exists(f'{completed_path}/{mp.m}'):
                shutil.move(mp.rfm, f'{completed_path}/{mp.m}')
                shutil.rmtree(mp.rf)
            else:
                with open(f'{mp.rf}/complete.txt', 'w', encoding='utf8') as w:
                    w.write('任务已被完成, 不再移动 main_path !')  # 写入文件提示这是完成而没移动的文件
                print('任务已被完成, 不再移动 main_path !')
        return response
