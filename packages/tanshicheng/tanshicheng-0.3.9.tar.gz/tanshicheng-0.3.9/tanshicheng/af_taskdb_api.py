...
class TaskDB:
    def __init__(self, dir, new=False, mongo_url=None, mongo_db=None, mongo_co=None, tiny_to_mongo=False):
        ...
        :param mongo_db: None or str; mongodb 的 database name
        :param mongo_co: None or str; mongodb 的 collection name
        ...
        if mongo_url and mongo_db and mongo_co:
            ...
            if new:
                ...
                self._db_tasks.create_index([('priority', pymongo.ASCENDING)], unique=False, background=True)
                self._db_tasks.create_index([('executed', pymongo.ASCENDING)], unique=False, background=True)
                self._db_tasks.create_index([('main_path', pymongo.ASCENDING)], unique=False, background=True)
    ...
    def _result_to_query(result, is_mongo=False, root=''):
        ...
        if is_mongo:
            ...
            if isinstance(result, dict):
                ...
                for v in result:
                    query.update(TaskDB._result_to_query(v, is_mongo=is_mongo, root=root))
            else:
                query[root] = result
        else:
        ...
        if isinstance(self._db_tasks, pymongo.collection.Collection):
            for result, no in zip(result_L, no_L):
                ...
                if self._db_tasks.update_many({'no': no}, result).modified_count:
                    doc_L += self._db_tasks.find({'no': no})
        else:
            results_L = []
            for result, no in zip(result_L, no_L):
                result.pop('no', 0)  # 更新不含 no
    ...
    def stat_result(self, out_tasked=False, **kwargs):
        ...
        uncomplete_tasks = self.uncomplete_tasks
        print('已完成任务数:', len(tasks), '; 未完成任务数:', len(uncomplete_tasks))
        if out_tasked:
            print('=' * 10, '所有已完成任务:')
            pprint(tasks)
            print('=' * 10, '所有未完成任务:')
            pprint(uncomplete_tasks)
    ...
    def test(db, mongo_url=None, mongo_db=None, mongo_co=None):
        obj = TaskDB(db, new=True, mongo_url=mongo_url, mongo_db=mongo_db, mongo_co=mongo_co, tiny_to_mongo=True)
        n = 6
        for i in range(n):
        ...
        if obj.is_mongo:
            q_L = obj.que_tasks([
                {'$match': {'priority': 1, 'run_tasks_test': 2}},
            ])
        else:
            q_L = obj.que_tasks({'priority': 1, 'run_tasks_test': [2]})
        print(q_L)
...
class TaskDBapi:
    ...
    def app_run(db_dirs: list, port=19999, log_path=f'log/TaskDBapi.app_run.log',
                mongo_url=None, mongo_db=None, mongo_co=None):
        """
        将数据库做成 http api 用于分布式机器任务分配
        :param db_dirs: [数据库目录1,..]
        :param port: int; 访问端口
        :param log_path: str; 日志输出地址
        以下参数不是list会自动扩展为长度为len(db_dirs)的list, 具体含义参见 TaskDB.__init__()
        :param mongo_url: None or str or [url,..]
        :param mongo_db: None or str or [database,..]
        :param mongo_co: None or str or [collection,..]
        :return:
        """
        if not db_dirs:
            return None
        ...
        if not isinstance(mongo_db, list):
            mongo_db = [mongo_db] * len(db_dirs)
        if not isinstance(mongo_co, list):
            mongo_co = [mongo_co] * len(db_dirs)
        assert len(db_dirs) == len(mongo_url) == len(mongo_db) == len(mongo_co), '数据库参数数量不匹配!'

        for d, url, db, co in zip(db_dirs, mongo_url, mongo_db, mongo_co):
            db_no_tasks_D[d] = {}  # {no:task,..}
            db_no8num_D[d] = []  # [[[no,分配次数],..],..]; 按这个顺序分配任务
            db_obj_D[d] = TaskDB(d, mongo_url=url, mongo_db=db, mongo_co=co)
    ...
    def test(db, mongo_url=None, mongo_db=None, mongo_co=None):
        ...
        if not TaskDBapi.request_api({}, try_times=1, url=url):  # 是否有服务器存在
            ...
            TaskDBapi.app_run(db_dirs=[db], port=port, mongo_url=mongo_url, mongo_db=mongo_db, mongo_co=mongo_co)
        else:
            # 查询请求
