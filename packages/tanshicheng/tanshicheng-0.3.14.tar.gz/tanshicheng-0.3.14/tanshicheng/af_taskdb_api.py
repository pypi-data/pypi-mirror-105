...
class TaskDBapi:
    ...
    def request_api(request_data: dict, url='http://127.0.0.1:19999', sleep=0.5, try_times=5, mp: MainPath = None,
        ...
        :param mp: MainPath() or None; 用于将完成的文件移动到子文件夹, MainPath()且是已完成任务才会移动
            main_path 会从 mp.p('ruf') 移动到 mp.('r'), 然后删除 mp.('ruf')
        ...
        if mp and 'status' in response and response['status'] >= 1 and request_data['type'] == 'complete':
            ...
            assert main_path == mp.p('m'), f"请求完成结果中的 main_path 与 mp.p('m') 不同: {main_path} != {mp.p('m')}"
            assert os.path.isdir(mp.p('rufm')), '没有生成未完成的包裹主目录 main_path: ' + mp.p('rufm')
            assert not os.path.isdir(mp.p('rum')), '生成了无包裹的主目录 main_path: ' + mp.p('rum')
            # 移动任务目录
            if not os.path.exists(mp.p('rm')):
                shutil.move(mp.p('rufm'), mp.p('rm'))
                shutil.rmtree(mp.p('ruf'))
            else:
                with open(f"{mp.p('ruf')}/complete.txt", 'w', encoding='utf8') as w:
