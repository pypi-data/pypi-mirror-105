...
def circuit_from_netlist(
    ...
    models.insert(0, m)
    for i, _models in enumerate(models):  # type: ignore
        if isinstance(_models, Module):
            _models = {
                k: v for k, v in _models.__dict__.items() if isinstance(v, Model)
            }
        models[i] = _models
    ...
    for instance_name, kwargs in netlist["instances"].items():
        ...
        if model_name in models:
            ...
            _model = getattr(m, model_name)
        else:
            raise ValueError(
                f"Could not find a model factory for '{model_name}' (instance '{instance_name}')"
            )
        if isinstance(_model, Model):
            instance = model(_model, settings)
        elif callable(_model):  # model factory
            instance = _model(**settings)
        else:
            raise ValueError(
                f"Could not find a model factory for '{model_name}' (instance '{instance_name}')"
            )
