...
def main():
    ...
    # Add any required site directories to the module search path. These may
    # be specified in the site or user-specific afterburner config files.
    try:
        import bootstrap
        bootstrap.add_extra_site_dirs()
    except ImportError:
        print("WARNING: Unable to import the afterburner bootstrap module.")

    try:
        from afterburner.apps import initialise_app
        from afterburner.exceptions import AfterburnerError
    except ImportError as exc:
        print("ERROR: Unable to initialise the afterburner package owing to an")
