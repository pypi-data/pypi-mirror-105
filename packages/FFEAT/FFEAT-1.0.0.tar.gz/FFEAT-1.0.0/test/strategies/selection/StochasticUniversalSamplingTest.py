...
class StochasticUniversalSamplingTest(unittest.TestCase):
    def test_absolute(self):
        s = selection.StochasticUniversalSampling(40)
    ...
    def test_fraction(self):
        s = selection.StochasticUniversalSampling(0.4)
        pop, fitness = t.rand((100,60)), t.rand((100,))
        (newpop,), kargs = s(fitness, pop)
        self.assertEqual(newpop.shape, (40,60))
        self.assertIsNot(newpop, pop)
    ...
    def test_no_number(self):
        s = selection.StochasticUniversalSampling()
        pop, fitness = t.rand((100,60)), t.rand((100,))
        (newpop,), kargs = s(fitness, pop)
    ...
    def test_to_select_object(self):
        s = selection.StochasticUniversalSampling(object())
        pop, fitness = t.rand((100,60)), t.rand((100,))
        with self.assertRaises(ValueError):
            s(pop, fitness)

    def test_is_better(self):
        s = selection.StochasticUniversalSampling(1.0)
    ...
    def test_absolute_callback(self):
        s = selection.StochasticUniversalSampling(ffeat.utils.decay.Linear(80, 40, result_type=int))
    ...
    def test_fraction_callback(self):
        s = selection.StochasticUniversalSampling(ffeat.utils.decay.Linear(0.8, 0.4))
        pop, fitness = t.randn((100,60)), t.rand((100,))
        (newpop,), kargs = s(fitness, pop, iteration=30, max_iteration=40)
        self.assertEqual(newpop.shape, (50,60))
        self.assertIsNot(newpop, pop)
