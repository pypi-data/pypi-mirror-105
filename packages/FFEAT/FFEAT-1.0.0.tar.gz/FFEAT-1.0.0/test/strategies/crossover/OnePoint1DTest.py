...
class OnePoint1DTest(unittest.TestCase):
    ...
    def test_absolute_without_replace(self):
        s = crossover.OnePoint1D(40, replace_parents=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (140,400))

    def test_fraction_without_replace(self):
        s = crossover.OnePoint1D(0.4, replace_parents=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (140,400))
    ...
    def test_odd_offsprings(self):
        with self.assertRaises(ValueError):
            crossover.OnePoint1D(offsprings=39)

    def test_offsprings_absolute_not_inplace(self):
        s = crossover.OnePoint1D(40, in_place=False)
        pop = t.randn(100,400)
        (newpop,), kargs = s(pop)
        self.assertEqual(newpop.shape, (100,400))
