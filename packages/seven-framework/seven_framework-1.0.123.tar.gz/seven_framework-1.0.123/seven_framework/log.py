...
:LastEditTime: 2021-05-11 16:56:01
...
class Logger:
    ...
    def __init__(self,
        ...
        backup_count = 30
        # 日志文件备份时间统一设置
        if "log_file_backup_count" in log_config and log_config["log_file_backup_count"]:
            try:
                backup_count = int(log_config["log_file_backup_count"])
            except:
                print(f"日志模块读取配置中log_file_backup_count值异常")
        # 日志文件备份时间按照level设置(优先)
        if f"log_file_backup_count_{log_level.lower()}" in log_config and log_config[f"log_file_backup_count_{log_level.lower()}"]:
            try:
                backup_count = int(
