...
def _should_set_reset_index(automl_run: Optional[Run] = None,
    ...
    if automl_run is not None:
        # In order to support the current way unit tests cases are mocked without too much refactoring,
        # it can not be compressed to e.g. automl_run.properties.get('ensembled_algorithms', []).
        # See "_create_mock_automl_run()" in model_explain_test_helper.py.
        run_algorithms = automl_run.properties.get('run_algorithm')
        if run_algorithms is None:
            run_algorithms = []
        ensembled_algorithms = automl_run.properties.get('ensembled_algorithms')
        if ensembled_algorithms is None:
            ensembled_algorithms = []
        if any([model in run_algorithms or model in ensembled_algorithms for model in forecasting_exclusive_algos]):
            return True
    elif fitted_model is not None:
        if any([model in fitted_model.named_steps for model in forecasting_exclusive_algos]):
            return True
    return False
