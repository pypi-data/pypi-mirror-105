...
.. code-block:: python
    ...
    # web3.py instance
    w3 = Web3(web3.platone.TesterProvider())

    # set pre-funded account as sender
    w3.platone.defaultAccount = w3.platone.accounts[0]

    # Instantiate and deploy contract
    Greeter = w3.platone.contract(abi=contract_interface['abi'], bytecode=contract_interface['bin'])

    # Submit the transaction that deploys the contract
    tx_hash = Greeter.constructor().transact()

    # Wait for the transaction to be mined, and get the transaction receipt
    tx_receipt = w3.platone.waitForTransactionReceipt(tx_hash)

    # Create the contract instance with the newly-deployed address
    greeter = w3.platone.contract(
    ...
    w3.platone.waitForTransactionReceipt(tx_hash)
...
Instead, create contract objects using the :meth:`w3.platone.contract() <web3.platone.Eth.contract>`
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).transact(transaction=None)
    ...
    ``web3.platone.sendTransaction(transaction)`` method.  This value may not
    ...
    deployment transaction will be created using the ``web3.platone.estimateGas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> deploy_txn = token_contract.constructor(web3.platone.coinbase, 12345).transact()
        >>> txn_receipt = web3.platone.getTransactionReceipt(deploy_txn)
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).estimateGas(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> token_contract.constructor(web3.platone.coinbase, 12345).estimateGas()
        12563
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).buildTransaction(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> transaction = {
        'gasPrice': w3.platone.gasPrice,
        'chainId': None
        }
        >>> contract_data = token_contract.constructor(web3.platone.coinbase, 12345).buildTransaction(transaction)
        >>> web3.platone.sendTransaction(contract_data)
...
For example:
    ...
    .. code-block:: python
        ...
        myContract = web3.platone.contract(address=contract_address, abi=contract_abi)
...
.. py:method:: ContractFunction.transact(transaction)
    ...
    the same format as the ``web3.platone.sendTransaction(transaction)`` method.
    ...
    If a ``gas`` value is not provided, then the ``gas`` value for the
    method transaction will be created using the ``web3.platone.estimateGas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> token_contract.functions.transfer(web3.platone.accounts[1], 12345).transact()
...
.. py:method:: ContractFunction.call(transaction, block_identifier='latest')
    ...
    .. code-block:: python
        ...
        21
        >>> token_contract.functions.myBalance().call({'from': web3.platone.coinbase})
        12345  # the token balance for `web3.platone.coinbase`
        >>> token_contract.functions.myBalance().call({'from': web3.platone.accounts[1]})
        54321  # the token balance for the account `web3.platone.accounts[1]`
...
For example:
    ...
    .. code-block:: python
        ...
        receipt = web3.platone.getTransactionReceipt(tx_hash)
...
.. py:method:: ContractEvents.myEvent(*args, **kwargs).processReceipt(transaction_receipt)
   ...
   .. code-block:: python
       ...
       >>> tx_receipt = w3.platone.getTransactionReceipt(tx_hash)
...
.. py:classmethod:: Contract.decode_function_input(data)
    ...
    .. code-block:: python
        ...
        >>> transaction = w3.platone.getTransaction('0x5798fbc45e3b63832abc4984b0f3574a13545f415dd672cd8540cd71f735db56')
        >>> transaction.input
