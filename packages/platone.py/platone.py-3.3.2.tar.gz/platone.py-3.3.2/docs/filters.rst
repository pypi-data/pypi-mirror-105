...
The :meth:`web3.platone.filter` method can be used to setup filters for:

* Pending Transactions: ``web3.platone.filter('pending')``

* New Blocks ``web3.platone.filter('latest')``
...
* Event Logs
    ...
    .. code-block:: python
        ...
        event_filter = web3.platone.filter({"address": contract_address})
...
* Attaching to an existing filter
    ...
    .. code-block:: python
        ...
        existing_filter = web3.platone.filter(filter_id="0x0")
...
.. py:method:: Filter.get_new_entries()
    ...
    :func:`web3.platone.getFilterChanges` which returns only new entries since the last
    poll.
...
.. py:method:: Filter.get_all_entries()
    ...
    Logs will be retrieved using the
    :func:`web3.platone.getFilterLogs` which returns all entries that match the given
    filter.
...
You can setup a filter for new blocks using ``web3.platone.filter('latest')`` which
...
You can setup a filter for new blocks using ``web3.platone.filter('pending')`` which
will return a new :py:class:`BlockFilter` object.
...
You can set up an event log filter like the one above with `web3.platone.filter` by supplying a
...
equivalent filter creation would look like:
    ...
    .. code-block:: python
        ...
        event_filter = web3.platone.filter({
...
^^^^^^^^^^^
    ...
    .. code-block:: python
        ...
        def main():
            block_filter = w3.platone.filter('latest')
...
entries to a handler.
        ...
        .. code-block:: python
            ...
            def main():
                block_filter = w3.platone.filter('latest')
                tx_filter = w3.platone.filter('pending')
