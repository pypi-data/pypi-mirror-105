...
def test_miner_setGasPrice(web3_empty, wait_for_block):
    ...
    initial_gas_price = web3.platone.gasPrice

    # sanity check
    assert web3.platone.gasPrice > 1000

    web3.miner.setGasPrice(initial_gas_price // 2)

    with Timeout(60) as timeout:
        while web3.platone.gasPrice == initial_gas_price:
            timeout.sleep(random.random())

    after_gas_price = web3.platone.gasPrice
