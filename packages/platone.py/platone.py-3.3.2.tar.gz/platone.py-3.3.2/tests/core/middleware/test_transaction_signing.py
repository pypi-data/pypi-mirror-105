...
from platone.middleware.signing import (
    gen_normalized_accounts,
)
from platone.providers import (
    BaseProvider,
)
...
def fund_account(w3):
    ...
    for address in (ADDRESS_1, ADDRESS_2):
        w3.platone.sendTransaction({
            'to': address,
            'from': w3.platone.accounts[0],
            'gas': 21000,
            'value': tx_value})
        assert w3.platone.getBalance(address) == tx_value
...
def test_signed_transaction(
    ...
    to_from = valfilter(bool, {'to': w3.platone.accounts[0], 'from': from_})

    _transaction = merge(transaction, to_from)

    if isinstance(expected, type) and issubclass(expected, Exception):
        with pytest.raises(expected):
            start_balance = w3.platone.getBalance(_transaction.get('from', w3.platone.accounts[0]))
            w3.platone.sendTransaction(_transaction)
    else:
        start_balance = w3.platone.getBalance(_transaction.get('from', w3.platone.accounts[0]))
        w3.platone.sendTransaction(_transaction)
        assert w3.platone.getBalance(_transaction.get('from')) <= start_balance + expected
