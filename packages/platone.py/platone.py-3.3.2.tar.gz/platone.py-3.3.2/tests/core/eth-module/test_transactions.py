...
from platone.middleware.simulate_unmined_transaction import (
...
def test_send_transaction_with_valid_chain_id(web3, make_chain_id, expect_success):
    transaction = {
        'to': web3.platone.accounts[1],
        'chainId': make_chain_id(web3),
    }
    if expect_success:
        # just be happy that we didn't crash
        web3.platone.sendTransaction(transaction)
    else:
...
def test_unmined_transaction_wait_for_receipt(web3, extra_accounts):
    ...
    assert web3.platone.getTransactionReceipt(txn_hash) is None
