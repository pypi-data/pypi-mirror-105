...
def test_no_gas_price_strategy_returns_none(web3):
    assert web3.platone.generateGasPrice() is None


def test_set_gas_price_strategy(web3):
    def my_gas_price_strategy(web3, transaction_params):
        return 5
    web3.platone.setGasPriceStrategy(my_gas_price_strategy)
    assert web3.platone.generateGasPrice() == 5
...
def test_gas_price_strategy_calls(web3):
    ...
    web3.platone.setGasPriceStrategy(my_gas_price_strategy)
    assert web3.platone.generateGasPrice(transaction) == 5
