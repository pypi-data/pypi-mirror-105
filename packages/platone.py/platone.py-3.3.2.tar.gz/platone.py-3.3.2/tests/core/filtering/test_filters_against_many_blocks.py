...
def deploy_contracts(web3, contract, wait_for_transaction):
    for i in range(25):
        ...
        yield web3.platone.getTransactionReceipt(tx_hash)['contractAddress']


def pad_with_transactions(w3):
    accounts = w3.platone.accounts
    for tx_count in range(random.randint(0, 10)):
        ...
        value = 50 + tx_count
        w3.platone.sendTransaction({'from': _from, 'to': _to, 'value': value})
...
def single_transaction(w3):
    ...
    _to = accounts[random.randint(0, len(accounts) - 1)]
    value = 50
    tx_hash = w3.platone.sendTransaction({'from': _from, 'to': _to, 'value': value})
    return tx_hash
...
def test_event_filter_new_events(
    ...
    while web3.platone.blockNumber < 50:
...
def test_block_filter(web3):
    block_filter = web3.platone.filter("latest")

    while web3.platone.blockNumber < 50:
        pad_with_transactions(web3)

    assert len(block_filter.get_new_entries()) == web3.platone.blockNumber


def test_transaction_filter_with_mining(
        web3):

    transaction_filter = web3.platone.filter("pending")
