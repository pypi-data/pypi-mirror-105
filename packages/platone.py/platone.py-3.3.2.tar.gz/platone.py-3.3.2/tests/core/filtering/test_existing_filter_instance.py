...
def test_instatiate_existing_filter(web3, sleep_interval, wait_for_block, filter_id):
    with pytest.raises(TypeError):
        web3.platone.filter('latest', filter_id)
    with pytest.raises(TypeError):
        web3.platone.filter('latest', filter_id=filter_id)
    with pytest.raises(TypeError):
        web3.platone.filter(filter_params='latest', filter_id=filter_id)

    block_filter = web3.platone.filter(filter_id=filter_id)

    current_block = web3.platone.blockNumber
    ...
    expected_block_hashes = [
        web3.platone.getBlock(n + 1).hash for n in range(current_block, current_block + 3)
    ]
