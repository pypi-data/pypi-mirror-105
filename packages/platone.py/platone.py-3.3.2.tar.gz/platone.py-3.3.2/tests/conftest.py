...
def wait_for_miner_start():
    def _wait_for_miner_start(web3, timeout=60):
        ...
        with Timeout(timeout) as timeout:
            while not web3.platone.mining or not web3.platone.hashrate:
...
def wait_for_block():
    def _wait_for_block(web3, block_number=1, timeout=None):
        if not timeout:
            timeout = (block_number - web3.platone.blockNumber) * 3
        poll_delay_counter = PollDelayCounter()
        with Timeout(timeout) as timeout:
            while True:
                if web3.platone.blockNumber >= block_number:
                    break
...
def wait_for_transaction():
    def _wait_for_transaction(web3, txn_hash, timeout=120):
        ...
        with Timeout(timeout) as timeout:
            while True:
                txn_receipt = web3.platone.getTransactionReceipt(txn_hash)
