...
from platone.utils.module_testing.emitter_contract import (
    EMITTER_ABI,
    EMITTER_BYTECODE,
    EMITTER_ENUM,
)
from platone.utils.module_testing.math_contract import (
    MATH_ABI,
    MATH_BYTECODE,
)
...
def verify_chain_state(web3, chain_data):
    receipt = web3.platone.waitForTransactionReceipt(chain_data['mined_txn_hash'])
    latest = web3.platone.getBlock('latest')
...
def mine_transaction_hash(web3, txn_hash):
    ...
    while time.time() < start_time + 60:
        receipt = web3.platone.waitForTransactionReceipt(txn_hash)
...
def mine_block(web3):
    origin_block_number = web3.platone.blockNumber
...
def deploy_contract(web3, name, factory):
    web3.personal.unlockAccount(web3.platone.coinbase, KEYFILE_PW)
    deploy_txn_hash = factory.constructor().transact({'from': web3.platone.coinbase})
...
def setup_chain_state(web3):
    ...
    # Math Contract
    #
    math_contract_factory = web3.platone.contract(
        abi=MATH_ABI,
    ...
    # Emitter Contract
    #
    emitter_contract_factory = web3.platone.contract(
        abi=EMITTER_ABI,
    ...
    block_with_log = web3.platone.getBlock(txn_receipt_with_log['blockHash'])
    ...
    empty_block = web3.platone.getBlock(empty_block_number)
    ...
    web3.miner.start(1)
    mined_txn_hash = web3.platone.sendTransaction({
        'from': coinbase,
        'to': coinbase,
        'value': 1,
        'gas': 21000,
    ...
    block_with_txn = web3.platone.getBlock(mined_txn_receipt['blockHash'])
