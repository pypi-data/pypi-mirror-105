...
from platone.utils.math import (
    percentile,
)
...
def _get_avg_block_time(w3, sample_size):
    latest = w3.platone.getBlock('latest')
    ...
    oldest = w3.platone.getBlock(latest['number'] - constrained_sample_size)
    return (latest['timestamp'] - oldest['timestamp']) / constrained_sample_size


def _get_raw_miner_data(w3, sample_size):
    latest = w3.platone.getBlock('latest', full_transactions=True)
    ...
    for _ in range(sample_size - 1):
        ...
        block = w3.platone.getBlock(block['parentHash'], full_transactions=True)
