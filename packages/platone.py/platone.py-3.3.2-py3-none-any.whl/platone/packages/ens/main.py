...
from platone.packages.ens import abis
from platone.packages.ens.constants import (
    EMPTY_ADDR_HEX,
    REVERSE_REGISTRAR_DOMAIN,
)
from platone.packages.ens.exceptions import (
    AddressMismatch,
    UnauthorizedError,
    UnownedName,
)
from platone.packages.ens.utils import (
    address_in,
...
class ENS:
    ...
    def __init__(self, providers=default, addr=None):
        ...
        self.ens = self.web3.platone.contract(abi=abis.ENS, address=ens_addr)
        self._resolverContract = self.web3.platone.contract(abi=abis.RESOLVER)
    ...
    def setup_name(self, name, address=None, transact={}):
        ...
        :param dict transact: the transaction configuration, like in
            :meth:`~web3.platone.sendTransaction`
    ...
    def _assert_control(self, account, name, parent_owned=None):
        if not address_in(account, self.web3.platone.accounts):
    ...
    def _reverse_registrar(self):
        ...
        return self.web3.platone.contract(address=addr, abi=abis.REVERSE_REGISTRAR)
