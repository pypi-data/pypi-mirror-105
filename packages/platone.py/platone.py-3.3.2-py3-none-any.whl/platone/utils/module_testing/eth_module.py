...
class EthModuleTest:
    ...
    def test_eth_accounts(self, web3):
        ...
        assert web3.platone.coinbase in accounts

    def test_eth_blockNumber(self, web3):
        block_number = web3.platone.blockNumber
    ...
    def test_eth_getBalance(self, web3):
        ...
        with pytest.raises(InvalidAddress):
            web3.platone.getBalance(coinbase.lower())

        balance = web3.platone.getBalance(coinbase)

        assert is_integer(balance)
        assert balance >= 0

    def test_eth_getStorageAt(self, web3, emitter_contract_address):
        storage = web3.platone.getStorageAt(emitter_contract_address, 0)
        assert isinstance(storage, HexBytes)

    def test_eth_getStorageAt_invalid_address(self, web3):
        coinbase = web3.platone.coinbase
        with pytest.raises(InvalidAddress):
            web3.platone.getStorageAt(coinbase.lower(), 0)

    def test_eth_getTransactionCount(self, web3, unlocked_account_dual_type):
        transaction_count = web3.platone.getTransactionCount(unlocked_account_dual_type)
    ...
    def test_eth_sendTransaction_addr_checksum_required(self, web3, unlocked_account):
        ...
        txn_params = {
            ...
            'value': 1,
            'gas': 21000,
            'gasPrice': web3.platone.gasPrice,
        }

        with pytest.raises(InvalidAddress):
            invalid_params = dict(txn_params, **{'from': non_checksum_addr})
            web3.platone.sendTransaction(invalid_params)
    ...
    def test_eth_sendTransaction(self, web3, unlocked_account_dual_type):
        ...
        txn_hash = web3.platone.sendTransaction(txn_params)
        txn = web3.platone.getTransaction(txn_hash)
    ...
    def test_eth_sendTransaction_with_nonce(self, web3, unlocked_account):
        txn_params = {
            ...
            'gasPrice': web3.platone.gasPrice * 2,
            'nonce': web3.platone.getTransactionCount(unlocked_account),
        }
        txn_hash = web3.platone.sendTransaction(txn_params)
        txn = web3.platone.getTransaction(txn_hash)
    ...
    def test_eth_replaceTransaction(self, web3, unlocked_account_dual_type):
        ...
        txn_params['gasPrice'] = web3.platone.gasPrice * 2
        replace_txn_hash = web3.platone.replaceTransaction(txn_hash, txn_params)
        replace_txn = web3.platone.getTransaction(replace_txn_hash)
    ...
    def test_eth_replaceTransaction_non_existing_transaction(
        ...
        with pytest.raises(ValueError):
            web3.platone.replaceTransaction(
    ...
    def test_eth_replaceTransaction_already_mined(self, web3, unlocked_account_dual_type):
        ...
        txn_params['gasPrice'] = web3.platone.gasPrice * 2
        with pytest.raises(ValueError):
            web3.platone.replaceTransaction(txn_hash, txn_params)
    ...
    def test_eth_replaceTransaction_gas_price_defaulting_strategy_higher(self,
        ...
        web3.platone.setGasPriceStrategy(higher_gas_price_strategy)
    ...
    def test_eth_replaceTransaction_gas_price_defaulting_strategy_lower(self,
        ...
        web3.platone.setGasPriceStrategy(lower_gas_price_strategy)
    ...
    def test_eth_modifyTransaction(self, web3, unlocked_account):
        ...
        modified_txn_hash = web3.platone.modifyTransaction(
            txn_hash, gasPrice=(txn_params['gasPrice'] * 2), value=2
        )
        modified_txn = web3.platone.getTransaction(modified_txn_hash)
    ...
    def test_eth_sendRawTransaction(self,
                                    ...
                                    expected_hash):
        txn_hash = web3.platone.sendRawTransaction(raw_transaction)
    ...
    def test_eth_call(self, web3, math_contract):
        ...
        call_result = web3.platone.call(txn_params)
    ...
    def test_eth_estimateGas(self, web3, unlocked_account_dual_type):
        gas_estimate = web3.platone.estimateGas({
            'from': unlocked_account_dual_type,
            'to': unlocked_account_dual_type,
            'value': 1,
        })
    ...
    def test_eth_getTransactionByHash_contract_creation(self,
        ...
        transaction = web3.platone.getTransaction(math_contract_deploy_txn_hash)
        assert is_dict(transaction)
    ...
    def test_eth_getTransactionReceipt_unmined(self, web3, unlocked_account_dual_type):
        txn_hash = web3.platone.sendTransaction({
            'from': unlocked_account_dual_type,
            'to': unlocked_account_dual_type,
            'value': 1,
            'gas': 21000,
        ...
        receipt = web3.platone.getTransactionReceipt(txn_hash)
    ...
    def test_eth_getTransactionReceipt_with_log_entry(self,
                                                      ...
                                                      emitter_contract,
                                                      txn_hash_with_log):
        receipt = web3.platone.getTransactionReceipt(txn_hash_with_log)
        assert is_dict(receipt)
    ...
    def test_eth_newFilter(self, web3):
        filter = web3.platone.filter({})

        changes = web3.platone.getFilterChanges(filter.filter_id)
        assert is_list_like(changes)
        assert not changes

        logs = web3.platone.getFilterLogs(filter.filter_id)
        assert is_list_like(logs)
        assert not logs

        result = web3.platone.uninstallFilter(filter.filter_id)
        assert result is True

    def test_eth_newBlockFilter(self, web3):
        filter = web3.platone.filter('latest')
    ...
    def test_eth_newPendingTransactionFilter(self, web3):
        filter = web3.platone.filter('pending')
        assert is_string(filter.filter_id)
    ...
    def test_eth_getLogs_without_logs(self, web3, block_with_txn_with_log):
        ...
        result = web3.platone.getLogs(filter_params)
        assert len(result) == 0
    ...
    def test_eth_call_old_contract_state(self, web3, math_contract, unlocked_account):
        start_block = web3.platone.getBlock('latest')
    ...
    def test_eth_uninstallFilter(self, web3):
        ...
        success = web3.platone.uninstallFilter(filter.filter_id)
        assert success is True

        failure = web3.platone.uninstallFilter(filter.filter_id)
