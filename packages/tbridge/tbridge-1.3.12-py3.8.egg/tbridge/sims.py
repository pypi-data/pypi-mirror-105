...
def _process_bin(b, config_values, separate_mags=None, provided_bgs=None,
    ...
    job_list = []
    full_profile_list, bg_infolist = [[] for i in range(profiles_per_row)], []
...
def _process_model(sersic_model, config, model_params, provided_bgs=None):
    ...
    if config["BG_PARAMS"] == "ellipse":
        bg_median, bg_mean, bg_std = tbridge.estimate_bg_elliptical_annulus(bg_added_model,
        ...
        bg_median, bg_mean, bg_std = tbridge.estimate_bg_annulus(bg_added_model, dynamic=True, annulus_width=50)
    else:
        bg_median, bg_mean, bg_std = tbridge.estimate_background(bg_added_model)
