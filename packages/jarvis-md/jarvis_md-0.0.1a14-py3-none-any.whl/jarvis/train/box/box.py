...
class BoundingBox():
    ...
    def __init__(self, image_shape, classes=1, c=[3, 4, 5], 
        ...
        iou_upper=0.7, iou_lower=0.3, iou_nms=0.7, box_padding=0, min_size_blob=0.1, cls_threshold=0.5, separate_maps=True):
        """
        ...
        :params
          ...
          (float)  min_size_blob   : remove blobs below this size before converting mask to box
    ...
    def calculate_boxes(self, msk, min_size=None):
        """
        ...
        min_size = min_size or self.params['min_size_blob']
        min_size = min_size * sizes.max()
