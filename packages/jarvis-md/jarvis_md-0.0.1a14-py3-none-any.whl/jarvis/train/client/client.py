...
class Client():
    ...
    def get(self, split=None, cohort=None, row=None, rows=None, infos=None, test=False, db=None, **kwargs):
        """
        ...
        for k in ['xs', 'ys']:
            for key in arrays[k]:
                ...
                arrays[k][key] = arrays[k][key].astype(dtype)
    ...
    def generator(self, split, batch_size=None, **kwargs):
        ...
        while True:
            ...
            xs = {k: np.stack([x[k] for x in xs]).reshape([-1] + self.specs['xs'][k]['shape']['input']) for k in self.specs['xs']}
            ys = {k: np.stack([y[k] for y in ys]).reshape([-1] + self.specs['ys'][k]['shape']['input']) for k in self.specs['ys']}
