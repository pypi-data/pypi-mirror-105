...
def load_netcdf(filename, var=None, gridXY=None, plot=False, cmap=None, **kwargs):
    ...
    if not gridXY:
        rng_lat = index_arr(kwargs['west'],  ncfile[axes['y']][:].data), index_arr(kwargs['east'],  ncfile[axes['y']][:].data)
        rng_lon = index_arr(kwargs['south'], ncfile[axes['x']][:].data), index_arr(kwargs['north'], ncfile[axes['x']][:].data)
    else:
        assert False
    '''
        ncgrid = netCDF4.Dataset(gridXY)
        rng_lat = index_arr(kwargs['west'],  ncgrid['latitude'][:].data), index_arr(kwargs['east'],  ncgrid['latitude'][:].data)
        rng_lon = index_arr(kwargs['south'], ncfile[axes['x']][:].data), index_arr(kwargs['north'], ncfile[axes['x']][:].data)
    '''
