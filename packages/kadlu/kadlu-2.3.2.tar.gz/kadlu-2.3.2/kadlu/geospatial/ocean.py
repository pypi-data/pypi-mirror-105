...
class Ocean():
    ...
    def __init__(self, /, *, 
        ...
        # compute interpolation in parallel and store attribute
        def set_attributes():
            ...
            x = 0
            while (x := x + 1) <= len(vartypes):
                obj = q.get()
                self.interps[obj[0]] = obj[1]
                setattr(self, obj[0],         eval(f'lambda lat, lon, {"depth," if (z := obj[0] in var3d) else ""} grid=False, **kw: obj[1].interp(lat, lon, {"depth," if z else ""} grid, **kw)', dict(obj=obj)))
                setattr(self, f'{obj[0]}_xy', eval(f'lambda x, y,     {"z,"     if (z := obj[0] in var3d) else ""} grid=False, **kw: obj[1].interp_xy(x, y,  {"z,"     if z else ""} grid, **kw)', dict(obj=obj)))
