...
def logmsg(source, var, ntup=(), **kwargs):
    logging.info(f'{source.upper()} {var.lower()}  logged {reduce(np.subtract, ntup[::-1])} points in region {json.dumps(kwargs, default=str)}')

def logmsg_nodata(source, var, **kwargs):
    logging.warning(f'{source.upper()} {var.lower()}: no data found, returning empty arrays {json.dumps({k:v for k,v in kwargs.items() if k in ("north","south","west","east","top","bottom","start","end")} , default=str)}')
